...
- [ ] 6.5 Follow RED -> GREEN -> REFACTOR to complete the resumable `workflow propose` happy path through exact review, challenge disposition, planning authorization, conflict recovery, and typed terminal output using the already-shared live validator.
- [ ] 6.6 Follow RED -> GREEN -> REFACTOR to make CI and local archive call the same pure validators, resolve immutable governing generations from managed Git history, and replay legacy plus non-default v2 plans without relying on runtime objects or provider credentials.
