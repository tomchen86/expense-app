...
- Add an engine-owned Investigation Session: deterministically derive a non-removable search floor from every available typed path/symbol/config/rename/removal/mirror fact, accept typed main-agent terms, launch an ordinarily provider-independent blind survey, preview and seal the union, and scan the complete pinned Git-tracked tree without using task `allowedPaths` as a breadth boundary.
...
- Add exact-tree advisory PlanReview as the second scope/depth challenge. Hard gates cover presence, freshness, target binding, independence or an eligible human continuation grant, and challenge disposition; the AI verdict remains advisory. Reviewer `proposedTerms` enter one bounded deterministic term projector directly, while unrelated semantic suggestions cannot mutate lifecycle state or create issues.
- Add a collaboration-continuation grant, separate from authority maintenance, that only a human maintainer at the controlling TTY can issue to authorize an enumerated same-provider role conflict when no alternate provider is available. It cannot suppress investigation, scope, checks, freshness, or managed Git authority, and degraded independence remains visible downstream.
...
- Make mutation classes visible to T1.5 breadth evidence, but leave cross-agent TDD implementation, mechanical transformation/retirement closure, mid-task revision, exact-diff/finalize, pre-merge coverage composition, and final surface migration to their ordered T2 changes.

Scope is limited to investigation-first planning, read-only provider orchestration, exact-plan review, degraded-planning grants, their OpenSpec/schema integration, CI-replayable validation, tests, and executable workflow guidance. It does not change application behavior, database policy, remote merge authority, or the existing task completion/commit/archive authority model.
