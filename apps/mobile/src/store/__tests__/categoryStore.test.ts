...
describe("CategoryStore", () => {
  ...
  it("warns and returns the existing category when adding a duplicate name", () => {
    ...
    const categories = useCategoryStore.getState().categories.filter(
      (category) => category.name === existing.name,
    );

    expect(result).toEqual(existing);
    expect(categories).toHaveLength(1);
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining(`Category with name "${existing.name}" already exists.`),
    );
  });
  ...
  it("updates the color when adding an existing category with a new color", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('Category with name "Transportation" already exists.'),
    );
  });
  ...
  it("prevents deleting the reserved 'Other' category", () => {
    ...
    expect(categories.find((category) => category.name === "Other")).toBeDefined();
