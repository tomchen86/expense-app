...
describe("ComposedExpenseStore", () => {
  ...
  it("manages categories across composed and feature stores", () => {
    ...
    expect(useCategoryStore.getState().getCategoryByName("Fitness")?.color).toBe(
      "#00FF00",
    );

    store.deleteCategory(created.id);
    expect(useCategoryStore.getState().getCategoryByName("Fitness")).toBeUndefined();
  });
  ...
  it("removes a participant across stores and expenses", () => {
    ...
    const composedParticipants = useComposedExpenseStore
      .getState()
      .participants;
  ...
  it("manages group participant membership helpers", () => {
    ...
    useParticipantStore.getState().addParticipant(otherParticipant.name, otherParticipant.id);
