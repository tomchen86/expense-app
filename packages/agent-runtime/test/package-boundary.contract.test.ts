...
test('@jigwright/agent-runtime has one-way public package dependencies', () => {
  ...
  assert.equal(manifest.name, '@jigwright/agent-runtime');
  ...
  for (const file of fs.readdirSync(path.join(packageRoot, 'src'))) {
    ...
    for (const match of source.matchAll(/from\s+['"]([^'"]+)['"]/gu)) {
      ...
      assert.equal(
        /workflow-engine|@jigwright\/grants|openspec|expense/iu.test(specifier),
        false,
