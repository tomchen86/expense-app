...
import { readProviderRepairLineage, } from '../provider-execution/provider-execution-governance.js';
...
export function providerRetryPolicyFromAccounting(accounting, createdAt) {
    const createdAtMs = Date.parse(createdAt);
    if (!Number.isFinite(createdAtMs)) {
        throw executionProjectionConflict('Provider retry accounting has an invalid Job creation timestamp.');
    }
    return deepFreeze({
        ...
        providerLimits: Object.fromEntries(Object.entries(accounting.providerLimits).sort(([left], [right]) => left.localeCompare(right))),
    });
}
export function decideLegacyExecutionFailure(input) {
    const failure = input.attempt.failure;
    if (failure === null) {
        throw executionProjectionConflict('A durable retry decision requires a failed Attempt.');
    }
    const sameFingerprintCount = input.attempts.filter(({ failureFingerprint }) => failureFingerprint !== null && failureFingerprint === failure.fingerprint).length;
    ...
    const requiredGrant = createExecutionBudgetGrantRequest({
        ...
        requestedChanges,
        rationale: 'Automatic retry budget is exhausted; authorize one bounded replacement Attempt with the same semantic context.',
...
export function executionStorePaths(paths) {
...
export function executionJobStatePath(paths, requestedJobId) {
    const jobId = assertExecutionJobId(requestedJobId);
    return path.join(executionStorePaths(paths).jobs, `${sha256Text(jobId)}.json`);
}
export function readExecutionJobState(paths, requestedJobId) {
    const jobId = assertExecutionJobId(requestedJobId);
    return readExecutionJobStateUnlocked(paths, jobId);
}
export function listExecutionJobStates(paths) {
    const stores = executionStorePaths(paths);
    const root = fs.lstatSync(stores.root, { throwIfNoEntry: false });
    if (root === undefined)
        return [];
    try {
        assertPrivateInvestigationDirectory(paths, stores.root, executionStoreUnsafe);
        const jobs = fs.lstatSync(stores.jobs, { throwIfNoEntry: false });
        if (jobs === undefined)
            return [];
        assertPrivateInvestigationDirectory(paths, stores.jobs, executionStoreUnsafe);
        const states = fs
            ...
            const value = assertDurableExecutionJobState(readPrivateCanonicalJson(paths, path.join(stores.jobs, entry.name), executionStoreUnsafe));
        ...
        if (new Set(jobIds).size !== jobIds.length)
            throw executionStoreUnsafe();
        return deepFreeze(states.sort((left, right) => left.job.jobId.localeCompare(right.job.jobId)));
    }
    catch (error) {
 ...
 */
export function materializeLegacyProviderExecutionJob(paths, entries, options = {}) {
 ...
 */
export function acceptLegacyProviderAttemptResult(paths, input) {
    ...
    return withExecutionJobLock(paths, prepared.jobId, () => {
        ...
        const attemptIndex = materialized.attempts.findIndex(({ attemptId }) => attemptId === input.attemptId);
        if (attemptIndex < 0) {
            throw executionProjectionConflict('The submitted legacy Attempt is not part of the projected Job.');
        }
        const outputDigest = normalizeDigest(input.outputDigest);
        const existingResult = materialized.results.find(({ attemptId }) => attemptId === input.attemptId);
        if (existingResult !== undefined) {
            if (existingResult.outputDigest !== outputDigest) {
                throw workflowError('EXECUTION_STORE_RESULT_CONFLICT', 'The durable Attempt already has a different result digest.', ExitCode.conflict);
            }
        ...
        const results = [...materialized.results, outcome.result].sort((left, right) => attemptNumber(attempts, left.attemptId) -
...
export function reconcileLegacyProviderExecutionJob(paths, input) {
    const prepared = prepareLegacyEntries(paths, input.entries);
    const trigger = prepared.entries.find(({ record }) => record.invocationId === input.triggerInvocationId);
    if (trigger === undefined ||
        ...
        throw workflowError('EXECUTION_RECONCILIATION_LEGACY_REVISION_MISMATCH', 'The trigger invocation does not have the expected legacy revision.', ExitCode.staleState, {
            details: {
    ...
    return withExecutionJobLock(paths, prepared.jobId, () => {
        const current = readExecutionJobStateUnlocked(paths, prepared.jobId);
        if (current === null) {
            throw workflowError('EXECUTION_RECONCILIATION_STATE_MISSING', 'Reconciliation requires an existing durable execution aggregate.', ExitCode.staleState);
        }
        const replay = current.reconciliationReceipts.find((receipt) => receipt.receiptId === receiptId);
        if (replay !== undefined) {
        ...
        if (materialized.job.acceptedAttemptId !== current.job.acceptedAttemptId) {
            throw workflowError('EXECUTION_RECONCILIATION_WINNER_CHANGE_REJECTED', 'Reconciliation cannot select or replace the accepted Attempt.', ExitCode.guard);
        }
        const currentSources = new Map(current.legacyProjection.invocations.map((source) => [
            source.invocationId,
            source.legacyRevision,
        ]));
        const reconciledInvocationIds = materialized.legacyProjection.invocations
            .filter((source) => currentSources.get(source.invocationId) !== source.legacyRevision)
...
function prepareLegacyEntries(paths, input) {
    if (input.length < 1 || input.length > MAX_LEGACY_ATTEMPTS) {
        throw executionProjectionConflict('Legacy execution materialization requires a bounded non-empty history.');
    }
    let entries;
    try {
        entries = input
        ...
        throw executionProjectionConflict('Legacy invocation history cannot be projected into execution records.', error);
    }
    ...
    for (let index = 0; index < entries.length; index += 1) {
        ...
        if ((index > 0 &&
            ...
            throw executionProjectionConflict('Legacy invocation history has duplicate Attempts or conflicting semantic identity.');
        }
    }
    return deepFreeze({
...
function buildLegacyExecutionState(prepared, current) {
    ...
    if (current !== null &&
        ...
        throw executionProjectionConflict('Durable execution state conflicts with the legacy semantic identity.');
    }
    assertLegacyProjectionMonotonic(prepared, current);
    const frozenRetryPolicy = firstEntry.retryAccounting === null
        ? (current?.job.retryPolicy ?? first.job.retryPolicy)
        : providerRetryPolicyFromAccounting(firstEntry.retryAccounting.retryPolicy, firstEntry.record.createdAt);
    const currentResults = new Map((current?.results ?? []).map((result) => [result.attemptId, result]));
    ...
    if (acceptedAttemptId === null) {
        const firstSuccess = [...prepared.entries]
            ...
            .sort((left, right) => Date.parse(left.record.updatedAt) -
            Date.parse(right.record.updatedAt) ||
            left.record.invocationId.localeCompare(right.record.invocationId))[0];
    ...
    for (let index = 0; index < prepared.entries.length; index += 1) {
        ...
        const chargedProjection = entry.retryAccounting === null
            ? projected
            : assertAttemptRecord({
                ...projected,
                runtimeMs: entry.retryAccounting.attemptReservation.runtimeMs,
                providerCostMicros: entry.retryAccounting.attemptReservation.providerCostMicros,
                providerTokens: entry.retryAccounting.attemptReservation.providerTokens,
            });
        const previous = attempts.at(-1);
        const previousEntry = prepared.entries[index - 1];
        const hasAdjacentPrevious = previous !== undefined &&
        ...
        const projectedAcceptance = entry.record.state !== 'succeeded'
            ? null
        ...
        if (hasAdjacentPrevious &&
            ...
            throw workflowError('PROVIDER_REPAIR_LINEAGE_REQUIRED', 'A repairable predecessor requires durable repair lineage before materialization.', ExitCode.guard);
        }
        ...
        if (entry.repairLineage !== null) {
            ...
            if (!hasAdjacentPrevious ||
                ...
                throw executionProjectionConflict('Durable repair lineage does not bind adjacent legacy Attempts.');
            }
            const priorRepairCount = attempts.filter(({ retryMode }) => retryMode === 'repair').length;
            ...
            if (replacement.attempt.attemptNumber !== projected.attemptNumber ||
                ...
                throw executionProjectionConflict('Repair Attempt does not preserve the projected attempt number.');
            }
            ...
            status: effectiveAcceptance === 'accepted'
                ...
                : effectiveAcceptance === 'late-duplicate'
                    ...
                    : effectiveAcceptance === 'stale'
                        ? 'stale'
                        : projected.status,
            retention: effectiveAcceptance === 'late-duplicate' ||
                effectiveAcceptance === 'stale'
                ? 'debug'
                : projected.retention,
            updatedAt: result === undefined
                ? projected.updatedAt
    ...
    for (const [index, entry] of prepared.entries.entries()) {
        ...
        const legacyOutputDigest = normalizeDigest(entry.record.result.outputDigest);
        const existingResult = results.get(attempt.attemptId);
        if (existingResult !== undefined) {
            if (existingResult.outputDigest !== legacyOutputDigest) {
                throw executionProjectionConflict('Legacy and durable Attempt result digests diverge.');
            }
            continue;
        }
        results.set(attempt.attemptId, createAttemptResult(attempt, legacyOutputDigest, attempt.attemptId === acceptedAttemptId ? 'accepted' : 'late-duplicate', entry.record.updatedAt));
    }
    const orderedResults = [...results.values()].sort((left, right) => attemptNumber(attempts, left.attemptId) -
        attemptNumber(attempts, right.attemptId));
    ...
    const providerCostMicros = sumAttemptAccounting(attempts, 'providerCostMicros');
    ...
    let job = assertJobRecord({
        ...(current?.job ?? first.job),
        status: acceptedAttemptId === null ? latestProjection.job.status : 'succeeded',
        acceptedAttemptId,
    ...
    return assertDurableExecutionJobState({
        ...
        legacyProjection: {
            ...
            completeHistory: attempts.every(({ attemptNumber }, index) => attemptNumber === index + 1),
...
function assertLegacyProjectionMonotonic(prepared, current) {
    if (current === null)
        return;
    const incoming = new Map(prepared.entries.map(({ record }) => [
        record.invocationId,
        record.revision,
    ]));
    for (const prior of current.legacyProjection.invocations) {
        ...
        if (nextRevision === undefined || nextRevision < prior.legacyRevision) {
            throw executionProjectionConflict('Legacy execution projection is missing history or moved backwards.');
        }
    }
}
function persistExecutionJobState(paths, current, input) {
    ...
    if (current === null) {
        createPrivateCanonicalJson(paths, statePath, next, executionStoreUnsafe, 'EXECUTION_STORE_COLLISION');
    }
    else {
        writePrivateCanonicalJsonAtomic(paths, statePath, next, executionStoreUnsafe);
    }
...
function readExecutionJobStateUnlocked(paths, jobId) {
    const statePath = executionJobStatePath(paths, jobId);
    try {
        if (!privatePathExists(paths, statePath, executionStoreUnsafe))
            return null;
        const state = assertDurableExecutionJobState(readPrivateCanonicalJson(paths, statePath, executionStoreUnsafe));
...
function assertDurableExecutionJobState(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'attempts',
            'job',
    ...
    try {
        ...
        reconciliationReceipts = value.reconciliationReceipts.map((receipt) => assertExecutionReconciliationReceipt(receipt, job.jobId));
    }
    catch (error) {
        throw executionStoreUnsafe(error);
    }
    ...
    for (const attempt of attempts) {
        if (['succeeded', 'late-duplicate'].includes(attempt.status) !==
            resultAttempts.has(attempt.attemptId)) {
            throw executionStoreUnsafe();
        }
    }
    if (reconciliationReceipts.some((receipt, index) => receipt.resultingExecutionRevision > value.revision ||
        (index > 0 &&
            receipt.receiptId <= reconciliationReceipts[index - 1].receiptId)) ||
        value.legacyProjection.completeHistory !==
            attempts.every(({ attemptNumber }, index) => attemptNumber === index + 1) ||
    ...
    return deepFreeze({
        ...
        legacyProjection: {
            ...
            invocations: value.legacyProjection.invocations.map((source) => ({
                ...
                legacyRevision: Number(source.legacyRevision),
                attemptId: String(source.attemptId),
            })),
        },
    });
}
function withExecutionJobLock(paths, jobId, operation) {
    return withPrivateRuntimeLock(paths, path.join(paths.locks, `execution-job-${sha256Text(jobId)}.lock`), operation, 'EXECUTION_STORE_OPERATION_CONFLICT', executionStoreUnsafe);
}
function assertExpectedRevision(current, expectedRevision) {
    ...
    if ((expectedRevision !== null &&
        ...
        throw workflowError('EXECUTION_STORE_CAS_MISMATCH', 'The durable execution aggregate revision changed.', ExitCode.staleState, {
            details: {
                expectedRevision,
...
function assertExecutionReconciliationReceipt(value, expectedJobId) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'acceptedAttemptId',
        ...
        value.expectedExecutionRevision < 0 ||
        ...
        value.resultingExecutionRevision !==
            value.expectedExecutionRevision + 1 ||
        ...
        value.reconciledInvocationIds.some((invocationId) => typeof invocationId !== 'string' ||
            !EXECUTION_JOB_ID.test(invocationId)) ||
        canonicalJson(value.reconciledInvocationIds) !==
            canonicalJson([...new Set(value.reconciledInvocationIds)].sort()) ||
    ...
    const expectedReceiptId = reconciliationReceiptId({
        ...
        expectedLegacyRevision: value.expectedLegacyRevision,
    });
...
function assertReconciliationReplay(current, prepared, receipt) {
    const materialized = buildLegacyExecutionState(prepared, current);
    if (current.revision < receipt.resultingExecutionRevision ||
        ...
        throw workflowError('EXECUTION_RECONCILIATION_RECEIPT_STALE', 'The existing reconciliation receipt no longer matches durable state.', ExitCode.staleState);
    }
}
function durableExecutionStateDigest(state) {
    return digestCanonical(state);
}
function durableExecutionProjectionDigest(state) {
    return digestCanonical({
...
function canonicalStateIgnoringRevision(state) {
...
function attemptNumber(attempts, attemptId) {
    const attempt = attempts.find((candidate) => candidate.attemptId === attemptId);
    if (attempt === undefined)
        throw executionStoreUnsafe();
    return attempt.attemptNumber;
}
function sumAttemptAccounting(attempts, field) {
    let total = 0;
    for (const attempt of attempts) {
        total += attempt[field];
        if (!Number.isSafeInteger(total)) {
            throw executionProjectionConflict('Provider retry accounting exceeds the durable integer range.');
        }
    }
    return total;
}
...
function assertExecutionJobId(value) {
    if (typeof value !== 'string' || !EXECUTION_JOB_ID.test(value)) {
        throw workflowError('EXECUTION_STORE_JOB_ID_INVALID', 'The durable execution Job identifier is invalid.', ExitCode.usage);
    }
    return value;
}
...
function executionProjectionConflict(message, cause) {
    return workflowError('EXECUTION_STORE_PROJECTION_CONFLICT', message, ExitCode.guard, cause === undefined ? {} : { details: { cause: errorMessage(cause) } });
}
function executionStoreUnsafe(cause) {
    return workflowError('EXECUTION_STORE_UNSAFE', 'The private durable execution store is malformed or unsafe.', ExitCode.unsafeEnvironment, cause === undefined ? {} : { details: { cause: errorMessage(cause) } });
}
