...
export function providerRetentionRoot(paths) {
...
export function providerRetentionReceiptPath(paths, requestedInvocationId) {
    const invocationId = assertInvocationId(requestedInvocationId);
    return path.join(providerRetentionRoot(paths).receipts, `${sha256(invocationId)}.json`);
}
export function providerRetentionStagingDirectory(paths, requestedInvocationId) {
    const invocationId = assertInvocationId(requestedInvocationId);
    return path.join(providerRetentionRoot(paths).staging, sha256(invocationId));
}
export function createProviderRetentionReceipt(input) {
    const receipt = {
        ...input,
...
export function providerRetentionReceiptId(input) {
...
export function canonicalProviderRetentionReceipt(receipt) {
    return `${canonicalJson(assertProviderRetentionReceipt(receipt))}\n`;
}
export function readProviderRetentionReceipt(paths, requestedInvocationId) {
    const invocationId = assertInvocationId(requestedInvocationId);
    if (!assertRetentionDirectories(paths, false))
        return null;
    ...
    let raw;
    try {
        const descriptor = fs.openSync(receiptPath, fs.constants.O_RDONLY |
        ...
        try {
            ...
            if (bytes.byteLength >= PROVIDER_RETENTION_MAX_RECEIPT_BYTES ||
        ...
        if (isRetentionReceiptError(error))
            throw error;
        throw receiptUnsafe();
    }
    let value;
...
export function readCompleteProviderRetentionReceipt(paths, requestedInvocationId, binding) {
    const receipt = readProviderRetentionReceipt(paths, requestedInvocationId);
    if (receipt === null)
        return null;
    if (receipt.state !== 'complete' || receipt.completedAt === null) {
        throw workflowError('PROVIDER_RETENTION_PRUNING_PARTIAL', 'Provider runtime pruning is incomplete and requires recovery.', ExitCode.staleState);
    }
...
export function providerRetentionArtifact(receipt, name) {
    return receipt.artifacts.find((artifact) => artifact.name === name) ?? null;
}
export function providerRetentionReviewRootArtifact(leaves) {
    if (leaves.length < 1 ||
        leaves.length > 4_096 ||
        leaves.some((leaf, index) => leaf.name !==
...
function assertProviderRetentionReceipt(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'artifacts',
            'acceptedAttemptId',
    ...
    if (value.receiptId !==
        providerRetentionReceiptId({
            ...
            legacyRevision: value.legacyRevision,
            terminalAt: value.terminalAt,
            artifacts,
            executionRevision: value.executionRevision,
            executionStateDigest: value.executionStateDigest,
            attemptRetention: value.attemptRetention,
            acceptedAttemptId: value.acceptedAttemptId === null ? null : value.acceptedAttemptId,
        })) {
        throw receiptUnsafe();
    }
    const payload = { ...value };
...
function assertArtifact(value) {
    if (!isRecord(value) ||
        ...
        !RETENTION_ARTIFACTS.has(value.name) ||
...
export function assertRetentionDirectories(paths, required) {
    ...
    if (!investigationRoot.isDirectory() ||
...
function isRetentionReceiptError(error) {
...
function receiptUnsafe() {
    return workflowError('PROVIDER_RETENTION_RECEIPT_UNSAFE', 'Provider runtime pruning receipt is missing, malformed, or tampered.', ExitCode.unsafeEnvironment);
}
...
function isNormalizedDigest(value) {
