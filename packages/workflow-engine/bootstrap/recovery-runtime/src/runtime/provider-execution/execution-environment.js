 ...
 */
export function createProviderExecutionEnvironment(providerId, nodeExecutable, temporaryDirectory, sourceEnvironment) {
    const nodeDirectory = path.dirname(fs.realpathSync(nodeExecutable));
    const temporary = fs.realpathSync(temporaryDirectory);
    const environment = {
        PATH: [...new Set([nodeDirectory, ...systemExecutableDirectories()])].join(path.delimiter),
        TMPDIR: temporary,
        TMP: temporary,
        TEMP: temporary,
        TERM: 'dumb',
    ...
    const configVariable = providerId === 'codex' ? 'CODEX_HOME' : 'CLAUDE_CONFIG_DIR';
...
export function createTrustedExecutionEnvironment(executables = []) {
    const nodeExecutable = fs.realpathSync(process.execPath);
    const executableDirectories = [
        path.dirname(nodeExecutable),
        ...executables.map((executable) => path.dirname(fs.realpathSync(executable))),
        ...systemExecutableDirectories(),
    ];
    const temporaryDirectory = fs.realpathSync(process.platform === 'win32' ? 'C:\\Windows\\Temp' : '/tmp');
    const environment = {
...
function systemExecutableDirectories() {
    if (process.platform === 'win32') {
