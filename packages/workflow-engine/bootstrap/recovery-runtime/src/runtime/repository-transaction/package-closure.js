...
export function collectWorkspacePackageClosure(repositoryRoot, workspaceManifestPath, workspaceManifest) {
    const workspaceDependencies = declaredDependencies(workspaceManifest, true);
    const workspaceIdentities = new Map(workspaceDependencies.map((dependency) => [
        dependency.name,
        declaredPackageIdentity(dependency.name, dependency.spec),
    ]));
    const pending = workspaceDependencies.flatMap((dependency) => {
        const resolved = resolveDeclaredPackage(repositoryRoot, workspaceManifestPath, dependency.name, dependency.spec, dependency.optional);
    ...
    while (pending.length > 0) {
        ...
        for (const dependency of declaredDependencies(manifest, false)) {
            if (createRequire(currentManifestPath).resolve.paths(dependency.name) ===
                null) {
                continue;
            }
            const resolved = resolveDeclaredPackage(repositoryRoot, currentManifestPath, dependency.name, dependency.spec, dependency.optional, declaredPackageIdentity(dependency.name, dependency.spec) ===
                dependency.name
...
export function collectInstalledPackageClosure(repositoryRoot, packageManifestPath) {
    ...
    while (pending.length > 0) {
        ...
        for (const dependency of declaredDependencies(manifest, false)) {
            const resolved = resolveDeclaredPackage(repositoryRoot, currentManifestPath, dependency.name, dependency.spec, dependency.optional);
            if (resolved) {
                pending.push(resolved.manifestPath);
            }
        }
...
export function resolveDeclaredPackage(repositoryRoot, requiringManifestPath, packageName, declaredSpec, optional = false, alternateIdentity) {
    if (!isPackageName(packageName)) {
        throw new Error('unsafe package name');
    }
    const searchPaths = createRequire(requiringManifestPath).resolve.paths(packageName) ?? [];
    for (const searchPath of searchPaths) {
        ...
        if (installedName !== declaredIdentity &&
            installedName !== alternateIdentity) {
            throw new Error(`resolved package ${packageName} identity does not match its declaration`);
        }
...
function declaredDependencies(manifest, includeDevelopment) {
    ...
    return [...names].sort().map((name) => {
        ...
        const identities = new Set(specs.map((spec) => declaredPackageIdentity(name, spec)));
...
function listPackageEntries(repositoryRoot, packageDirectory) {
    const entries = [];
...
function declaredPackageIdentity(packageName, declaredSpec) {
...
function packageNameFromAlias(value) {
    const versionSeparator = value.startsWith('@')
        ? value.indexOf('@', value.indexOf('/') + 1)
        : value.indexOf('@');
    const packageName = versionSeparator === -1 ? value : value.slice(0, versionSeparator);
    if (!isPackageName(packageName)) {
...
function isPackageName(value) {
    ...
    return ((segments.length === 1 || segments.length === 2) &&
        segments.every((segment) => /^[a-z0-9](?:[a-z0-9._-]*[a-z0-9])?$/.test(segment)));
}
...
function assertNoLoadAsFileShadow(packageEntry) {
    for (const extension of ['.js', '.json', '.node']) {
        if (fs.lstatSync(`${packageEntry}${extension}`, { throwIfNoEntry: false })) {
