...
import { readInvestigationDispositionNode, readInvestigationGroupNode, readInvestigationHitNode, } from './investigation-groups.js';
import { assertPathIdentity as assertCanonicalPathIdentity } from './mutation-class-policy.js';
...
const WHY_POLICY_DIGEST = sha256(canonicalJson({ schema: 'investigation.why-policy.v1' }));
/**
...
export function deriveInvestigationFullBlobManifest(input) {
    assertDigest(input.snapshot.treeDigest, 'Investigation snapshot tree digest is malformed.');
    ...
    return [...perPath.values()]
        .map((accumulator) => buildManifestEntry(accumulator, input.snapshot, entryByPath))
        .sort((left, right) => Buffer.compare(rawPath(left.path), rawPath(right.path)));
}
/**
...
export function createInvestigationWhyNodes(input) {
    ...
    const dispositionNodeById = new Map([...dispositionRecords].map(([nodeId, record]) => [
        nodeId,
        record.node,
    ]));
    const answerByEntry = new Map();
    ...
    return manifest.map((entry) => buildWhyNode(entry, answerByEntry.get(entry.manifestEntryId), hitNodeById, dispositionNodeById));
}
export function readInvestigationWhyNode(node) {
    const validated = assertStoredEvidenceNode(node, whyInvalid);
    if (validated.type !== WHY_TYPE ||
    ...
    const manifestEntryId = assertDigest(output.manifestEntryId, 'WHY manifest row identity is malformed.');
    const path = assertPathIdentity(output.path);
    const treeDigest = assertDigest(output.treeDigest, 'WHY tree digest is malformed.');
    ...
    const dispositionNodeIds = assertSortedUniqueDigests(output.dispositionNodeIds);
    const relevantLocations = assertArray(output.relevantLocations).map(assertRelevantLocation);
    const relationshipsToChange = assertArray(output.relationshipsToChange).map(assertRelationshipToChange);
    ...
    assertNodeRoles(validated, ['answer', 'manifest'], [
        ...coveredHitIds.map((_, index) => `hit-${index}`),
        ...dispositionNodeIds.map((_, index) => `disposition-${index}`),
    ], whyInvalid);
    const expectedProvenance = parentProvenance(coveredHitIds, dispositionNodeIds);
    if (canonicalJson(validated.provenanceParentNodeIds) !==
...
export function validateInvestigationWhyEvidence(input) {
    ...
    const hitRecords = indexHitNodes(input.hitNodes);
    const dispositionRecords = indexDispositionNodes(input.dispositionNodes);
    const byId = new Map(manifest.map((entry) => [
        entry.manifestEntryId,
        entry,
    ]));
    const byPath = new Map(manifest.map((entry) => [
    ...
    for (const node of input.whyNodes) {
        ...
        if (canonicalJson(whyRowView(why)) !== canonicalJson(manifestRowView(current))) {
            throw whyInvalid('WHY row output does not match the recomputed manifest.');
        }
...
function buildManifestEntry(accumulator, snapshot, entryByPath) {
    ...
    const dispositionNodeIds = [...accumulator.dispositionNodeIds].sort(compareString);
    ...
    const relationshipsToChange = [...accumulator.relationships.values()].sort((left, right) => compareString(left.dispositionNodeId, right.dispositionNodeId));
    const row = {
...
function buildWhyNode(entry, answer, hitNodeById, dispositionNodeById) {
...
function resolveNode(nodesById, nodeId, role) {
...
function assertWhyAnswer(value) {
    const record = assertExactKeys(value, WHY_ANSWER_KEYS, whyInvalid);
    const manifestEntryId = assertDigest(record.manifestEntryId, 'WHY answer manifest row identity is malformed.');
    const why = assertSemanticText(record.why, 'why');
    const protectedInvariant = assertSemanticText(record.protectedInvariant, 'protected invariant');
    const reviewerQuestion = assertSemanticText(record.reviewerQuestion, 'reviewer question');
    const answer = assertSemanticText(record.answer, 'answer');
    if (typeof record.semanticAuthor !== 'string' ||
...
function assertWhySemanticFields(output) {
    const why = assertSemanticText(output.why, 'why');
    const protectedInvariant = assertSemanticText(output.protectedInvariant, 'protected invariant');
    const reviewerQuestion = assertSemanticText(output.reviewerQuestion, 'reviewer question');
    const answer = assertSemanticText(output.answer, 'answer');
    if (typeof output.semanticAuthor !== 'string' ||
...
function assertSemanticText(value, label) {
...
function assertRelevantLocation(value) {
    const record = assertExactKeys(value, ['path', 'surface', 'byteOffset', 'byteLength', 'termId', 'hitId'], whyInvalid);
    const path = assertPathIdentity(record.path);
    if ((record.surface !== 'path' && record.surface !== 'content') ||
...
function assertRelationshipToChange(value) {
    const record = assertExactKeys(value, ['groupId', 'dispositionNodeId', 'classification', 'rationale', 'author'], whyInvalid);
...
function indexGroupNodes(nodes) {
    const records = new Map();
...
function indexDispositionNodes(nodes) {
    const records = new Map();
    for (const node of nodes) {
        let output;
        try {
...
function validateHitAgainstSnapshot(hit, snapshot, entries) {
    ...
    if (canonicalJson(sourceObject) !== canonicalJson(hit.output.sourceObject)) {
        throw whyStale('Investigation hit source object no longer matches the selected tree.');
    }
    const extent = hit.output.surface === 'path'
...
function validateGroupCoverage(hits, groups) {
    ...
    for (const { node, output } of groups.values()) {
        ...
        if (canonicalJson(node.semanticParentResultDigests) !==
    ...
    if (covered.size !== hits.size) {
        throw whyInvalid('Every current investigation hit requires exactly one group.');
    }
}
function validateDispositionCoverage(hits, groups, dispositions) {
    const covered = new Set();
    const dispositionedGroups = new Set();
    for (const { node, output } of dispositions.values()) {
        ...
        if (node.provenanceParentNodeIds.group !== group.node.nodeId ||
        ...
        if (canonicalJson(output.sourceObjects) !==
            canonicalJson(dedupeHitSourceObjects(dispositionHits))) {
            throw whyInvalid('Disposition source objects do not match its covered hits.');
        }
    }
    if (covered.size !== hits.size) {
        throw whyInvalid('Every current investigation hit requires exactly one disposition.');
    }
    if (dispositionedGroups.size !== groups.size) {
        throw whyInvalid('Every current investigation group requires exactly one disposition.');
    }
}
function dedupeHitSourceObjects(hits) {
    ...
    return [...byObjectId.values()].sort((left, right) => compareString(left.objectId, right.objectId));
}
function assertManifestEntries(value) {
...
function assertManifestEntry(value) {
    const record = assertExactKeys(value, MANIFEST_ENTRY_KEYS, whyInvalid);
    const manifestEntryId = assertDigest(record.manifestEntryId, 'Full-blob manifest row identity is malformed.');
    const path = assertPathIdentity(record.path);
    const treeDigest = assertDigest(record.treeDigest, 'Full-blob manifest tree digest is malformed.');
    ...
    const dispositionNodeIds = assertSortedUniqueDigests(record.dispositionNodeIds);
    const relevantLocations = assertArray(record.relevantLocations).map(assertRelevantLocation);
    const relationshipsToChange = assertArray(record.relationshipsToChange).map(assertRelationshipToChange);
    assertWhyRowRelationships({
        path,
        blob,
        coveredHitIds,
    ...
    if (manifestEntryDigest(row) !== manifestEntryId) {
        throw whyInvalid('Full-blob manifest row identity does not match its bytes.');
    }
    return { manifestEntryId, ...row };
}
function assertManifestBlob(value) {
    ...
    if (content.toString('base64') !== record.contentBase64 ||
...
function validateManifestEvidenceBindings(manifest, hits, groups, dispositions) {
    ...
    if (expectedByPath.size !== manifest.length) {
        throw whyInvalid('Full-blob manifest does not cover every load-bearing path exactly once.');
    }
    for (const entry of manifest) {
        ...
        const expectedView = {
            ...
            relevantLocations: expectedHits
                ...
                .sort((left, right) => compareString(left.hitId, right.hitId)),
            relationshipsToChange: [...expected.relationships.values()].sort((left, right) => compareString(left.dispositionNodeId, right.dispositionNodeId)),
        };
        if (canonicalJson({
            coveredHitIds: entry.coveredHitIds,
            matchedTermIds: entry.matchedTermIds,
            ...
            throw whyInvalid('Full-blob manifest row does not match its evidence parents.');
        }
        for (const hit of expectedHits) {
            if (hit.sourceObject.objectId !== entry.blob.objectId ||
                ...
                throw whyInvalid('Full-blob manifest blob or tree differs from its hit evidence.');
            }
        }
    }
}
function assertWhyRowRelationships(input) {
    ...
    if (canonicalJson(input.relevantLocations.map(({ hitId }) => hitId)) !==
        canonicalJson(input.coveredHitIds) ||
        canonicalJson(sortedUnique(input.relevantLocations.map(({ termId }) => termId))) !== canonicalJson(input.matchedTermIds)) {
        throw whyInvalid('WHY hit or term identities do not match its locations.');
    }
    let priorDisposition = null;
    for (const relationship of input.relationshipsToChange) {
        if (priorDisposition !== null &&
    ...
    if (canonicalJson(input.relationshipsToChange.map(({ dispositionNodeId }) => dispositionNodeId)) !== canonicalJson(input.dispositionNodeIds) ||
        canonicalJson(sortedUnique(input.relationshipsToChange.map(({ groupId }) => groupId))) !== canonicalJson(input.groupIds)) {
        throw whyInvalid('WHY group or disposition identities do not match its relationships.');
    }
}
function assertCurrentWhyParents(node, why, hits, dispositions) {
    const semantic = {};
    const provenance = parentProvenance(why.coveredHitIds, why.dispositionNodeIds);
    ...
    why.dispositionNodeIds.forEach((nodeId, index) => {
        ...
        if (!disposition) {
            throw whyInvalid('WHY row references an unavailable current disposition.');
        }
        semantic[`disposition-${index}`] = disposition.node.resultDigest;
    });
    if (canonicalJson(node.provenanceParentNodeIds) !== canonicalJson(provenance) ||
...
function parentProvenance(hitIds, dispositionNodeIds) {
    const provenance = {};
...
function manifestEntryDigest(entry) {
    return sha256(canonicalJson({ schema: MANIFEST_SCHEMA, ...entry }));
}
function whyRowView(why) {
    return {
...
function manifestRowView(entry) {
    return {
        manifestEntryId: entry.manifestEntryId,
        path: entry.path,
        treeDigest: entry.treeDigest,
...
function assertPathIdentity(value) {
...
function assertSortedUniqueDigests(value) {
...
function assertExactKeys(value, keys, invalid) {
