...
export function createEvidenceNode(input) {
    ...
    try {
        ...
        snapshot = {
            ...
            semanticParentResultDigests: structuredClone(input.semanticParentResultDigests),
...
export function canonicalEvidenceNodeEnvelope(node) {
 ...
 */
export function assertStoredEvidenceNode(value, invalid) {
    if (!isPlainRecord(value) || !hasExactKeys(value, EVIDENCE_NODE_KEYS)) {
        throw invalid();
    }
    if (!isLabel(value.nodeSchema) ||
    ...
    if (digests.nodeId !== node.nodeId ||
        digests.resultDigest !== node.resultDigest) {
        throw invalid();
    }
    return node;
}
function computeEvidenceDigests(node) {
    const nodeId = sha256(canonicalJson({
        type: node.type,
...
function isDigestMap(value) {
    return (isPlainRecord(value) &&
        Object.entries(value).every(([role, digest]) => DIGEST_ROLE_PATTERN.test(role) && isDigest(digest)));
}
...
function nodeInvalid() {
    return workflowError('EVIDENCE_NODE_INVALID', 'Evidence node input is malformed.', ExitCode.usage);
}
