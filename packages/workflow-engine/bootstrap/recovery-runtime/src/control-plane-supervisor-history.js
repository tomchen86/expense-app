...
export function controlPlaneSupervisorHistoryRecordDigest(payload) {
...
export function createControlPlaneSupervisorHistoryTransition(input) {
    ...
    assertTransitionFollows(previous, record);
    return record;
}
export function createControlPlaneSupervisorHistoryTerminal(input) {
    ...
    if (previous.kind !== 'control-plane-supervisor-history-transition.v1') {
        throw transitionInvalid('A terminal seal must follow a supervisor transition.');
    }
    const payload = {
    ...
    assertTerminalFollows(previous, record);
    return record;
}
export function verifyControlPlaneSupervisorHistoryRecord(value) {
...
export function verifyControlPlaneSupervisorHistoryProgress(values) {
    ...
    for (const record of records) {
        ...
        if (record.previousRecordDigest !== null) {
            children.set(record.previousRecordDigest, (children.get(record.previousRecordDigest) ?? 0) + 1);
        }
    }
    ...
    for (let index = 1; index < ordered.length; index += 1) {
        ...
        if (current.previousRecordDigest !== previous.recordDigest) {
            throw historyGap('Supervisor history previous digests are discontinuous.');
        }
        if (current.repositoryId !== anchor.repositoryId) {
            throw transitionInvalid('Supervisor history crosses repository identities.');
        }
        if (Date.parse(current.recordedAt) < Date.parse(previous.recordedAt)) {
            throw transitionInvalid('Supervisor history timestamps are not monotonic.');
        }
        if (current.kind === 'control-plane-supervisor-history-transition.v1') {
            ...
            if (current.phase === 'rollback-restored') {
                ...
                if (restored === undefined ||
                    ...
                    throw transitionInvalid('Rollback does not restore the exact pre-candidate selection.');
                }
            }
        }
        else if (current.kind === 'control-plane-supervisor-history-terminal.v1') {
    ...
    const progress = {
        ...
        terminal: leaf.kind === 'control-plane-supervisor-history-anchor.v1' ||
            leaf.kind === 'control-plane-supervisor-history-terminal.v1',
    };
    return deepFreeze(progress);
}
export function verifyControlPlaneSupervisorHistory(values) {
    const progress = verifyControlPlaneSupervisorHistoryProgress(values);
    if (!progress.terminal) {
        throw workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_NOT_TERMINAL', 'Supervisor history leaf is an incomplete transition.', ExitCode.conflict);
    }
    return progress;
}
export function controlPlaneSupervisorHistoryDirectory(storageRoot) {
    return path.join(storageRoot, HISTORY_DIRECTORY_NAME);
}
export function controlPlaneSupervisorHistoryRecordPath(storageRoot, recordDigest) {
    assertDigest(recordDigest);
    return path.join(controlPlaneSupervisorHistoryDirectory(storageRoot), `${recordDigest.slice('sha256:'.length)}.json`);
}
export function controlPlaneSupervisorHistoryPendingPath(storageRoot, recordDigest) {
    assertDigest(recordDigest);
    return path.join(controlPlaneSupervisorHistoryDirectory(storageRoot), `.${recordDigest.slice('sha256:'.length)}.json.pending`);
}
export function readControlPlaneSupervisorHistoryProgress(storageRoot) {
    const root = assertStorageRoot(storageRoot);
    const directory = controlPlaneSupervisorHistoryDirectory(root);
    assertPrivateDirectory(directory);
    return verifyControlPlaneSupervisorHistoryProgress(readHistoryRecords(directory));
}
export function readControlPlaneSupervisorHistory(storageRoot) {
...
export function appendControlPlaneSupervisorHistoryRecord(storageRoot, recordValue, hooks = {}) {
    const record = verifyControlPlaneSupervisorHistoryRecord(recordValue);
    const root = assertStorageRoot(storageRoot);
    const directory = ensureHistoryDirectory(root);
    const target = controlPlaneSupervisorHistoryRecordPath(root, record.recordDigest);
    const pending = controlPlaneSupervisorHistoryPendingPath(root, record.recordDigest);
    ...
    if (targetBefore === undefined && pendingBefore === undefined) {
        ...
        const existing = readHistoryRecords(directory, path.basename(pending), path.basename(target));
        verifyAppendRelation(existing, record);
    }
    reconcilePreparedRecord(directory, pending, target, content, hooks);
    const history = verifyControlPlaneSupervisorHistoryProgress(readHistoryRecords(directory));
    if (!history.records.some(({ recordDigest }) => recordDigest === record.recordDigest)) {
        throw storageUnsafe('Published history record is absent after reconciliation.');
    }
    return deepFreeze({ replayed, history });
}
function verifyAppendRelation(existing, record) {
    if (existing.length === 0) {
        if (record.kind !== 'control-plane-supervisor-history-anchor.v1') {
            throw historyGap('The first supervisor history record must be an anchor.');
        }
        verifyControlPlaneSupervisorHistoryProgress([record]);
        return;
    }
    const current = verifyControlPlaneSupervisorHistoryProgress(existing);
    if (record.sequence !== current.leaf.sequence + 1 ||
...
function reconcilePreparedRecord(directory, pending, target, content, hooks) {
...
function assertPreparedLinkPair(pending, target, content, pendingStats = fs.lstatSync(pending, { throwIfNoEntry: false }), targetStats = fs.lstatSync(target, { throwIfNoEntry: false })) {
    if (pendingStats === undefined ||
        ...
        targetStats.nlink !== 2) {
        throw storageUnsafe('Prepared and published history paths are not one exact hard-link pair.');
    }
    assertPrivateRecordBytes(pending, content, 2);
    assertPrivateRecordBytes(target, content, 2);
}
function createPreparedRecord(filePath, content) {
    let descriptor;
    try {
...
function readHistoryRecords(directory, allowedPendingName, allowedLinkedTargetName) {
    const records = [];
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        if (entry.name === allowedPendingName ||
        ...
        if (!RECORD_FILE.test(entry.name) ||
            ...
            throw workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_UNKNOWN_RESIDUE', 'Supervisor history directory contains an unknown or indirect entry.', ExitCode.unsafeEnvironment);
        }
        ...
        if (`${record.recordDigest.slice('sha256:'.length)}.json` !== entry.name) {
            throw recordInvalid('Supervisor history filename does not match its digest.');
        }
        records.push(record);
    }
    return records;
}
function assertAppendInventory(directory, allowedPendingName) {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        if (entry.name === allowedPendingName)
            continue;
        if (!RECORD_FILE.test(entry.name) ||
            !entry.isFile() ||
            entry.isSymbolicLink()) {
            throw workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_UNKNOWN_RESIDUE', 'Supervisor history directory contains residue unrelated to this append.', ExitCode.unsafeEnvironment);
        }
    }
}
function verifyAnchor(value) {
...
function verifyTransition(value) {
    if (!hasExactKeys(value, [
        'activeArtifact',
        'activeTrustCommit',
        'fromGeneration',
...
function verifyArtifactSelection(value) {
    if (!isRecord(value) ||
...
function verifyAnchorAuthority(value) {
    if (!isRecord(value)) {
    ...
    if (value.kind !== 'legacy-v2-terminal-anchor.v1' ||
        !hasExactKeys(value, [
            'grantEnvelopeDigest',
            'grantId',
...
function assertTransitionFollows(previous, current) {
    if (current.previousRecordDigest !== previous.recordDigest ||
        ...
        throw transitionInvalid('Supervisor transition does not extend its exact predecessor.');
    }
    if (current.phase === 'candidate-selected') {
        if ((previous.kind !== 'control-plane-supervisor-history-anchor.v1' &&
            previous.kind !== 'control-plane-supervisor-history-terminal.v1') ||
            current.sourceTransactionState !== 'RECOVERY_VERIFIED') {
            throw transitionInvalid('Candidate selection must extend a terminal history leaf.');
        }
        return;
    }
    if (previous.kind !== 'control-plane-supervisor-history-transition.v1' ||
        ...
        throw transitionInvalid('Rollback must extend the exact candidate selection transaction.');
    }
}
function assertTerminalFollows(previous, current) {
    if (previous.kind !== 'control-plane-supervisor-history-transition.v1' ||
        ...
        throw transitionInvalid('Terminal seal does not match its exact supervisor transition.');
    }
}
...
function recordSupervisorDigest(record) {
    return record.kind === 'control-plane-supervisor-history-transition.v1'
        ? record.toSupervisorRecordDigest
        : record.supervisorRecordDigest;
}
function recordActiveArtifact(record) {
    return record.activeArtifact;
}
function recordActiveTrustCommit(record) {
    return record.activeTrustCommit;
}
function verifySelfDigest(value) {
    ...
    if (recordDigest !== controlPlaneSupervisorHistoryRecordDigest(payload)) {
        throw recordInvalid('Supervisor history self-digest does not match its canonical bytes.');
    }
}
function assertStorageRoot(storageRoot) {
    const stats = typeof storageRoot === 'string'
    ...
    if (typeof storageRoot !== 'string' ||
        ...
        throw storageUnsafe('Supervisor history storage root is not an exact private directory.');
    }
    return storageRoot;
}
function ensureHistoryDirectory(storageRoot) {
    ...
    if (existing === undefined) {
        try {
            ...
            throw storageUnsafe('Supervisor history directory could not be created safely.');
        }
    }
    assertPrivateDirectory(directory);
    return directory;
...
function assertPrivateDirectory(directory) {
    ...
    if (stats === undefined ||
        ...
        throw storageUnsafe('Supervisor history directory is missing, indirect, or not private.');
    }
}
function assertPrivateRecordBytes(filePath, expected, expectedLinks) {
    const observed = readPrivateRecordText(filePath, expectedLinks);
    if (observed !== expected) {
        throw workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_CONFLICT', 'Existing supervisor history path contains different canonical bytes.', ExitCode.conflict);
    }
}
function readPrivateRecordText(filePath, expectedLinks) {
    ...
    if (stats === undefined ||
        ...
        throw storageUnsafe('Supervisor history record is indirect, linked, oversized, or not private.');
    }
    ...
    try {
        ...
        if (opened.dev !== stats.dev ||
            ...
            throw storageUnsafe('Supervisor history record changed while it was read.');
        }
        return raw;
    }
    finally {
...
function assertDigest(value) {
    if (typeof value !== 'string' || !DIGEST.test(value)) {
        throw recordInvalid('Supervisor history expected a canonical sha256 digest.');
    }
}
function assertGitOid(value) {
    if (typeof value !== 'string' || !GIT_OID.test(value)) {
        throw recordInvalid('Supervisor history trust commit is not a full Git object id.');
    }
}
function assertNonEmpty(value) {
    if (typeof value !== 'string' ||
        value.length === 0 ||
        value.trim() !== value) {
        throw recordInvalid('Supervisor history identifier is not a non-empty trimmed string.');
    }
}
function assertIso(value) {
    if (typeof value !== 'string' ||
        ...
        throw recordInvalid('Supervisor history timestamp is not canonical ISO-8601.');
    }
}
...
function recordInvalid(message) {
    return workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_RECORD_INVALID', message, ExitCode.verification);
}
function transitionInvalid(message) {
    return workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_TRANSITION_INVALID', message, ExitCode.verification);
}
function historyGap(message) {
    return workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_GAP', message, ExitCode.verification);
}
function historyFork(message) {
    return workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_FORK', message, ExitCode.verification);
}
function storageUnsafe(message) {
    return workflowError('CONTROL_PLANE_SUPERVISOR_HISTORY_STORAGE_UNSAFE', message, ExitCode.unsafeEnvironment);
}
