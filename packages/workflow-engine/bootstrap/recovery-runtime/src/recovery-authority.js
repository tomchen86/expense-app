...
const PREPARATION_FILE = /^descriptor\.prepare\.([0-9a-f]{64})\.[1-9][0-9]*\.[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.json$/;
...
const PAYLOAD_KEYS = DESCRIPTOR_KEYS.filter((key) => key !== 'descriptorDigest');
const REPOSITORY_KEYS = ['gitObjectFormat', 'origin', 'repositoryId'];
...
const AUDIT_KEYS = ['ledgerId', 'rootBindingDigest'];
...
const REPOSITORY_ORIGIN = /^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\.git$/;
...
export function recoveryAuthorityRepositoryIdentityDigest(repositoryIdentity) {
 ...
 */
export function verifyRecoveryAuthorityDescriptor(value, expected) {
    ...
    if (canonicalJson(descriptor.repositoryIdentity) !==
        ...
        throw authorityError('RECOVERY_AUTHORITY_REPOSITORY_MISMATCH', 'Recovery authority belongs to a different out-of-band repository identity.', ExitCode.staleState);
    }
    if (descriptor.generation !== pinned.generation) {
        throw authorityError('RECOVERY_AUTHORITY_GENERATION_MISMATCH', 'Recovery authority generation differs from the out-of-band expectation.', ExitCode.staleState);
    }
    if (descriptor.signer.fingerprint !== pinned.signerFingerprint) {
        throw authorityError('RECOVERY_AUTHORITY_SIGNER_FINGERPRINT_MISMATCH', 'Recovery authority signer fingerprint differs from the out-of-band expectation.', ExitCode.verification);
    }
    if (canonicalJson(descriptor.sealedRuntime) !==
        canonicalJson(pinned.sealedRuntime)) {
        throw authorityError('RECOVERY_AUTHORITY_RUNTIME_MISMATCH', 'Recovery authority sealed runtime binding differs from the out-of-band expectation.', ExitCode.verification);
    }
    if (canonicalJson(descriptor.auditLedger) !== canonicalJson(pinned.auditLedger)) {
        throw authorityError('RECOVERY_AUTHORITY_AUDIT_BINDING_MISMATCH', 'Recovery authority audit binding differs from the out-of-band expectation.', ExitCode.verification);
    }
    if (descriptor.descriptorDigest !== pinned.descriptorDigest) {
        throw authorityError('RECOVERY_AUTHORITY_DESCRIPTOR_DIGEST_MISMATCH', 'Recovery authority descriptor digest differs from the out-of-band expectation.', ExitCode.verification);
    }
    return descriptor;
}
/**
 ...
 */
export function assertRecoveryAuthorityDomain(descriptor, expected, namespace, signerFingerprint) {
    const exact = verifyRecoveryAuthorityDescriptor(descriptor, expected);
    if (exact.signer.fingerprint !== signerFingerprint) {
        throw authorityError('RECOVERY_AUTHORITY_SIGNER_FINGERPRINT_MISMATCH', 'Recovery signature signer differs from the pinned recovery authority.', ExitCode.verification);
    }
    if (typeof namespace !== 'string' ||
        !RECOVERY_AUTHORITY_KNOWN_DOMAINS.includes(namespace) ||
        !exact.allowedDomains.includes(namespace)) {
        throw authorityError('RECOVERY_AUTHORITY_DOMAIN_FORBIDDEN', 'Recovery authority cannot be replayed into this signature domain.', ExitCode.guard);
    }
}
/**
 ...
 */
export function importRecoveryAuthorityDescriptor(externalDescriptorPath, privateStoreRoot, expected, boundary, hooks) {
    ...
    const external = verifyRecoveryAuthorityDescriptor(readExternalDescriptor(externalDescriptorPath), expected);
...
export function readRecoveryAuthorityDescriptor(privateStoreRoot, expected, boundary) {
    assertPrivateStoreBoundary(privateStoreRoot, boundary);
    const directory = recoveryAuthorityStoreDirectory(privateStoreRoot, false);
    const target = path.join(directory, 'descriptor.json');
    if (fs.lstatSync(target, { throwIfNoEntry: false }) === undefined) {
        throw authorityError('RECOVERY_AUTHORITY_NOT_PROVISIONED', 'No external Recovery Authority Descriptor has been provisioned.', ExitCode.unsafeEnvironment);
    }
    return verifyRecoveryAuthorityDescriptor(readStoredDescriptor(target), expected);
}
function ensureDurablePreparation(directory, external, hooks) {
    ...
    const preparation = reusable ?? writePrivatePreparation(directory, external, hooks);
...
function reconcilePreparedDescriptor(directory, prepared, external) {
    ...
    const aliases = listPreparationPaths(directory).filter((candidate) => {
        ...
        return (candidateStats !== undefined && sameInode(preparedStats, candidateStats));
    });
    if (aliases.length !== 1)
        throw storeCorrupt();
...
function reconcilePublishedDescriptor(directory, target, external, expected) {
    const targetStats = fs.lstatSync(target, { throwIfNoEntry: false });
...
function selectExactPreparation(directory, external) {
    let selected;
...
function inspectPreparation(filePath) {
    ...
    try {
        return parseRecoveryAuthorityDescriptor(readPrivateCanonicalFile(filePath, {
        ...
        if (hasWorkflowCode(error, 'RECOVERY_AUTHORITY_PREPARATION_INCOMPLETE') ||
            hasWorkflowCode(error, 'RECOVERY_AUTHORITY_DESCRIPTOR_INVALID')) {
            return null;
        }
        if (hasWorkflowCode(error, 'RECOVERY_AUTHORITY_STORE_CORRUPT'))
            throw error;
        throw storeCorrupt();
    }
}
function listPreparationPaths(directory) {
    return fs
...
function writePrivatePreparation(directory, external, hooks) {
    const filePath = path.join(directory, `descriptor.prepare.${external.descriptorDigest.slice('sha256:'.length)}.${process.pid}.${crypto.randomUUID()}.json`);
    ...
    try {
        ...
        if (invokingHook || hasWorkflowCode(error))
            throw error;
        throw storeUnsafe();
    }
    finally {
...
function readPreparedDescriptor(filePath, allowedLinkCounts) {
    try {
        return parseRecoveryAuthorityDescriptor(readPrivateCanonicalFile(filePath, {
            missingCode: 'RECOVERY_AUTHORITY_STORE_CORRUPT',
...
function assertSameDescriptor(observed, expected) {
...
function unlinkExactAlias(alias, anchor) {
    ...
    if (!anchorStats || anchorStats.nlink !== 1)
        throw storeCorrupt();
}
function sameInode(left, right) {
    return left.dev === right.dev && left.ino === right.ino;
}
function assertPreparationNameMatches(filePath, external) {
    const match = PREPARATION_FILE.exec(path.basename(filePath));
    if (`sha256:${match?.[1] ?? ''}` !== external.descriptorDigest) {
        throw replacementForbidden();
    }
}
function assertImportHooks(hooks) {
    if (hooks !== undefined &&
        (!isRecord(hooks) ||
            !hasExactKeys(hooks, ['afterPhase']) ||
            typeof hooks.afterPhase !== 'function')) {
        throw authorityError('RECOVERY_AUTHORITY_IMPORT_HOOK_INVALID', 'Recovery Authority import hook is malformed.', ExitCode.usage);
    }
}
function normalizePayload(value) {
    ...
    const repositoryIdentity = normalizeRepositoryIdentity(value.repositoryIdentity);
    if (value.repositoryIdentityDigest !==
        recoveryAuthorityRepositoryIdentityDigest(repositoryIdentity)) {
        throw descriptorInvalid();
    }
    return {
...
function normalizeSealedRuntime(value) {
    if (!isRecord(value) ||
...
function normalizeAuditLedger(value) {
    if (!isRecord(value) ||
...
function isAllowedDomainSubset(value) {
    const known = RECOVERY_AUTHORITY_KNOWN_DOMAINS;
    if (value.length === 0 ||
        value.some((domain) => typeof domain !== 'string' || !known.includes(domain))) {
        return false;
    }
    const sortedUnique = [...new Set(value)].sort();
    return (value.length === sortedUnique.length &&
...
function sshPublicKeyFingerprint(publicKey) {
    ...
    if (!algorithm || !encoded || extra.length !== 0)
        throw descriptorInvalid();
    let blob;
    try {
...
function readExternalDescriptor(filePath) {
    return readPrivateCanonicalFile(filePath, {
        ...
        unsafeMessage: 'External Recovery Authority Descriptor must be one exact private regular file.',
...
function assertExternalDescriptorSource(filePath, boundary) {
    if (!isRecord(boundary) || !hasExactKeys(boundary, IMPORT_BOUNDARY_KEYS)) {
        throw authorityError('RECOVERY_AUTHORITY_IMPORT_BOUNDARY_INVALID', 'Recovery authority import requires exact repository source boundaries.', ExitCode.unsafeEnvironment);
    }
    const roots = [
    ...
    if (typeof filePath !== 'string' ||
        ...
        throw authorityError('RECOVERY_AUTHORITY_DESCRIPTOR_SOURCE_FORBIDDEN', 'Recovery Authority Descriptor must come from outside the repository worktree and Git common directory.', ExitCode.unsafeEnvironment);
    }
}
function assertPrivateStoreBoundary(privateStoreRoot, boundary) {
    ...
    if (relative.length === 0 ||
        ...
        throw authorityError('RECOVERY_AUTHORITY_STORE_BOUNDARY_MISMATCH', 'Recovery Authority private store must be an exact private descendant of the Git common directory.', ExitCode.unsafeEnvironment);
    }
}
function exactDirectoryPath(value) {
    if (typeof value !== 'string' ||
        ...
        throw authorityError('RECOVERY_AUTHORITY_IMPORT_BOUNDARY_INVALID', 'Recovery authority import boundary must be an exact absolute directory.', ExitCode.unsafeEnvironment);
    }
    const stats = fs.lstatSync(value, { throwIfNoEntry: false });
    if (!stats?.isDirectory() ||
        stats.isSymbolicLink() ||
        safeRealpath(value) !== value) {
        throw authorityError('RECOVERY_AUTHORITY_IMPORT_BOUNDARY_INVALID', 'Recovery authority import boundary must be an exact real directory.', ExitCode.unsafeEnvironment);
    }
    return value;
}
...
function readStoredDescriptor(filePath) {
    try {
        return parseRecoveryAuthorityDescriptor(readPrivateCanonicalFile(filePath, {
            missingCode: 'RECOVERY_AUTHORITY_STORE_CORRUPT',
...
function readPrivateCanonicalFile(filePath, errors, allowedLinkCounts = [1]) {
    if (typeof filePath !== 'string' ||
        !path.isAbsolute(filePath) ||
        path.resolve(filePath) !== filePath) {
        throw authorityError(errors.unsafeCode, errors.unsafeMessage, ExitCode.unsafeEnvironment);
    }
    const stats = fs.lstatSync(filePath, { throwIfNoEntry: false });
    if (stats === undefined) {
        throw authorityError(errors.missingCode, errors.missingMessage, ExitCode.unsafeEnvironment);
    }
    if (stats.size > MAX_RECOVERY_AUTHORITY_DESCRIPTOR_BYTES) {
        throw authorityError(errors.oversizedCode, errors.oversizedMessage, ExitCode.unsafeEnvironment);
    }
    if (!stats.isFile() ||
        stats.isSymbolicLink() ||
...
function recoveryAuthorityStoreDirectory(privateStoreRoot, create) {
    assertPrivateDirectory(privateStoreRoot, 'RECOVERY_AUTHORITY_STORE_UNSAFE');
...
function assertPrivateDirectory(directory, code) {
    if (typeof directory !== 'string' ||
        ...
        throw authorityError(code, 'Recovery Authority private store must be one exact absolute directory.', ExitCode.unsafeEnvironment);
    }
    ...
    if (!stats?.isDirectory() ||
        ...
        throw authorityError(code, 'Recovery Authority private store must be one exact private directory.', ExitCode.unsafeEnvironment);
    }
}
...
function fsyncPrivateFile(filePath, allowedLinkCounts) {
...
function descriptorInvalid() {
    return authorityError('RECOVERY_AUTHORITY_DESCRIPTOR_INVALID', 'Recovery Authority Descriptor does not match its exact schema or intrinsic bindings.', ExitCode.verification);
}
function expectationInvalid() {
    return authorityError('RECOVERY_AUTHORITY_EXPECTATION_INVALID', 'Recovery authority requires complete exact out-of-band expectations.', ExitCode.unsafeEnvironment);
}
function replacementForbidden() {
    return authorityError('RECOVERY_AUTHORITY_REPLACEMENT_FORBIDDEN', 'An imported Recovery Authority cannot authorize or overwrite its own replacement.', ExitCode.guard);
}
function preparationIncomplete() {
    return authorityError('RECOVERY_AUTHORITY_PREPARATION_INCOMPLETE', 'Recovery Authority preparation is incomplete.', ExitCode.unsafeEnvironment);
}
function storeUnsafe() {
    return authorityError('RECOVERY_AUTHORITY_STORE_UNSAFE', 'Recovery Authority private store is unavailable or unsafe.', ExitCode.unsafeEnvironment);
}
function storeCorrupt() {
    return authorityError('RECOVERY_AUTHORITY_STORE_CORRUPT', 'Recovery Authority private store failed integrity verification.', ExitCode.verification);
}
function authorityError(code, message, exitCode) {
