...
export function ensurePlainDirectory(directory) {
...
export function preparedLockTemporaryPath(lockPath, pid, ownerToken) {
...
export function publishPreparedExclusiveLock(lockPath, content, ownerToken, makeUnsafeError = () => new Error('Prepared lock publication is unsafe.')) {
    const temporaryPath = preparedLockTemporaryPath(lockPath, process.pid, ownerToken);
    let descriptor;
    let published = false;
    try {
...
export function reclaimDeadPreparedLock(lockPath, readOwner) {
    ...
    try {
        ...
        const temporaryPath = opened.nlink === 2
...
export function withPreparedLockCleanupClaim(lockPath, operation) {
...
function createPreparedLockReclaimClaim(lockPath) {
    const claimPath = preparedLockReclaimClaimPath(lockPath, process.pid, crypto.randomUUID());
    fs.mkdirSync(claimPath, { mode: 0o700 });
    fs.chmodSync(claimPath, 0o700);
    fsyncDirectory(path.dirname(lockPath));
    return claimPath;
}
function removePreparedLockReclaimClaim(claimPath) {
...
function clearDeadPreparedLockReclaimClaims(lockPath) {
    ...
    for (const name of fs.readdirSync(directory)) {
        ...
        const match = /^(.+)\.reclaim\.([1-9][0-9]*)\.([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/.exec(name);
        if (!match ||
...
function unsafeDirectory(directory) {
    return workflowError('RUNTIME_DIRECTORY_UNSAFE', 'Workflow runtime directory is not a canonical plain directory.', ExitCode.staleState, { details: { directory } });
}
