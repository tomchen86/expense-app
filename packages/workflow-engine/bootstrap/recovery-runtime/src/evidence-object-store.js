...
import { assertStoredEvidenceNode, canonicalEvidenceNodeEnvelope, } from './evidence-node.js';
...
import { TASK_STRATEGY_IMPLEMENTATION_POLICY_DIGEST, assertTaskStrategyImplementationSubject, } from './task-strategy-provider-contract.js';
...
const EVIDENCE_PUBLICATION_TEMP_SUFFIX = /^([1-9][0-9]*)\.([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.publish\.tmp$/;
export function exactUnsafePathObservationDigest(filePath, object) {
...
export function writeEvidenceNode(paths, node) {
    ...
    ensureNoFollowDirectory(paths.base, paths.root, objectDirectory, objectUnsafe);
    ...
    try {
        ...
        assertExactEvidencePublicationAlias(publishAlias, publishAliasStats, contentBytes, 1);
        ...
        if (publishAliasOwnedStats !== undefined) {
            unlinkOwnedEvidencePublicationAlias(publishAlias, publishAliasOwnedStats, objectDirectory);
        }
        throw error;
    }
    return node.nodeId;
}
export function readEvidenceNode(paths, nodeId) {
    assertNodeId(nodeId);
    const objectPath = evidenceObjectPath(paths, nodeId);
    assertNoFollowDirectory(paths.base, paths.root, path.dirname(objectPath), objectUnsafe);
...
export function resolvePlanReviewInvocationOwner(paths, input) {
    ...
    const materializationId = authorization.provenanceParentNodeIds.materialization;
    if (authorization.type !== 'plan-review-authorization' ||
        ...
        !DIGEST_PATTERN.test(authorization.semanticParentResultDigests.materialization ?? '')) {
        throw refInvalid();
    }
    const materialization = readEvidenceNode(paths, materializationId);
    const ownership = assertPlanningMaterializationOwnership(paths, materialization, changeId);
    if (materialization.resultDigest !==
...
export function resolveTaskStrategyImplementationInvocationOwner(paths, input) {
    const changeId = assertChangeId(input.changeId);
    ...
    const authorization = readEvidenceNode(paths, input.authorizationNodeId);
    const output = authorization.output;
    if (authorization.type !== 'task-strategy-implementation-authorization' ||
        ...
        !isTaskStrategyImplementationAssignmentForAuthor(output.assignment, output.redAuthor, subject.subjectDigest) ||
        !isTaskDiffReviewMandateBinding(output.mandateBinding, changeId) ||
    ...
    if (red.type !== 'task-strategy-red-evidence' ||
...
export function readEvidenceRefsSnapshot(paths, requestedChangeId) {
...
export function readInvestigationEvidenceRefsClosure(paths, requestedChangeId) {
    const changeId = assertChangeId(requestedChangeId);
    ...
    const closure = computeInvestigationEvidenceRefsClosure(paths, changeId, snapshot.refs);
    return Object.freeze({
        snapshot,
        ...closure,
    });
}
export function observeInvestigationEvidenceRefsAmbiguities(paths, requestedChangeId) {
    const changeId = assertChangeId(requestedChangeId);
    const observations = new Map();
    const observe = (object, filePath) => {
        observations.set(object, exactUnsafePathObservationDigest(filePath, object));
    };
    const refPath = evidenceRefPath(paths, changeId);
...
function sortedAmbiguityObservations(observations) {
    return [...observations]
        .sort(([left], [right]) => left.localeCompare(right))
        .map(([object, observationDigest]) => ({ object, observationDigest }));
}
export function computeInvestigationEvidenceRefsClosure(paths, requestedChangeId, refs) {
    const changeId = assertChangeId(requestedChangeId);
    const owners = {};
    const entries = Object.entries(refs)
        ...
        .map(([refName, nodeId]) => {
        const node = readEvidenceNode(paths, nodeId);
        const ownership = assertInvestigationEvidenceRefOwner(paths, refs, refName, node, changeId);
...
export function investigationEvidenceRefsClosureDigest(requestedChangeId, entries) {
    const changeId = assertChangeId(requestedChangeId);
    const sorted = [...entries].sort((left, right) => left.refName.localeCompare(right.refName));
    if (sorted.some((entry, index) => entry.refName !== entries[index]?.refName ||
        ...
        entry.dependencies.some((dependency, dependencyIndex) => dependency.role.length === 0 ||
            dependency.role !==
                [...entry.dependencies].sort((left, right) => left.role.localeCompare(right.role))[dependencyIndex]?.role ||
...
export function quarantineUnsafeEvidenceRefsDocument(paths, requestedChangeId, expectedObservationDigest) {
    const changeId = assertChangeId(requestedChangeId);
    if (!DIGEST_PATTERN.test(expectedObservationDigest)) {
        throw refInvalid();
    }
    const source = evidenceRefPath(paths, changeId);
    const quarantineDirectory = path.join(paths.root, 'human-resolutions', 'quarantine');
    const target = path.join(quarantineDirectory, `${changeId}.${expectedObservationDigest}.evidence-refs.artifact`);
    return withRefLock(paths, changeId, () => {
        ensureNoFollowDirectory(paths.base, paths.root, quarantineDirectory, refUnsafe);
        ...
        if (exactUnsafePathObservationDigest(source, 'evidence-refs') !==
            expectedObservationDigest) {
            throw refsDocumentCasMismatch(expectedObservationDigest, exactUnsafePathObservationDigest(source, 'evidence-refs'), null);
        }
        if (fs.lstatSync(target, { throwIfNoEntry: false })) {
            throw refInvalid();
        }
        ...
        if (exactUnsafePathObservationDigest(target, 'evidence-refs') !==
            expectedObservationDigest) {
            throw refInvalid();
        }
        return target;
...
export function compareAndSwapEvidenceRefsDocument(paths, params) {
    const changeId = assertChangeId(params.changeId);
    if (params.expectedDigest !== null &&
        !DIGEST_PATTERN.test(params.expectedDigest)) {
        throw refInvalid();
    }
    const nextRefs = params.nextRefs === null
        ? null
        : validateEvidenceRefsForPublication(paths, params.nextRefs);
    const nextDigest = nextRefs === null ? null : sha256(canonicalRefDocument(changeId, nextRefs));
    return withRefLock(paths, changeId, () => {
        ...
        if (current.digest !== params.expectedDigest) {
            throw refsDocumentCasMismatch(params.expectedDigest, current.digest, nextDigest);
        }
        if (nextRefs === null) {
...
export function compareAndSwapEvidenceRef(paths, params) {
    assertChangeId(params.changeId);
    assertRefName(params.refName);
    assertNodeId(params.nextNodeId);
    if (params.expectedNodeId !== null &&
        !DIGEST_PATTERN.test(params.expectedNodeId)) {
        throw refInvalid();
    }
    const nextObjectPath = evidenceObjectPath(paths, params.nextNodeId);
    const nextDirectoryExists = assertNoFollowDirectory(paths.base, paths.root, path.dirname(nextObjectPath), objectUnsafe);
...
function parseRefDocument(content, changeId) {
    let value;
    try {
...
function writeRefDocument(paths, changeId, refs) {
    const refPath = evidenceRefPath(paths, changeId);
...
function validateEvidenceRefsForPublication(paths, refs) {
...
function canonicalRefDocument(changeId, refs) {
    return canonicalJson({ schemaVersion: 1, changeId, refs });
}
function absentEvidenceRefsSnapshot() {
    return Object.freeze({
        rawDocument: null,
        digest: null,
        refs: null,
...
function withRefLock(paths, changeId, operation) {
    ensureNoFollowDirectory(paths.base, paths.root, paths.refs, refUnsafe);
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            descriptor = publishPreparedExclusiveLock(lockPath, marker, ownerToken, refLockInvalid);
            break;
        }
        catch (error) {
            if (descriptor !== undefined) {
...
function reclaimDeadRefLock(lockPath) {
    try {
...
function assertInvestigationEvidenceRefOwner(paths, refs, refName, node, expectedChangeId) {
    try {
        switch (refName) {
            case 'propose/planning-materialization':
                return assertPlanningMaterializationOwnership(paths, node, expectedChangeId);
            case 'propose/plan-review-request':
                return assertPlanReviewRequestOwnership(paths, refs, node, expectedChangeId);
            case 'propose/plan-review-grant-requirement':
                return assertPlanReviewGrantRequirementOwnership(node, expectedChangeId);
...
function assertPlanningMaterializationOwnership(paths, node, expectedChangeId) {
    const output = node.output;
    if (!isPlainRecord(output)) {
        throw refInvalid();
    }
    if (node.type === 'propose-planning-materialization') {
        const semanticReceipt = node.nodeSchema === 'workflow.propose-planning-materialization.v2' &&
            node.outputSchema ===
                'workflow.propose-planning-materialization-output.v2';
        const legacyReceipt = node.nodeSchema === 'workflow.propose-planning-materialization.v1' &&
            node.outputSchema ===
        ...
        if (!hasExactKeys(output, [
            ...
            !Number.isSafeInteger(semanticReceipt ? output.semanticRevision : output.revision) ||
    ...
    assertProposeEvidenceNode(node, {
        ...
        outputSchema: 'workflow.propose-exemption-planning-materialization-output.v1',
    ...
    const applicability = readApplicabilityDependency(paths, output.applicabilityNodeId, output.applicabilityResultDigest);
    return Object.freeze({
        ownerInvestigationId: output.investigationId,
        dependencies: Object.freeze([applicability]),
    });
}
function assertPlanReviewRequestOwnership(paths, refs, node, expectedChangeId) {
    const output = node.output;
    const retryDecisionShape = node.nodeSchema === 'workflow.plan-review-request-reservation.v3' &&
        node.outputSchema === 'workflow.plan-review-request-reservation-output.v3';
    const retryShape = retryDecisionShape ||
        (node.nodeSchema === 'workflow.plan-review-request-reservation.v2' &&
            node.outputSchema ===
                'workflow.plan-review-request-reservation-output.v2');
    const initialShape = node.nodeSchema === 'workflow.plan-review-request-reservation.v1' &&
    ...
    if (!hasExactKeys(output, outputKeys) ||
    ...
    if (!isPlainRecord(output.planning.subject) ||
    ...
    const authorization = readEvidenceNode(paths, node.provenanceParentNodeIds.authorization);
    const authorizationOutput = authorization.output;
    const authorizationCurrentShape = Object.hasOwn(authorization.exactInputDigests, 'targetSnapshot');
    if (authorization.type !== 'plan-review-authorization' ||
        authorization.nodeSchema !== 'workflow.plan-review-authorization.v1' ||
        authorization.evaluator !== 'workflow-propose.v1' ||
        authorization.policyDigest !== PROPOSE_POLICY_DIGEST ||
    ...
    const materializationId = authorization.provenanceParentNodeIds.materialization;
    const materialization = readEvidenceNode(paths, materializationId);
    const materializationOwnership = assertPlanningMaterializationOwnership(paths, materialization, expectedChangeId);
    if (materialization.resultDigest !==
        authorization.semanticParentResultDigests.materialization ||
    ...
    const dependencies = [
        ...
        ...materializationOwnership.dependencies.map((dependency) => Object.freeze({
            ...dependency,
    ...
    if (currentShape) {
        if (!isPlainRecord(output.materializationNode) ||
            !isPlainRecord(output.targetSnapshotNode)) {
            throw refInvalid();
        }
        const embeddedMaterialization = assertStoredEvidenceNode(output.materializationNode, refInvalid);
        const embeddedTargetSnapshot = assertStoredEvidenceNode(output.targetSnapshotNode, refInvalid);
        const storedTargetSnapshot = readEvidenceNode(paths, embeddedTargetSnapshot.nodeId);
        const target = readPlanReviewTargetSnapshotNode(storedTargetSnapshot);
        if (canonicalJson(embeddedMaterialization) !==
        ...
        dependencies.push(dependencyClosureEntry('target-snapshot', storedTargetSnapshot));
    }
    else if (Object.hasOwn(manifest, 'planningTarget')) {
        throw refInvalid();
    }
    if (retryShape) {
        const retry = assertPlanReviewRetryOwnership(paths, refs, node, output, request, expectedChangeId);
        if (retry.previousOwnership.ownerInvestigationId !==
            materializationOwnership.ownerInvestigationId) {
            throw refInvalid();
        }
        dependencies.push(dependencyClosureEntry('previous-request', retry.previousNode), ...retry.previousOwnership.dependencies.map((dependency) => Object.freeze({
            ...dependency,
    ...
    return Object.freeze({
        ...
        dependencies: Object.freeze(dependencies.sort((left, right) => left.role.localeCompare(right.role))),
    });
}
function assertPlanReviewRetryOwnership(paths, refs, node, output, request, expectedChangeId) {
    ...
    if (!isPlainRecord(retry) ||
        ...
        retry.failedInvocation.attempt < 1 ||
        !Number.isSafeInteger(retry.failedInvocation.revision) ||
        retry.failedInvocation.revision < 0 ||
        ...
        (retryDecisionShape &&
            (!isPlainRecord(retry.retryDecision) ||
                ...
                !/^[a-z0-9][a-z0-9._:-]{0,255}$/.test(retry.retryDecision.executionJobId) ||
                !Number.isSafeInteger(retry.retryDecision.executionRevision) ||
                retry.retryDecision.executionRevision < 0 ||
        ...
        (retryDecisionShape &&
            (!isPlainRecord(retry.executionPolicySnapshot) ||
                ...
                ![2, 3].includes(retry.executionPolicySnapshot.schemaVersion) ||
                (retry.executionPolicySnapshot.schemaVersion === 3 &&
    ...
    if (previousNode.resultDigest !==
        node.semanticParentResultDigests.previousRequest) {
        throw refInvalid();
    }
    const previousOwnership = assertPlanReviewRequestOwnership(paths, refs, previousNode, expectedChangeId);
    if (!isPlainRecord(previousNode.output)) {
        throw refInvalid();
    }
    const previousOutput = previousNode.output;
    const previousRequest = recreateProviderInvocationRequest(previousOutput.request);
    const previousRetry = previousOutput.retry;
    const previousAttempt = previousNode.nodeSchema === 'workflow.plan-review-request-reservation.v1'
        ? 1
        : isPlainRecord(previousRetry) &&
            Number.isSafeInteger(previousRetry.attempt) &&
            previousRetry.attempt >= 2
            ? previousRetry.attempt
            : 0;
    const { invocationId: _previousInvocationId, nonce: _previousNonce, requestDigest: _previousRequestDigest, policyDigest: _previousPolicyDigest, limits: _previousLimits, ...previousRequestBinding } = previousRequest;
    const { invocationId: _replacementInvocationId, nonce: _replacementNonce, requestDigest: _replacementRequestDigest, policyDigest: _replacementPolicyDigest, limits: _replacementLimits, ...replacementRequestBinding } = request;
...
function assertPlanReviewGrantRequirementOwnership(node, expectedChangeId) {
    const output = node.output;
    assertProposeEvidenceNode(node, {
    ...
    const subject = assertPlanReviewSubject(output.subject);
    if (node.exactInputDigests.subject !== subject.subjectDigest ||
...
function assertExemptionSessionOwnership(paths, node, expectedChangeId) {
    const output = node.output;
    if (node.type !== 'propose-exemption-session-reservation' ||
    ...
    const applicability = readApplicabilityDependency(paths, node.provenanceParentNodeIds.applicability, node.semanticParentResultDigests.applicability);
    return Object.freeze({
        ownerInvestigationId: output.investigationId,
        dependencies: Object.freeze([applicability]),
    });
}
function assertProposeEvidenceNode(node, expected) {
...
function readApplicabilityDependency(paths, nodeId, resultDigest) {
    const node = readEvidenceNode(paths, nodeId);
...
function dependencyClosureEntry(role, node) {
    return Object.freeze({
        role,
        nodeId: node.nodeId,
        resultDigest: node.resultDigest,
...
function isDigestRecord(value) {
    return (isPlainRecord(value) &&
        Object.entries(value).every(([name, digest]) => name.length > 0 && typeof digest === 'string' && isDigest(digest)));
}
function isInvestigationOwner(value) {
...
function isTaskStrategyImplementationAssignmentForAuthor(value, author, subjectDigest) {
    if (!isProviderRoleAssignment(value) ||
    ...
    if (!('grantId' in value)) {
        return (value.providerId !== author.providerId &&
            value.achievedIndependence === 'provider-independent');
    }
    return (value.degradedForm === 'same-provider-fresh-session' &&
...
function isTaskStrategyRedAuthor(value) {
    return (isPlainRecord(value) &&
        hasExactKeys(value, [
            'providerId',
            'sessionId',
...
function ensureNoFollowDirectory(base, privateRoot, directory, makeError) {
    walkNoFollowDirectory(base, privateRoot, directory, makeError, true);
}
function assertNoFollowDirectory(base, privateRoot, directory, makeError) {
    return walkNoFollowDirectory(base, privateRoot, directory, makeError, false);
}
function walkNoFollowDirectory(base, privateRoot, directory, makeError, create) {
    ...
    for (const segment of relative.split(path.sep)) {
        ...
        if (isInsideOrEqual(privateRoot, current) &&
...
function evidenceObjectPath(paths, nodeId) {
    return path.join(paths.objects, nodeId.slice(0, 2), `${nodeId}.json`);
}
function evidencePublicationAlias(objectPath) {
    return `${objectPath}.${process.pid}.${crypto.randomUUID()}.publish.tmp`;
}
function inspectEvidenceObjectFinal(objectPath, exactContent) {
...
function publishLegacyEvidenceRepairClaim(objectPath, publishAlias, exactContent) {
    ...
    const claim = readStablePrivateFile(claimPath, [claimStats.nlink], objectUnsafe);
    if (!claim.bytes.equals(exactContent)) {
        throw objectCollision(path.basename(objectPath, '.json'));
    }
    recoverLegacyEvidenceRepairClaim(objectPath, exactContent);
}
function recoverLegacyEvidenceRepairClaim(objectPath, exactContent) {
    const claimPath = `${objectPath}.legacy-prefix-repair`;
    let claimStats = fs.lstatSync(claimPath, { throwIfNoEntry: false });
    if (!claimStats) {
        return;
    }
    let claim = readStablePrivateFile(claimPath, [claimStats.nlink], objectUnsafe);
    if (!claim.bytes.equals(exactContent)) {
        throw objectCollision(path.basename(objectPath, '.json'));
    }
    if (claim.stats.nlink === 2) {
        ...
        if (finalStats?.dev === claim.stats.dev &&
        ...
        const linkedAliases = listEvidencePublicationAliases(objectPath).filter((alias) => {
        ...
        const linkedAlias = readStablePrivateFile(linkedAliases[0].aliasPath, [2], objectUnsafe);
    ...
    if (finalStats) {
        const final = readStablePrivateFile(objectPath, [finalStats.nlink], objectUnsafe);
        if (final.bytes.equals(exactContent)) {
            fs.unlinkSync(claimPath);
            fsyncDirectory(path.dirname(objectPath));
            return;
        }
        if (final.stats.nlink !== 1 ||
    ...
    if (!published.bytes.equals(exactContent) ||
...
function resolveLinkedEvidenceObjectFinal(objectPath, exactContent) {
    const initial = fs.lstatSync(objectPath, { throwIfNoEntry: false });
    if (!initial || initial.nlink === 1) {
        return;
    }
    const final = readStablePrivateFile(objectPath, [initial.nlink], objectUnsafe);
    const linkedAliases = [];
    for (const alias of listEvidencePublicationAliases(objectPath)) {
        ...
        const observed = readStablePrivateFile(alias.aliasPath, [initial.nlink], objectUnsafe);
        linkedAliases.push({
            alias,
    ...
    if (linkedAliases[0].alias.kind !== 'publish' ||
        !linkedAliases[0].bytes.equals(exactContent)) {
        throw objectUnsafe();
    }
    unlinkValidatedEvidenceAliases(linkedAliases.map(({ alias, stats }) => ({ alias, stats })), path.dirname(objectPath));
}
function reclaimEvidencePublicationAliases(objectPath, exactContent) {
    const objectDirectory = path.dirname(objectPath);
    const finalStats = fs.lstatSync(objectPath, { throwIfNoEntry: false });
    const finalIsExact = finalStats !== undefined &&
        readStablePrivateFile(objectPath, [finalStats.nlink], objectUnsafe).bytes.equals(exactContent);
    ...
    for (const alias of aliases) {
        ...
        const observed = readStablePrivateFile(alias.aliasPath, [aliasStats.nlink], objectUnsafe);
        if (!observed.bytes.equals(exactContent) &&
            !(observed.stats.nlink === 1 &&
                isStrictBufferPrefix(observed.bytes, exactContent))) {
            throw objectUnsafe();
        }
        if (observed.stats.nlink > 1 &&
            (!finalStats ||
                !finalIsExact ||
...
function unlinkValidatedEvidenceAliases(reclaimable, objectDirectory) {
    let removed = false;
...
function listEvidencePublicationAliases(objectPath) {
    const objectDirectory = path.dirname(objectPath);
    ...
    return fs
        ...
        const match = EVIDENCE_PUBLICATION_TEMP_SUFFIX.exec(name.slice(prefix.length));
        if (!match) {
            return [];
        }
        return [
...
function assertExactEvidenceObjectFinal(objectPath, exactContent) {
    const observed = readStablePrivateFile(objectPath, [1], objectUnsafe);
    if (!observed.bytes.equals(exactContent)) {
        throw objectCollision(path.basename(objectPath, '.json'));
    }
}
function assertExactEvidencePublicationAlias(aliasPath, expected, exactContent, expectedLinkCount) {
    const observed = readStablePrivateFile(aliasPath, [expectedLinkCount], objectUnsafe);
    if (observed.stats.dev !== expected.dev ||
        observed.stats.ino !== expected.ino ||
        !observed.bytes.equals(exactContent)) {
        throw objectUnsafe();
    }
}
function unlinkOwnedEvidencePublicationAlias(aliasPath, expected, objectDirectory) {
...
function readStablePrivateFile(filePath, allowedLinkCounts, makeError) {
    ...
    try {
        ...
        if (typeof error === 'object' &&
            error !== null &&
            'code' in error &&
            error.code === 'EVIDENCE_OBJECT_UNSAFE') {
            throw error;
        }
        throw makeError();
    }
...
function isStrictBufferPrefix(prefix, exact) {
    return (prefix.byteLength < exact.byteLength &&
...
function evidenceRefPath(paths, changeId) {
    return path.join(paths.refs, `${changeId}.json`);
}
function readNoFollow(filePath, makeError) {
...
function assertPrivateFileStats(stats, makeError) {
    if (!stats.isFile() ||
        stats.isSymbolicLink() ||
        stats.nlink !== 1 ||
        (stats.mode & 0o777) !== 0o600) {
...
function assertNodeId(nodeId) {
    if (!DIGEST_PATTERN.test(nodeId)) {
        throw workflowError('EVIDENCE_OBJECT_ID_INVALID', `Invalid evidence node identifier: ${nodeId}`, ExitCode.usage);
    }
}
function assertRefName(refName) {
    if (typeof refName !== 'string' || !REF_NAME_PATTERN.test(refName)) {
        throw workflowError('EVIDENCE_REF_NAME_INVALID', `Invalid evidence ref name: ${refName}`, ExitCode.usage);
    }
}
function objectUnsafe() {
    return workflowError('EVIDENCE_OBJECT_UNSAFE', 'Evidence object path is not a canonical no-follow location.', ExitCode.unsafeEnvironment);
}
function objectInvalid() {
    return workflowError('EVIDENCE_OBJECT_INVALID', 'Evidence object envelope is forged, tampered, or noncanonical.', ExitCode.staleState);
}
function objectCollision(nodeId) {
    return workflowError('EVIDENCE_OBJECT_COLLISION', 'A different evidence envelope already exists for this node identifier.', ExitCode.conflict, { details: { nodeId } });
}
function objectUnavailable(nodeId) {
    return workflowError('EVIDENCE_OBJECT_UNAVAILABLE', 'The next evidence node must already exist as a stored object.', ExitCode.staleState, { details: { nodeId } });
}
function refUnsafe() {
    return workflowError('EVIDENCE_REF_UNSAFE', 'Evidence ref path is not a canonical no-follow location.', ExitCode.unsafeEnvironment);
}
function refInvalid() {
    return workflowError('EVIDENCE_REF_INVALID', 'Evidence ref document is malformed or noncanonical.', ExitCode.staleState);
}
function refCasMismatch(refName, expectedNodeId, observedNodeId) {
    return workflowError('EVIDENCE_REF_CAS_MISMATCH', 'Evidence ref changed during compare-and-swap.', ExitCode.conflict, { details: { refName, expectedNodeId, observedNodeId } });
}
function refsDocumentCasMismatch(expectedDigest, observedDigest, nextDigest) {
    return workflowError('EVIDENCE_REFS_CAS_MISMATCH', 'Evidence ref document changed during compare-and-swap.', ExitCode.conflict, { details: { expectedDigest, observedDigest, nextDigest } });
}
function refLocked(changeId) {
    return workflowError('EVIDENCE_REF_LOCKED', 'Evidence ref for this change is locked by another operation.', ExitCode.conflict, { details: { changeId } });
}
function refLockInvalid() {
    return workflowError('EVIDENCE_REF_LOCK_INVALID', 'Evidence ref lock ownership changed during the operation.', ExitCode.staleState);
}
...
function observeUnsafeNode(filePath) {
    const before = fs.lstatSync(filePath, { throwIfNoEntry: false });
    if (!before) {
        return { exists: false };
    }
    ...
    if (before.isSymbolicLink()) {
        ...
        const names = fs.readdirSync(filePath, { encoding: 'buffer' }).sort(Buffer.compare);
        observation = {
            ...common,
            kind: 'directory',
    ...
    const after = fs.lstatSync(filePath, { throwIfNoEntry: false });
    if (!after ||
        !sameUnsafeObservationStableStats(stableBefore, unsafeObservationStableStats(after))) {
        throw new Error('Unsafe path changed while it was being observed.');
    }
    return observation;
}
function digestUnsafeRegularFile(filePath, expected) {
    const descriptor = fs.openSync(filePath, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW);
    try {
        const opened = fs.fstatSync(descriptor);
        if (!sameUnsafeObservationStableStats(expected, unsafeObservationStableStats(opened))) {
        ...
        if (!sameUnsafeObservationStableStats(expected, unsafeObservationStableStats(closedOver))) {
...
function unsafeObservationChildPath(parent, name) {
...
function unsafeObservationStableStats(stats) {
    return {
        dev: stats.dev,
        ino: stats.ino,
        mode: stats.mode,
...
function sameUnsafeObservationStableStats(left, right) {
