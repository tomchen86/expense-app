...
const ACTIVE_HUMAN_RESOLUTION_JOURNAL_TEMPORARY = /^[a-z0-9]+(?:-[a-z0-9]+)*\.json\.[1-9][0-9]*\.[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.tmp$/;
const HUMAN_RESOLUTION_GRANT_FILE = /^([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.json$/;
...
export function investigationCheckpointId(session, kind) {
    const prerequisites = kind === 'main-terms'
        ? {
    ...
    return `checkpoint-${sha256(canonicalJson({
        schemaVersion: 1,
...
export function createInvestigationSessionRecord(paths, session) {
    const validated = assertInvestigationSession(session);
    const sessionPath = investigationSessionPath(paths, validated.investigationId);
    createPrivateCanonicalJson(paths, sessionPath, validated, sessionUnsafe, 'INVESTIGATION_SESSION_COLLISION');
    return readInvestigationSession(paths, validated.investigationId);
}
export function readInvestigationSession(paths, requestedInvestigationId) {
    const investigationId = assertInvestigationId(requestedInvestigationId);
    const value = readPrivateCanonicalJson(paths, investigationSessionPath(paths, investigationId), sessionUnsafe);
    const session = assertInvestigationSession(value);
    if (session.investigationId !== investigationId) {
        throw workflowError('INVESTIGATION_SESSION_ID_MISMATCH', 'Investigation session content does not match its filename.', ExitCode.staleState);
    }
    return deepFreeze(structuredClone(session));
}
export function investigationSessionExists(paths, requestedInvestigationId) {
    const investigationId = assertInvestigationId(requestedInvestigationId);
    return privatePathExists(paths, investigationSessionPath(paths, investigationId), sessionUnsafe);
}
export function compareAndSwapInvestigationSession(paths, requestedInvestigationId, expectedRevision, transition) {
    const investigationId = assertInvestigationId(requestedInvestigationId);
    return withPrivateRuntimeLock(paths, path.join(paths.locks, `${investigationId}.lock`), () => {
        ...
        if (proposed.semanticRevision !== current.semanticRevision ||
        ...
        writePrivateCanonicalJsonAtomic(paths, investigationSessionPath(paths, investigationId), next, sessionUnsafe);
        return readInvestigationSession(paths, investigationId);
    }, 'INVESTIGATION_SESSION_OPERATION_CONFLICT', sessionLockInvalid);
}
export function createCurrentInvestigationRef(paths, changeId, investigationId) {
    const ref = {
        schemaVersion: 1,
        changeId: assertChangeId(changeId),
        investigationId: assertInvestigationId(investigationId),
    };
    createPrivateCanonicalJson(paths, currentInvestigationRefPath(paths, ref.changeId), ref, refUnsafe, 'CURRENT_INVESTIGATION_CONFLICT');
}
export function readCurrentInvestigationRef(paths, requestedChangeId) {
...
export function compareAndSwapCurrentInvestigationRef(paths, requestedChangeId, expectedInvestigationId, nextInvestigationId) {
    const changeId = assertChangeId(requestedChangeId);
    ...
    if (nextInvestigationId !== null) {
        ...
        if (next.changeId !== changeId) {
            throw workflowError('CURRENT_INVESTIGATION_REPAIR_INVALID', 'A replacement current investigation must belong to the same change.', ExitCode.guard);
        }
    }
    withPrivateRuntimeLock(paths, path.join(paths.locks, `${changeId}.current-investigation.lock`), () => {
        const observed = readCurrentInvestigationRef(paths, changeId);
        if ((observed?.investigationId ?? null) !==
            (expectedInvestigationId ?? null)) {
            throw workflowError('CURRENT_INVESTIGATION_REF_STALE', 'The current investigation ref no longer matches the authorized state.', ExitCode.staleState);
        }
        const refPath = currentInvestigationRefPath(paths, changeId);
        if (nextInvestigationId === null) {
            if (observed !== null) {
        ...
        writePrivateCanonicalJsonAtomic(paths, refPath, {
            schemaVersion: 1,
            changeId,
            investigationId: nextInvestigationId,
        }, refUnsafe);
    }, 'CURRENT_INVESTIGATION_OPERATION_CONFLICT', refUnsafe);
}
export function quarantineUnsafeCurrentInvestigationRef(paths, requestedChangeId, expectedObservationDigest) {
    const changeId = assertChangeId(requestedChangeId);
    ...
    const target = path.join(quarantineDirectory, `${changeId}.${expectedObservationDigest}.artifact`);
    return withPrivateRuntimeLock(paths, path.join(paths.locks, `${changeId}.current-investigation.lock`), () => {
        if (!walkPrivateDirectory(paths, path.dirname(source), refUnsafe, false)) {
            throw workflowError('CURRENT_INVESTIGATION_REF_STALE', 'The current investigation ref disappeared before quarantine.', ExitCode.staleState);
        }
        ...
        if (!sourceStats) {
            ...
            throw workflowError('CURRENT_INVESTIGATION_REF_STALE', 'The current investigation ref changed before quarantine.', ExitCode.staleState);
        }
        if (observeUnsafePath(source, 'current-ref', []) !==
            expectedObservationDigest) {
            throw workflowError('CURRENT_INVESTIGATION_REF_STALE', 'The current investigation ref changed before quarantine.', ExitCode.staleState);
        }
        try {
        ...
        ensurePrivateInvestigationDirectory(paths, quarantineDirectory, humanResolutionArchiveUnsafe);
        if (fs.lstatSync(target, { throwIfNoEntry: false })) {
            throw workflowError('CURRENT_INVESTIGATION_QUARANTINE_CONFLICT', 'A different quarantine artifact already occupies the exact target.', ExitCode.conflict);
        }
...
export function archiveHumanResolutionSingleton(paths, kind, expectedDigest, content, newlineRequired) {
    if (!isDigest(expectedDigest)) {
        throw humanResolutionRecoveryAmbiguous();
    }
    const targetDirectory = path.join(humanResolutionPaths(paths).retiredRefs, kind);
    ...
    ensurePrivateInvestigationDirectory(paths, targetDirectory, humanResolutionArchiveUnsafe);
    const targetStats = fs.lstatSync(target, { throwIfNoEntry: false });
    if (targetStats) {
        const targetContent = readPrivateFile(target, humanResolutionArchiveUnsafe);
        if (sha256(targetContent) !== expectedDigest ||
...
export function quarantineUnsafeInvestigationStartReservation(paths, requestedChangeId, expectedObservationDigest) {
    const changeId = assertChangeId(requestedChangeId);
    if (!isDigest(expectedObservationDigest)) {
        throw humanResolutionRecoveryAmbiguous();
    }
    ...
    const quarantineDirectory = humanResolutionPaths(paths).quarantine;
    const target = path.join(quarantineDirectory, `${changeId}.${expectedObservationDigest}.start-reservation.artifact`);
    const lockPath = path.join(paths.locks, `${changeId}.investigation-start.lock`);
    const makeError = startReservationUnsafe;
    return withPrivateRuntimeLock(paths, lockPath, () => {
        ...
        return target;
    }, 'HUMAN_RESOLUTION_SINGLETON_OPERATION_CONFLICT', makeError);
}
function canonicalPrivateContent(content, newlineRequired) {
    let value;
    try {
...
export function humanResolutionDecisionSchemaDigest(kind) {
    return sha256(canonicalJson(HUMAN_RESOLUTION_DECISION_SCHEMAS[kind]));
}
export function assertHumanResolutionDecision(value) {
    if (!isRecord(value) || typeof value.kind !== 'string') {
        throw humanResolutionInvalid('Human resolution decision is malformed.');
    }
    if (value.kind === 'resume-with-capability' &&
 ...
 */
export function assertLegacySupersedeHumanResolutionDecisionReadOnly(value) {
    if (!isRecord(value) ||
        ...
        (value.parameters.successorInvestigationId !== null &&
            typeof value.parameters.successorInvestigationId !== 'string')) {
        throw humanResolutionInvalid('Legacy supersede decision is not a read-only historical record.');
    }
    if (typeof value.parameters.successorInvestigationId === 'string') {
        assertInvestigationId(value.parameters.successorInvestigationId);
    }
...
export function assertHumanResolutionConsequences(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, ['continuity', 'assurance', 'claimsWaived']) ||
        !['preserved', 'broken', 'not-applicable'].includes(String(value.continuity)) ||
        !['unchanged', 'human-waived', 'degraded'].includes(String(value.assurance)) ||
        ...
        throw humanResolutionInvalid('Human resolution consequences are malformed.');
    }
    return deepFreeze(structuredClone(value));
}
export function listProviderInvocationLifecycleProjections(paths) {
    const scan = scanProviderInvocationLifecycles(paths);
    const unsafeInvocations = scan.unsafeInvocations.filter(({ invocationId }) => !isDurablyReservedProviderExecutionPolicySnapshot(paths, invocationId));
    if (unsafeInvocations.length > 0) {
        throw providerInvocationUnsafe();
    }
    return [...scan.projections];
}
export function scanProviderInvocationLifecycles(paths) {
    ...
    if (!stats.isDirectory() ||
        ...
        !walkPrivateDirectory(paths, paths.invocations, providerInvocationUnsafe, false)) {
        throw providerInvocationUnsafe();
    }
    const projections = [];
    const unsafeInvocations = [];
    for (const entry of fs
        ...
        try {
            projections.push(readProviderInvocationLifecycleProjection(paths, invocationId));
        }
        catch {
            unsafeInvocations.push({
                invocationId,
                ownerInvestigationId: tryReadPlanReviewInvocationOwnerHint(paths, invocationId),
                observationDigest: exactUnsafePathObservationDigest(path.join(paths.invocations, invocationId), `provider-invocation:${invocationId}`),
            });
        }
    }
    return deepFreeze({
...
function tryReadPlanReviewInvocationOwnerHint(paths, invocationId) {
    try {
        const directory = path.join(paths.invocations, invocationId);
        const request = recreateProviderInvocationRequest(readPrivateCanonicalJson(paths, path.join(directory, 'request.json'), providerInvocationUnsafe));
        const manifest = readPrivateCanonicalJson(paths, path.join(directory, 'manifest.json'), providerInvocationUnsafe);
...
export function readProviderInvocationLifecycleProjection(paths, requestedInvocationId) {
    const invocationId = assertInvocationId(requestedInvocationId);
    const directory = path.join(paths.invocations, invocationId);
    const value = readPrivateCanonicalJson(paths, path.join(directory, 'state.json'), providerInvocationUnsafe);
    const requestValue = readPrivateCanonicalJson(paths, path.join(directory, 'request.json'), providerInvocationUnsafe);
    const manifest = readPrivateCanonicalJson(paths, path.join(directory, 'manifest.json'), providerInvocationUnsafe);
    let request;
    try {
    ...
    if (!isRecord(value) ||
        ...
        !['blind-survey-manifest', 'plan-review-manifest'].includes(String(manifest.kind)) ||
    ...
    const ownerInvestigationId = value.purpose === 'plan-review'
...
export function inspectInvestigationResolutionState(paths, requestedInvestigationId, repositoryId) {
    ...
    const currentRefDigest = currentRef === null ? null : sha256(`${canonicalJson(currentRef)}\n`);
    const startReservationDigest = digestBoundStartReservation(paths, session, startReservationUnsafe);
    const resolutionHeadNodeId = readHumanResolutionHead(paths, investigationId);
    const resolutionNode = resolutionHeadNodeId === null
        ? null
    ...
    const retryState = bindProviderRetryReservationResolutionState(paths, session, providerInvocationUnsafe);
    const providerState = bindProviderInvocationResolutionState(paths, session, retryState.bindings);
    const evidenceClosure = readInvestigationEvidenceRefsClosure(paths, session.changeId);
    assertEvidenceClosureTargetBinding(paths, session, evidenceClosure);
    const evidenceRefsDigest = evidenceClosure.snapshot.digest;
    const evidenceRefs = evidenceClosure.snapshot.refs === null
        ? null
        : { ...evidenceClosure.snapshot.refs };
    const evidenceRefsClosureDigest = evidenceClosure.closureDigest;
    const blockerDigest = session.blocker === null ? null : sha256(canonicalJson(session.blocker));
    const envelope = {
        schemaVersion: 2,
...
export function inspectInvestigationQuarantineState(paths, requestedInvestigationId, repositoryId) {
    try {
        const strict = inspectInvestigationResolutionState(paths, requestedInvestigationId, repositoryId);
        ...
        const before = readHumanResolutionArchive(paths, journal.evidenceArchiveDigest);
        const envelope = {
            ...strict.envelope,
    ...
    try {
        ...
        currentRefDigest = observeUnsafePath(currentInvestigationRefPath(paths, session.changeId), 'current-ref', ambiguities);
    }
    let startReservationDigest;
    try {
        startReservationDigest = digestBoundStartReservation(paths, session, startReservationUnsafe);
    }
    catch {
        startReservationDigest = observeUnsafePath(investigationStartReservationPath(paths, session.changeId), 'start-reservation', ambiguities);
    }
    let resolutionHeadNodeId;
    let resolutionNode = null;
    try {
        resolutionHeadNodeId = readHumanResolutionHeadReference(paths, investigationId);
        if (resolutionHeadNodeId !== null) {
            try {
                resolutionNode = readHumanResolutionNode(paths, resolutionHeadNodeId);
            }
            catch {
                observeUnsafePath(humanResolutionNodePath(paths, resolutionHeadNodeId), `resolution-node:${resolutionHeadNodeId}`, ambiguities);
            }
        }
    }
    catch {
        observeUnsafePath(humanResolutionRefPath(paths, investigationId), 'resolution-ref', ambiguities);
        resolutionHeadNodeId = null;
    }
    let providerRetryReservations;
    let providerRetryBindings = [];
    try {
        const retryState = bindProviderRetryReservationResolutionState(paths, session, providerInvocationUnsafe);
        providerRetryReservations = retryState.reservations;
    ...
    let providerInvocationDigests;
    let repositoryProviderLeases;
    try {
        const providerState = bindProviderInvocationResolutionState(paths, session, providerRetryBindings);
        providerInvocationDigests = providerState.providerInvocationDigests;
        repositoryProviderLeases = providerState.repositoryProviderLeases;
    }
    catch {
        providerInvocationDigests = session.blindInvocationIds.map((invocationId) => {
            try {
                return {
                    invocationId,
                    files: digestPrivateDirectoryEntries(paths, path.join(paths.invocations, invocationId), providerInvocationUnsafe),
                };
            }
            catch {
                const digestValue = observeUnsafePath(path.join(paths.invocations, invocationId), `provider-invocation:${invocationId}`, ambiguities);
                return {
                    invocationId,
    ...
    let evidenceRefs;
    let evidenceRefsDigest;
    let evidenceRefsClosureDigest;
    try {
        const evidenceClosure = readInvestigationEvidenceRefsClosure(paths, session.changeId);
        assertEvidenceClosureTargetBinding(paths, session, evidenceClosure);
        ...
        const observations = observeInvestigationEvidenceRefsAmbiguities(paths, session.changeId);
    ...
    const ambiguityDigest = sha256(canonicalJson({
    ...
    const effectiveState = resolutionNode === null
    ...
    return deepFreeze({
        ...
        availableResolutions: terminalHumanResolutionState(effectiveState)
            ...
            : [
                {
                    kind: 'quarantine',
                    parameterSchemaDigest: humanResolutionDecisionSchemaDigest('quarantine'),
                },
            ],
        effectiveState,
    });
}
export function investigationResolutionStateDigest(envelope) {
    return sha256(canonicalJson({
        schema: HUMAN_RESOLUTION_SCHEMA,
        envelope,
    }));
}
export function investigationCurrentRefDigest(input) {
    return input === null
        ? null
...
export function advertisedHumanResolutions(blocker) {
    const kinds = [
        'abort',
        'quarantine',
        'supersede',
...
export function assertInvestigationCheckpointEnvelope(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
...
export function checkpointEnvelopeDigest(envelope) {
    return sha256(canonicalJson(assertInvestigationCheckpointEnvelope(envelope)));
}
export function checkpointContributionDigest(envelope) {
...
export function ensurePrivateInvestigationDirectory(paths, directory, makeError) {
    walkPrivateDirectory(paths, directory, makeError, true);
}
export function assertPrivateInvestigationDirectory(paths, directory, makeError) {
    walkPrivateDirectory(paths, directory, makeError, false);
}
export function readPrivateCanonicalJson(paths, filePath, makeError) {
...
export function createPrivateCanonicalJson(paths, filePath, value, makeError, collisionCode) {
    ...
    if (existing) {
        ...
        throw workflowError(collisionCode, 'A different durable investigation record already exists.', ExitCode.conflict);
    }
...
export function writePrivateCanonicalJsonAtomic(paths, filePath, value, makeError) {
    const content = `${canonicalJson(value)}\n`;
    ensurePrivateInvestigationDirectory(paths, path.dirname(filePath), makeError);
    const existing = fs.lstatSync(filePath, { throwIfNoEntry: false });
    if (existing) {
...
export function withPrivateRuntimeLock(paths, lockPath, operation, conflictCode, invalidLock) {
    ensurePrivateInvestigationDirectory(paths, path.dirname(lockPath), invalidLock);
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            descriptor = publishPreparedExclusiveLock(lockPath, marker, ownerToken, invalidLock);
            break;
        }
        catch (error) {
            ...
            if (isNodeError(error) && error.code === 'EEXIST') {
                throw workflowError(conflictCode, 'A durable investigation operation is already in progress.', ExitCode.conflict);
            }
            throw error;
        }
    }
...
export function privatePathExists(paths, filePath, makeError) {
...
export function createInvestigationHumanActionBlocker(input) {
    if (!isBoundedResolutionText(input.reasonCode, 256) ||
        !isBoundedResolutionText(input.blockedTransition, 256) ||
        !isRecord(input.facts)) {
        throw humanResolutionInvalid('Investigation human-action blocker is malformed.');
    }
    ...
    if (Buffer.byteLength(canonicalJson(blocker), 'utf8') > 65_536) {
        throw humanResolutionInvalid('Investigation human-action blocker exceeds its fixed bound.');
    }
    return deepFreeze(blocker);
}
export function createHumanResolutionNode(input) {
...
export function createHumanResolutionJournal(input) {
    const semantic = {
        schemaVersion: 2,
    ...
    const candidate = {
        ...semantic,
        journalId: sha256(canonicalJson({
...
export function writeHumanResolutionNode(paths, node) {
    const validated = assertHumanResolutionNode(node);
    createPrivateCanonicalJson(paths, humanResolutionNodePath(paths, validated.nodeId), validated, humanResolutionObjectUnsafe, 'HUMAN_RESOLUTION_NODE_COLLISION');
    return validated.nodeId;
}
export function readHumanResolutionNode(paths, requestedNodeId) {
    if (!isDigest(requestedNodeId)) {
        throw humanResolutionObjectUnsafe();
    }
    const value = readPrivateCanonicalJson(paths, humanResolutionNodePath(paths, requestedNodeId), humanResolutionObjectUnsafe);
...
export function readHumanResolutionHead(paths, requestedInvestigationId) {
    const nodeId = readHumanResolutionHeadReference(paths, requestedInvestigationId);
    if (nodeId !== null) {
        readHumanResolutionNode(paths, nodeId);
    }
    return nodeId;
}
function readHumanResolutionHeadReference(paths, requestedInvestigationId) {
    const investigationId = assertInvestigationId(requestedInvestigationId);
    const refPath = humanResolutionRefPath(paths, investigationId);
    if (!privatePathExists(paths, refPath, humanResolutionRefUnsafe)) {
        return null;
    }
    const value = readPrivateCanonicalJson(paths, refPath, humanResolutionRefUnsafe);
    if (!isRecord(value) ||
...
export function compareAndSwapHumanResolutionHead(paths, requestedInvestigationId, expectedNodeId, nextNodeId) {
    const investigationId = assertInvestigationId(requestedInvestigationId);
    ...
    withPrivateRuntimeLock(paths, path.join(humanResolutionPaths(paths).locks, `${investigationId}.resolution.lock`), () => {
        const observed = readHumanResolutionHead(paths, investigationId);
        if (observed !== expectedNodeId) {
            throw workflowError('HUMAN_RESOLUTION_REF_STALE', 'The human-resolution overlay changed after grant issuance.', ExitCode.staleState);
        }
        writePrivateCanonicalJsonAtomic(paths, humanResolutionRefPath(paths, investigationId), {
            schemaVersion: 1,
            investigationId,
            nodeId: nextNodeId,
        }, humanResolutionRefUnsafe);
    }, 'HUMAN_RESOLUTION_OPERATION_CONFLICT', humanResolutionRefUnsafe);
}
export function quarantineUnsafeHumanResolutionRef(paths, requestedInvestigationId, repositoryId, expectedStateDigest) {
    const investigationId = assertInvestigationId(requestedInvestigationId);
    if (!isBoundedResolutionText(repositoryId, 1024) ||
    ...
    const quarantineDirectory = humanResolutionPaths(paths).quarantine;
    const target = path.join(quarantineDirectory, `${investigationId}.${expectedStateDigest}.resolution-ref-artifact`);
    return withPrivateRuntimeLock(paths, path.join(humanResolutionPaths(paths).locks, `${investigationId}.resolution.lock`), () => {
        if (!walkPrivateDirectory(paths, path.dirname(source), humanResolutionRefUnsafe, false)) {
            throw workflowError('HUMAN_RESOLUTION_REF_STALE', 'The human-resolution ref disappeared before quarantine.', ExitCode.staleState);
        }
        const sourceStats = fs.lstatSync(source, { throwIfNoEntry: false });
        if (!sourceStats) {
            if (fs.lstatSync(target, { throwIfNoEntry: false })) {
            ...
            throw workflowError('HUMAN_RESOLUTION_REF_STALE', 'The human-resolution ref changed before quarantine.', ExitCode.staleState);
        }
        const observed = inspectInvestigationQuarantineState(paths, investigationId, repositoryId);
        if (observed.currentStateDigest !== expectedStateDigest ||
            observed.envelope.ambiguityDigest === null) {
            throw workflowError('HUMAN_RESOLUTION_REF_STALE', 'The human-resolution ref changed after grant issuance.', ExitCode.staleState);
        }
        try {
            readHumanResolutionHead(paths, investigationId);
            throw workflowError('HUMAN_RESOLUTION_REF_STALE', 'A readable human-resolution ref cannot use the unsafe-ref quarantine transition.', ExitCode.staleState);
        }
        catch (error) {
            if (error &&
                typeof error === 'object' &&
        ...
        ensurePrivateInvestigationDirectory(paths, quarantineDirectory, humanResolutionArchiveUnsafe);
        if (fs.lstatSync(target, { throwIfNoEntry: false })) {
            throw workflowError('HUMAN_RESOLUTION_QUARANTINE_CONFLICT', 'A different quarantine artifact already occupies the exact target.', ExitCode.conflict);
        }
        fs.renameSync(source, target);
        fsyncDirectory(path.dirname(source));
        fsyncDirectory(quarantineDirectory);
...
export function storeAvailableHumanResolutionGrant(paths, requestedGrantId, canonicalEnvelope) {
    ...
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        if (readHumanResolutionGrantPublicationRecovery(paths, stores, grantId) !==
            null) {
            throw workflowError('HUMAN_RESOLUTION_GRANT_EXISTS', `Human resolution grant ${grantId} already has publication recovery history.`, ExitCode.conflict);
        }
        for (const directory of [
            ...
            if (privatePathExists(paths, candidate, humanResolutionGrantUnsafe)) {
                throw workflowError('HUMAN_RESOLUTION_GRANT_EXISTS', `Human resolution grant ${grantId} already has durable state.`, ExitCode.conflict);
            }
        }
        const target = path.join(stores.available, `${grantId}.json`);
        createPrivateRawFile(paths, target, canonicalEnvelope, humanResolutionGrantUnsafe);
        return target;
    }, {
...
export function rollbackAvailableHumanResolutionGrant(paths, requestedGrantId, canonicalEnvelope) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    assertCanonicalResolutionEnvelopeBytes(canonicalEnvelope);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        ...
        if (readPrivateFile(target, humanResolutionGrantUnsafe) !==
            canonicalEnvelope) {
            return 'different';
        }
...
export function withHumanResolutionGrantExecution(paths, requestedGrantId, operation, options = {}) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    return withPrivateRuntimeLock(paths, path.join(humanResolutionPaths(paths).locks, `${grantId}.execution.lock`), () => {
        if (!options.allowPublicationRecovery) {
            assertNoPreparedHumanResolutionGrantPublicationRecovery(paths, grantId);
        }
        return operation();
    }, 'HUMAN_RESOLUTION_OPERATION_CONFLICT', humanResolutionGrantUnsafe);
}
export function inspectStoredHumanResolutionGrants(paths, requestedGrantId) {
    const grantId = requestedGrantId === undefined
        ? undefined
    ...
    const ids = grantId
        ? [grantId]
        : [
            ...new Set([stores.available, stores.reserved, stores.terminal].flatMap((directory) => listHumanResolutionGrantIds(paths, directory))),
        ].sort();
    const entries = ids
...
export function inspectInterruptedHumanResolutionGrantPublications(paths, requestedGrantId) {
    const grantId = requestedGrantId === undefined
        ? undefined
        : assertHumanResolutionGrantId(requestedGrantId);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        ...
        const temporaryGrantIds = walkPrivateDirectory(paths, stores.available, humanResolutionGrantUnsafe, false)
            ...
            : [];
        const recoveryGrantIds = grantId === undefined
            ? listHumanResolutionGrantPublicationRecoveryIds(paths, stores)
            : privatePathExists(paths, humanResolutionGrantPublicationRecoveryPath(stores, grantId), humanResolutionGrantUnsafe)
                ? [grantId]
                : [];
        const preparedReceiptGrantIds = recoveryGrantIds.filter((observedGrantId) => readHumanResolutionGrantPublicationRecovery(paths, stores, observedGrantId)?.phase === 'prepared');
        const grantIds = [
            ...new Set([...temporaryGrantIds, ...preparedReceiptGrantIds]),
        ].sort();
        return grantIds.map((observedGrantId) => {
            const receipt = readHumanResolutionGrantPublicationRecovery(paths, stores, observedGrantId);
            if (receipt?.phase === 'prepared') {
                assertHumanResolutionGrantPublicationQuarantineReplayState(paths, stores, receipt);
                return {
            ...
            return {
                storeState: observeHumanResolutionGrantPublicationStoreState(paths, stores, observedGrantId),
...
export function quarantineInterruptedHumanResolutionGrantPublication(paths, requestedGrantId, expectedPublicationStateDigest, reason, bindPublicationState, now = new Date()) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    if (!isDigest(expectedPublicationStateDigest) ||
        !isBoundedResolutionText(reason, 1024) ||
        !Number.isFinite(now.getTime())) {
        throw humanResolutionGrantPublicationRecoveryInvalid();
    }
    const stores = humanResolutionPaths(paths);
    const recoveryId = sha256(canonicalJson({
        schema: 'human-resolution-grant-publication-quarantine.v1',
        grantId,
        publicationStateDigest: expectedPublicationStateDigest,
    }));
    const receiptPath = humanResolutionGrantPublicationRecoveryPath(stores, grantId);
    const result = {
        action: 'quarantined',
        grantId,
        publicationStateDigest: expectedPublicationStateDigest,
    ...
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        const existingReceipt = privatePathExists(paths, receiptPath, humanResolutionGrantUnsafe)
            ? readPrivateCanonicalJson(paths, receiptPath, humanResolutionGrantUnsafe)
            : null;
        if (existingReceipt !== null) {
            const receipt = assertHumanResolutionGrantPublicationQuarantineReceipt(existingReceipt, grantId);
            if (receipt.recoveryId !== recoveryId ||
                receipt.publicationStateDigest !== expectedPublicationStateDigest ||
                receipt.reason !== reason) {
                throw humanResolutionGrantPublicationRecoveryStale();
            }
            assertHumanResolutionGrantPublicationQuarantineReplayState(paths, stores, receipt);
            completeHumanResolutionGrantPublicationQuarantine(paths, stores, receipt);
            finalizeHumanResolutionGrantPublicationQuarantineReceipt(paths, receiptPath, receipt);
            return result;
        }
        const observed = observeHumanResolutionGrantPublicationStoreState(paths, stores, grantId);
        const binding = bindPublicationState(observed);
        const observedStateDigest = binding.publicationStateDigest;
        if (!isDigest(observedStateDigest) ||
        ...
        ensurePrivateInvestigationDirectory(paths, stores.quarantine, humanResolutionGrantUnsafe);
        const receipt = {
            schemaVersion: 1,
        ...
        createPrivateRawFile(paths, receiptPath, `${canonicalJson(receipt)}\n`, humanResolutionGrantUnsafe);
        const validatedReceipt = assertHumanResolutionGrantPublicationQuarantineReceipt(readPrivateCanonicalJson(paths, receiptPath, humanResolutionGrantUnsafe), grantId);
        completeHumanResolutionGrantPublicationQuarantine(paths, stores, validatedReceipt);
        finalizeHumanResolutionGrantPublicationQuarantineReceipt(paths, receiptPath, validatedReceipt);
        return result;
    }, {
        allowInterruptedAvailablePublication: true,
        allowPreparedPublicationRecovery: true,
...
export function revokeStoredHumanResolutionGrant(paths, cwd, requestedGrantId, options) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const current = inspectStoredHumanResolutionGrant(paths, grantId);
    if (current === null)
        throw humanResolutionGrantNotFound(grantId);
    if (current.state === 'consumed') {
        throw workflowError('HUMAN_REVOCATION_STATE_INVALID', 'Consumed human-resolution authority cannot be revoked.', ExitCode.guard);
    }
    if (current.state === 'reserved' &&
        readHumanResolutionJournal(paths, grantId) !== null) {
        throw workflowError('HUMAN_RESOLUTION_RECOVERY_REQUIRED', 'A prepared human-resolution transaction must be recovered instead of revoked.', ExitCode.conflict);
    }
    const binding = humanResolutionRevocationBinding(paths, current.envelopeBytes, grantId);
    const authorization = authorizeHumanRevocation(cwd, binding, options, humanResolutionRevocationAuthorizationPath(paths, grantId), current.revocationAuthorization ?? null);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        ...
        if (privatePathExists(paths, terminalPath, humanResolutionGrantUnsafe)) {
            const terminal = readPrivateCanonicalJson(paths, terminalPath, humanResolutionGrantUnsafe);
            reconcileTerminalHumanResolutionGrantResidual(paths, stores, grantId, terminal);
            const existing = inspectStoredHumanResolutionGrant(paths, grantId);
            if (existing.state !== 'revoked' ||
                existing.terminalReason !== authorization.payload.reason ||
                existing.revocationAuthorization === undefined ||
                canonicalHumanRevocationAuthorization(existing.revocationAuthorization) !== canonicalHumanRevocationAuthorization(authorization)) {
                throw workflowError('HUMAN_REVOCATION_CONFLICT', 'Human-resolution grant already has a different terminal record.', ExitCode.conflict);
            }
            return existing;
        }
        ...
        if (terminalEntry.state !== 'available' &&
            terminalEntry.state !== 'reserved') {
            throw workflowError('HUMAN_REVOCATION_STATE_INVALID', 'Only active human-resolution authority can be revoked.', ExitCode.guard);
        }
        if (terminalEntry.state === 'reserved' &&
            readHumanResolutionJournal(paths, grantId) !== null) {
            throw workflowError('HUMAN_RESOLUTION_RECOVERY_REQUIRED', 'A prepared human-resolution transaction must be recovered instead of revoked.', ExitCode.conflict);
        }
        const sourceDirectory = terminalEntry.state === 'available'
            ? stores.available
            : stores.reserved;
        const sourcePath = path.join(sourceDirectory, `${grantId}.json`);
        const envelope = JSON.parse(terminalEntry.envelopeBytes);
        const record = {
            schemaVersion: 1,
            state: 'revoked',
            grantId,
        ...
        createPrivateCanonicalJson(paths, terminalPath, record, humanResolutionGrantUnsafe, 'HUMAN_RESOLUTION_GRANT_TERMINAL_CONFLICT');
        fs.unlinkSync(sourcePath);
        fsyncDirectory(sourceDirectory);
        return inspectStoredHumanResolutionGrant(paths, grantId);
    }, { targetGrantId: grantId });
}
export function reserveHumanResolutionGrant(paths, requestedGrantId) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        ...
        ensurePrivateInvestigationDirectory(paths, path.dirname(reserved), humanResolutionGrantUnsafe);
...
export function readAvailableHumanResolutionGrant(paths, requestedGrantId) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        const available = path.join(stores.available, `${grantId}.json`);
...
export function readReservedHumanResolutionGrant(paths, requestedGrantId) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
...
export function terminalizeHumanResolutionGrant(paths, requestedGrantId, record) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const stores = humanResolutionPaths(paths);
    withHumanResolutionGrantStoreLock(paths, stores, () => {
        const reserved = path.join(stores.reserved, `${grantId}.json`);
        const terminal = path.join(stores.terminal, `${grantId}.json`);
        if (privatePathExists(paths, terminal, humanResolutionGrantUnsafe)) {
            const existing = readPrivateCanonicalJson(paths, terminal, humanResolutionGrantUnsafe);
            if (canonicalJson(existing) !== canonicalJson(record)) {
                throw humanResolutionGrantUnavailable(grantId);
            }
            reconcileTerminalHumanResolutionGrantResidual(paths, stores, grantId, existing);
            return;
        }
        if (!privatePathExists(paths, reserved, humanResolutionGrantUnsafe)) {
            throw humanResolutionGrantUnavailable(grantId);
        }
        createPrivateCanonicalJson(paths, terminal, record, humanResolutionGrantUnsafe, 'HUMAN_RESOLUTION_GRANT_TERMINAL_CONFLICT');
        fs.unlinkSync(reserved);
        fsyncDirectory(path.dirname(reserved));
    }, { targetGrantId: grantId });
}
function reconcileTerminalHumanResolutionGrantResidual(paths, stores, grantId, terminal) {
    assertTerminalHumanResolutionGrant(terminal, grantId);
    const residuals = [stores.available, stores.reserved]
        .map((directory) => ({
        directory,
        state: directory === stores.available
            ? 'available'
            : 'reserved',
        filePath: path.join(directory, `${grantId}.json`),
    }))
        .filter(({ filePath }) => privatePathExists(paths, filePath, humanResolutionGrantUnsafe));
    ...
    assertTerminalHumanResolutionGrantResidual(paths, grantId, terminal, residual.state, residual.filePath);
    fs.unlinkSync(residual.filePath);
    fsyncDirectory(residual.directory);
}
export function readTerminalHumanResolutionGrant(paths, requestedGrantId) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const stores = humanResolutionPaths(paths);
    return withHumanResolutionGrantStoreLock(paths, stores, () => {
        const terminal = path.join(stores.terminal, `${grantId}.json`);
        ...
        const value = readPrivateCanonicalJson(paths, terminal, humanResolutionGrantUnsafe);
        if (!isRecord(value)) {
            throw humanResolutionGrantUnsafe();
        }
...
function inspectStoredHumanResolutionGrant(paths, grantId) {
    const stores = humanResolutionPaths(paths);
    const candidates = [
        { state: 'available', directory: stores.available },
        { state: 'reserved', directory: stores.reserved },
        { state: 'terminal', directory: stores.terminal },
    ].filter(({ directory }) => privatePathExists(paths, path.join(directory, `${grantId}.json`), humanResolutionGrantUnsafe));
    if (candidates.length === 0) {
        return null;
    }
    const terminalCandidate = candidates.find(({ state }) => state === 'terminal');
    if (terminalCandidate) {
        const terminal = readPrivateCanonicalJson(paths, path.join(terminalCandidate.directory, `${grantId}.json`), humanResolutionGrantUnsafe);
        ...
        if (residual) {
            ...
            assertTerminalHumanResolutionGrantResidual(paths, grantId, terminal, residual.state, path.join(residual.directory, `${grantId}.json`));
        }
...
function assertTerminalHumanResolutionGrant(terminal, grantId) {
    const hasRevocationAuthorization = isRecord(terminal) &&
        Object.prototype.hasOwnProperty.call(terminal, 'revocationAuthorization');
    const terminalState = isRecord(terminal) &&
        (terminal.state === 'revoked' || terminal.state === 'consumed')
        ? terminal.state
        : null;
    const expectedKeys = terminalState === 'revoked'
        ? [
            'schemaVersion',
            'state',
            'grantId',
    ...
    assertCanonicalResolutionEnvelopeBytes(envelopeBytes);
    let revocationAuthorization = null;
    if (terminalState === 'revoked' && hasRevocationAuthorization) {
        revocationAuthorization = assertHumanRevocationAuthorization(terminal.revocationAuthorization);
        if (revocationAuthorization.payload.subjectKind !==
    ...
    return deepFreeze({
        ...
        state: terminalState,
        envelopeBytes,
        terminalReason: terminalState === 'revoked' ? String(terminal.reason) : null,
        recordedAt: terminal.recordedAt,
        ...(revocationAuthorization ? { revocationAuthorization } : {}),
    });
}
function assertTerminalHumanResolutionGrantResidual(paths, grantId, terminal, residualState, residualPath) {
    const terminalEntry = assertTerminalHumanResolutionGrant(terminal, grantId);
    if (terminalEntry.state === 'consumed' && residualState !== 'reserved') {
        throw humanResolutionGrantUnsafe();
    }
    if (readPrivateFile(residualPath, humanResolutionGrantUnsafe) !==
        terminalEntry.envelopeBytes) {
        throw humanResolutionGrantUnsafe();
    }
}
export function writeHumanResolutionJournal(paths, journal) {
    ...
    if (validated.phase === 'grant-consumed') {
        ...
        createPrivateCanonicalJson(paths, historicalPath, validated, humanResolutionJournalUnsafe, 'HUMAN_RESOLUTION_JOURNAL_COLLISION');
    ...
    if (active === null) {
        ...
        createPrivateCanonicalJson(paths, activePath, validated, humanResolutionJournalUnsafe, 'HUMAN_RESOLUTION_ACTIVE_CONFLICT');
        return;
    }
    if (active.journalId !== validated.journalId ||
        active.grantId !== validated.grantId) {
        throw workflowError('HUMAN_RESOLUTION_RECOVERY_REQUIRED', 'Another human-resolution transaction must be recovered first.', ExitCode.conflict);
    }
    if (humanResolutionJournalPhaseIndex(validated.phase) <
        humanResolutionJournalPhaseIndex(active.phase)) {
        throw humanResolutionRecoveryAmbiguous();
    }
    writePrivateCanonicalJsonAtomic(paths, activePath, validated, humanResolutionJournalUnsafe);
}
function humanResolutionJournalPhaseIndex(phase) {
    return [
        'prepared',
...
export function readActiveHumanResolutionJournal(paths, requestedChangeId) {
    const changeId = assertChangeId(requestedChangeId);
...
function readHistoricalHumanResolutionJournal(paths, grantId) {
...
export function assertHumanResolutionLifecycleBarrier(runtimeRoot, allowedGrantId = null, allowedChangeId = null) {
    ...
    throw workflowError('HUMAN_RESOLUTION_RECOVERY_REQUIRED', 'An active human-resolution transaction blocks repository lifecycle changes.', ExitCode.conflict);
}
export function reclaimHumanResolutionJournalTemporaries(runtimeRoot, assertRepositoryLifecycleOwned) {
...
function investigationPathsFromLifecycleRoot(runtimeRoot) {
...
function scanActiveHumanResolutionJournalDirectory(paths) {
    const activeDirectory = humanResolutionPaths(paths).active;
    if (!walkPrivateDirectory(paths, activeDirectory, humanResolutionJournalUnsafe, false)) {
        return null;
    }
    const journalNames = [];
...
export function writeHumanResolutionArchive(paths, currentStateDigest, state) {
    if (!isDigest(currentStateDigest) ||
    ...
    createPrivateCanonicalJson(paths, path.join(humanResolutionPaths(paths).archives, `${archiveDigest}.json`), archive, humanResolutionArchiveUnsafe, 'HUMAN_RESOLUTION_ARCHIVE_COLLISION');
    return archiveDigest;
}
export function readHumanResolutionArchive(paths, requestedArchiveDigest) {
    if (!isDigest(requestedArchiveDigest)) {
        throw humanResolutionArchiveUnsafe();
    }
    const value = readPrivateCanonicalJson(paths, path.join(humanResolutionPaths(paths).archives, `${requestedArchiveDigest}.json`), humanResolutionArchiveUnsafe);
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
    ...
    if (archiveDigest !== requestedArchiveDigest ||
...
export function writeHumanResolutionReceipt(paths, requestedGrantId, receipt) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    const digest = sha256(canonicalJson(receipt));
    createPrivateCanonicalJson(paths, path.join(humanResolutionPaths(paths).receipts, `${grantId}.json`), receipt, humanResolutionReceiptUnsafe, 'HUMAN_RESOLUTION_RECEIPT_COLLISION');
    return digest;
}
function assertInvestigationSession(value) {
    const hasSemanticRevision = isRecord(value) &&
        Object.prototype.hasOwnProperty.call(value, 'semanticRevision');
    const hasLifecycleRevision = isRecord(value) &&
    ...
    if (session.state !== deriveInvestigationSessionState(session) ||
        !milestonesBelongToSession(session)) {
        throw sessionInvalid();
    }
    return session;
}
function milestonesBelongToSession(session) {
...
function assertMonotonicSessionTransition(current, next) {
    for (const key of [
        'schemaVersion',
        'investigationId',
        'changeId',
        ...
        if (canonicalJson(current[key] ?? null) !== canonicalJson(next[key] ?? null)) {
            throw sessionTransitionInvalid();
        }
    }
    const blockerEntered = current.blocker === null &&
        next.blocker !== null &&
        !('code' in next.blocker) &&
        next.blocker.state === 'human-action-required';
    const blockerCleared = current.blocker !== null &&
    ...
    if (next.revision !== current.revision + 1 ||
        ...
        current.blindInvocationIds.some((invocationId, index) => next.blindInvocationIds[index] !== invocationId)) {
        throw sessionTransitionInvalid();
    }
    if (next.currentBlindInvocationId === current.currentBlindInvocationId &&
    ...
    const reviewerReopen = current.milestones.reviewerTermSourceNodeId !==
    ...
    for (const key of ['groupDispositions', 'whyAnswers']) {
        const before = current.milestones[key];
        const after = next.milestones[key];
        if (before !== null &&
...
function isMilestones(value) {
    ...
    if (value.groupDispositions !== null &&
...
function isStoredCheckpoint(value, kind) {
    if (value === null) {
        return true;
    }
    if (!isRecord(value) ||
...
function isBlocker(value) {
    ...
    const expected = advertisedHumanResolutions(value);
    return (canonicalJson(value.availableResolutions) === canonicalJson(expected) &&
        Buffer.byteLength(canonicalJson(value), 'utf8') <= 65_536);
}
function assertMainTermsPayload(value) {
    if (!isRecord(value) ||
...
function assertWhyAnswersPayload(value) {
    if (!isRecord(value) ||
...
function writePrivateTemporary(filePath, content) {
...
function readPrivateFile(filePath, makeError) {
    return readPrivateBuffer(filePath, makeError).toString('utf8');
}
function readPrivateBuffer(filePath, makeError) {
...
function walkPrivateDirectory(paths, directory, makeError, create) {
...
function releasePrivateRuntimeLock(lockPath, descriptor, owned, marker, invalidLock) {
...
function reclaimDeadPrivateRuntimeLock(lockPath, invalidLock) {
...
function assertPrivateFile(stats, makeError) {
    if (!stats.isFile() ||
        stats.isSymbolicLink() ||
        stats.nlink !== 1 ||
...
function investigationSessionPath(paths, investigationId) {
    return path.join(paths.sessions, `${investigationId}.json`);
}
function currentInvestigationRefPath(paths, changeId) {
    return path.join(paths.refs, `${changeId}.investigation-session.json`);
}
function investigationStartReservationPath(paths, changeId) {
    return path.join(paths.refs, `${changeId}.investigation-start.json`);
}
function humanResolutionPaths(paths) {
...
function humanResolutionRevocationAuthorizationPath(paths, requestedGrantId) {
    const grantId = assertHumanResolutionGrantId(requestedGrantId);
    return path.join(humanResolutionPaths(paths).root, 'grants', 'revocation-authorizations', `${grantId}.json`);
}
function humanResolutionRevocationBinding(paths, envelopeBytes, grantId) {
    let envelope;
    try {
    ...
    if (session.changeId !== payload.target.changeId ||
        (session.mandateBinding !== undefined &&
            session.mandateBinding.changeId !== payload.target.changeId)) {
        throw workflowError('HUMAN_REVOCATION_BINDING_INVALID', 'Human-resolution revocation session binding is unavailable or different.', ExitCode.guard);
    }
    return {
        subjectKind: 'human-resolution-grant',
        grantId,
...
function humanResolutionGrantPublicationRecoveryPath(stores, grantId) {
    return path.join(stores.publicationRecoveries, `${grantId}.json`);
}
function readHumanResolutionGrantPublicationRecovery(paths, stores, grantId) {
    const receiptPath = humanResolutionGrantPublicationRecoveryPath(stores, grantId);
    if (!privatePathExists(paths, receiptPath, humanResolutionGrantUnsafe)) {
        return null;
    }
    return assertHumanResolutionGrantPublicationQuarantineReceipt(readPrivateCanonicalJson(paths, receiptPath, humanResolutionGrantUnsafe), grantId);
}
function assertNoPreparedHumanResolutionGrantPublicationRecovery(paths, grantId) {
    const stores = humanResolutionPaths(paths);
    withHumanResolutionGrantStoreLock(paths, stores, () => {
        const receipt = readHumanResolutionGrantPublicationRecovery(paths, stores, grantId);
        if (receipt?.phase === 'prepared') {
...
function withHumanResolutionGrantStoreLock(paths, stores, operation, options = {}) {
    return withPrivateRuntimeLock(paths, path.join(stores.locks, 'grant-store.lock'), () => {
        reclaimHumanResolutionGrantTemporaries(paths, stores, options.allowInterruptedAvailablePublication ?? false, options.targetGrantId);
        if (options.targetGrantId !== undefined &&
            !options.allowPreparedPublicationRecovery &&
            readHumanResolutionGrantPublicationRecovery(paths, stores, options.targetGrantId)?.phase === 'prepared') {
            throw humanResolutionGrantPublicationRecoveryRequired();
        }
        return operation();
    }, 'HUMAN_RESOLUTION_GRANT_STORE_CONFLICT', humanResolutionGrantUnsafe);
}
function reclaimHumanResolutionGrantTemporaries(paths, stores, allowInterruptedAvailablePublication, targetGrantId) {
    for (const directory of [
        stores.available,
        stores.terminal,
        stores.publicationRecoveries,
    ]) {
        if (!walkPrivateDirectory(paths, directory, humanResolutionGrantUnsafe, false)) {
            continue;
        }
        let removed = false;
        for (const name of fs.readdirSync(directory)) {
            ...
            if (directory === stores.available) {
                if (allowInterruptedAvailablePublication ||
...
function listHumanResolutionGrantIds(paths, directory) {
    if (!walkPrivateDirectory(paths, directory, humanResolutionGrantUnsafe, false)) {
        return [];
    }
...
function listHumanResolutionGrantPublicationRecoveryIds(paths, stores) {
    if (!walkPrivateDirectory(paths, stores.publicationRecoveries, humanResolutionGrantUnsafe, false)) {
        return [];
    }
    ...
    return names.map((name) => {
        ...
        readHumanResolutionGrantPublicationRecovery(paths, stores, validatedGrantId);
        return validatedGrantId;
    });
}
function humanResolutionGrantPublicationRecoveryRequired() {
    return workflowError('HUMAN_RESOLUTION_GRANT_PUBLICATION_RECOVERY_REQUIRED', 'An interrupted human resolution grant publication requires explicit reconciliation through maintainer resolution-inspect and resolution-publication-discard.', ExitCode.unsafeEnvironment);
}
function observeHumanResolutionGrantPublicationStoreState(paths, stores, grantId) {
    for (const directory of [
        stores.available,
        stores.reserved,
        stores.terminal,
    ...
    const temporaries = walkPrivateDirectory(paths, stores.available, humanResolutionGrantUnsafe, false)
        ? fs
            ...
            const { content, ...observed } = observePrivateGrantPublicationFile(path.join(stores.available, temporaryName));
            return [
                {
                    temporaryName,
                    ...observed,
                    parsedEnvelopeGrantId: parseInterruptedHumanResolutionGrantEnvelopeId(content),
                },
            ];
        })
            .sort((left, right) => left.temporaryName.localeCompare(right.temporaryName))
        : [];
    const digestIfPresent = (filePath) => privatePathExists(paths, filePath, humanResolutionGrantUnsafe)
    ...
    return {
        ...
        temporaries,
        durable: {
            availableDigest: digestIfPresent(path.join(stores.available, `${grantId}.json`)),
            reservedDigest: digestIfPresent(path.join(stores.reserved, `${grantId}.json`)),
            terminalDigest: digestIfPresent(path.join(stores.terminal, `${grantId}.json`)),
        },
        sameGrantJournalDigest: digestIfPresent(humanResolutionJournalPath(paths, grantId)),
        sameGrantActiveJournalDigest: observeSameGrantActiveJournalDigest(paths, grantId),
        sameGrantReceiptDigest: digestIfPresent(path.join(stores.receipts, `${grantId}.json`)),
    };
}
function observeSameGrantActiveJournalDigest(paths, grantId) {
    const scanned = scanActiveHumanResolutionJournalDirectory(paths);
    if (scanned === null) {
        return null;
    }
    ...
    for (const name of scanned.journalNames) {
        ...
        match = observePrivateGrantPublicationFile(path.join(scanned.activeDirectory, name)).rawSha256;
    }
    return match;
}
function observePrivateGrantPublicationFile(filePath) {
    const before = fs.lstatSync(filePath, { throwIfNoEntry: false });
    if (!before) {
        throw humanResolutionGrantUnsafe();
    }
    ...
    return {
        rawSha256,
        unsafeObservationDigest: sha256(canonicalJson({
...
function parseInterruptedHumanResolutionGrantEnvelopeId(content) {
    try {
...
function completeHumanResolutionGrantPublicationQuarantine(paths, stores, receipt) {
    assertHumanResolutionGrantPublicationQuarantineReplayState(paths, stores, receipt);
    for (const artifact of receipt.artifacts) {
        const temporaryPath = path.join(stores.available, artifact.temporaryName);
        const quarantinePath = path.join(stores.quarantine, artifact.quarantineArtifact);
...
function finalizeHumanResolutionGrantPublicationQuarantineReceipt(paths, receiptPath, receipt) {
    if (receipt.phase === 'quarantined') {
        return;
    }
    writePrivateCanonicalJsonAtomic(paths, receiptPath, {
        ...receipt,
        phase: 'quarantined',
    }, humanResolutionGrantUnsafe);
}
function assertHumanResolutionGrantPublicationQuarantineReceipt(value, grantId) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
    ...
    const publicationStateDigest = value.publicationStateDigest;
    if (recoveryId !==
        sha256(canonicalJson({
            schema: 'human-resolution-grant-publication-quarantine.v1',
            grantId,
    ...
    const publicationStoreState = assertHumanResolutionGrantPublicationStoreState(value.publicationStoreState, grantId);
    const auditTag = assertHumanResolutionGrantPublicationAuditTag(value.auditTag, grantId);
    ...
    const artifacts = value.artifacts.map((artifact, index) => assertHumanResolutionGrantPublicationQuarantineArtifact(artifact, grantId, recoveryId, index, publicationStoreState.temporaries[index]));
    if (artifacts.some((artifact, index) => index > 0 &&
        artifact.temporaryName <= artifacts[index - 1].temporaryName)) {
        throw humanResolutionGrantPublicationRecoveryStale();
    }
    return value;
}
function assertHumanResolutionGrantPublicationAuditTag(value, grantId) {
    if (!isRecord(value) ||
...
function assertHumanResolutionGrantPublicationQuarantineArtifact(value, grantId, recoveryId, index, temporary) {
    if (!temporary ||
        !isRecord(value) ||
        !hasExactKeys(value, [
            'temporaryName',
...
function assertHumanResolutionGrantPublicationStoreState(value, grantId) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'grantId',
            'temporaries',
    ...
    const temporaries = value.temporaries.map((temporary) => assertHumanResolutionGrantPublicationTemporary(temporary, grantId));
    if (temporaries.length === 0 ||
        temporaries.some((temporary, index) => index > 0 &&
            temporary.temporaryName <= temporaries[index - 1].temporaryName)) {
        throw humanResolutionGrantPublicationRecoveryStale();
    }
    return value;
}
function assertHumanResolutionGrantPublicationTemporary(value, grantId) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'temporaryName',
            'rawSha256',
        ...
        (value.parsedEnvelopeGrantId !== null &&
            (typeof value.parsedEnvelopeGrantId !== 'string' ||
                HUMAN_RESOLUTION_GRANT_FILE.exec(`${value.parsedEnvelopeGrantId}.json`)?.[1] !== value.parsedEnvelopeGrantId))) {
        throw humanResolutionGrantPublicationRecoveryStale();
    }
    return value;
}
function assertHumanResolutionGrantPublicationQuarantineReplayState(paths, stores, receipt) {
    const observed = observeHumanResolutionGrantPublicationStoreState(paths, stores, receipt.grantId);
    if (canonicalJson({
        ...
        canonicalJson({
            ...
            sameGrantJournalDigest: receipt.publicationStoreState.sameGrantJournalDigest,
            sameGrantActiveJournalDigest: receipt.publicationStoreState.sameGrantActiveJournalDigest,
            sameGrantReceiptDigest: receipt.publicationStoreState.sameGrantReceiptDigest,
        })) {
        throw humanResolutionGrantPublicationRecoveryStale();
    }
    const remainingTemporaries = new Map(observed.temporaries.map((temporary) => [
    ...
    for (const artifact of receipt.artifacts) {
        const temporaryPath = path.join(stores.available, artifact.temporaryName);
        const quarantinePath = path.join(stores.quarantine, artifact.quarantineArtifact);
        const temporaryExists = privatePathExists(paths, temporaryPath, humanResolutionGrantUnsafe);
        const quarantineExists = privatePathExists(paths, quarantinePath, humanResolutionGrantUnsafe);
        if (temporaryExists === quarantineExists) {
            throw humanResolutionGrantPublicationRecoveryStale();
        }
        const observedArtifact = observePrivateGrantPublicationFile(temporaryExists ? temporaryPath : quarantinePath);
        if (observedArtifact.rawSha256 !== artifact.rawSha256 ||
...
function humanResolutionGrantPublicationRecoveryInvalid() {
    return workflowError('HUMAN_RESOLUTION_GRANT_PUBLICATION_RECOVERY_INVALID', 'Interrupted human resolution grant publication recovery input is malformed.', ExitCode.guard);
}
function humanResolutionGrantPublicationRecoveryStale() {
    return workflowError('HUMAN_RESOLUTION_GRANT_PUBLICATION_RECOVERY_STALE', 'Interrupted human resolution grant publication no longer matches the inspected bytes.', ExitCode.staleState);
}
function humanResolutionGrantPublicationRecoveryAmbiguous() {
    return workflowError('HUMAN_RESOLUTION_GRANT_PUBLICATION_RECOVERY_AMBIGUOUS', 'Interrupted human resolution grant publication state is not a unique temp-only prefix.', ExitCode.unsafeEnvironment);
}
function humanResolutionNodePath(paths, nodeId) {
    return path.join(humanResolutionPaths(paths).nodes, `${nodeId}.json`);
}
function humanResolutionRefPath(paths, investigationId) {
    return path.join(humanResolutionPaths(paths).refs, `${investigationId}.json`);
}
function humanResolutionJournalPath(paths, grantId) {
    return path.join(humanResolutionPaths(paths).journals, `${grantId}.json`);
}
function activeHumanResolutionJournalPath(paths, changeId) {
    return path.join(humanResolutionPaths(paths).active, `${changeId}.json`);
}
function assertHumanResolutionTarget(value) {
    if (!isRecord(value) ||
...
function assertHumanResolutionExpected(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'reasonCode',
            'blockedTransition',
        ...
        throw humanResolutionInvalid('Human resolution expected-state binding is malformed.');
    }
    return deepFreeze(structuredClone(value));
}
function assertHumanResolutionNode(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
    ...
    if (candidate.nodeId !== value.nodeId ||
        canonicalJson(candidate) !== canonicalJson(value)) {
        throw humanResolutionObjectUnsafe();
    }
    return candidate;
}
function assertHumanResolutionJournal(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
        ...
        !['preserve', 'quarantine-whole'].includes(String(value.resolutionRefMode)) ||
    ...
    if (value.plannedCurrentWorkflowRef.expectedDigest !==
        investigationCurrentRefDigest(value.plannedCurrentWorkflowRef.expectedInvestigationId === null
            ? null
            : {
                changeId: target.changeId,
                investigationId: value.plannedCurrentWorkflowRef.expectedInvestigationId,
            }) ||
        value.plannedCurrentWorkflowRef.nextDigest !==
            investigationCurrentRefDigest(value.plannedCurrentWorkflowRef.nextInvestigationId === null
                ? null
                : {
                    changeId: target.changeId,
                    investigationId: value.plannedCurrentWorkflowRef.nextInvestigationId,
                }) ||
    ...
    const semantic = {
        ...
        resolutionRefMode: value.resolutionRefMode,
        plannedResolutionNodeId: value.plannedResolutionNodeId,
        plannedCurrentWorkflowRef: value.plannedCurrentWorkflowRef,
        plannedStartReservation: value.plannedStartReservation,
        plannedEvidenceRefs: value.plannedEvidenceRefs,
        evidenceArchiveDigest: value.evidenceArchiveDigest,
        receiptDigest: value.receiptDigest,
        createdAt: value.createdAt,
    };
    const journalId = sha256(canonicalJson({
        schema: 'human-resolution-journal.v2',
        journal: humanResolutionJournalIdentity(semantic),
    }));
...
function isEvidenceRefMap(value) {
    return (isRecord(value) &&
        Object.entries(value).every(([name, nodeId]) => /^[a-z0-9]+(?:-[a-z0-9]+)*(?:\/[a-z0-9]+(?:-[a-z0-9]+)*)*$/.test(name) && isDigest(nodeId)));
}
function isPlannedStartReservation(value) {
    return (isRecord(value) &&
        hasExactKeys(value, [
            'mode',
            'expectedDigest',
...
function validPlannedStartReservation(value) {
...
function isPlannedEvidenceRefs(value) {
    return (isRecord(value) &&
        hasExactKeys(value, [
            'mode',
            'expectedDigest',
...
function validPlannedEvidenceRefs(changeId, value) {
    if (Object.keys(value.retiredRefs).some((name) => Object.hasOwn(value.retainedRefs, name))) {
        return false;
    }
    if (value.mode === 'preserve') {
        return (value.nextDigest === value.expectedDigest &&
    ...
    if (value.mode === 'quarantine-whole') {
        return (value.expectedDigest !== null &&
            value.nextDigest === null &&
    ...
    const nextDigest = Object.keys(value.retainedRefs).length === 0
        ? null
        : sha256(canonicalJson({
            schemaVersion: 1,
            changeId,
...
function humanResolutionJournalIdentity(journal) {
    const { phase: _phase, ...identity } = journal;
    return identity;
}
function assertDecisionConsequences(decision, consequences) {
    if (['supersede', 'repair'].includes(decision.kind) &&
        consequences.continuity !== 'broken') {
        throw humanResolutionInvalid('Supersede and repair decisions must declare broken continuity.');
    }
    if (decision.kind === 'abort' && consequences.continuity === 'preserved') {
        throw humanResolutionInvalid('Abort cannot claim preserved workflow continuity.');
    }
    if (decision.kind === 'waive-assurance' &&
        (consequences.assurance !== 'human-waived' ||
            !consequences.claimsWaived.includes(decision.claim))) {
        throw humanResolutionInvalid('An assurance waiver must name and downgrade the waived claim.');
    }
    if (decision.kind === 'close-input' &&
        consequences.assurance === 'unchanged') {
        throw humanResolutionInvalid('Closing reviewer input must declare degraded or waived assurance.');
    }
}
function effectiveHumanResolutionState(node, fallback) {
...
function terminalHumanResolutionState(value) {
    return (value === 'aborted-by-human-resolution' ||
        value === 'superseded-by-human-resolution' ||
        value === 'quarantined-by-human-resolution');
}
function bindProviderRetryReservationResolutionState(paths, session, makeError) {
    ...
    for (const name of fs.readdirSync(paths.refs).sort()) {
        ...
        const status = attempt <= session.blindInvocationIds.length
            ? 'committed'
            : 'pending';
        if (attempt > session.blindInvocationIds.length + 1) {
            throw makeError();
        }
        byAttempt.set(attempt, readBoundProviderRetryReservation(paths, path.join(paths.refs, name), session, attempt, status, makeError));
    }
    for (let attempt = 2; attempt <= session.blindInvocationIds.length; attempt += 1) {
        if (!byAttempt.has(attempt)) {
            throw makeError();
        }
    }
    const bindings = [...byAttempt.values()].sort((left, right) => left.attempt - right.attempt);
    return {
        reservations: bindings.map(({ attempt, previousInvocationId, invocationId, reservationDigest, status, }) => ({
            attempt,
...
function readBoundProviderRetryReservation(paths, filePath, session, expectedAttempt, status, makeError) {
    walkPrivateDirectory(paths, path.dirname(filePath), makeError, false);
    const content = readPrivateFile(filePath, makeError);
    ...
    const expectedPrevious = session.blindInvocationIds[expectedAttempt - 2] ?? null;
    const expectedCommitted = status === 'committed'
    ...
    if (retryV2) {
        if (!isRecord(value.executionPolicySnapshot) ||
...
function observeProviderRetryReservationAmbiguities(paths, session, observations) {
    const prefix = `${session.investigationId}.provider-retry-`;
    let names;
    try {
        assertPrivateInvestigationDirectory(paths, paths.refs, providerInvocationUnsafe);
        names = fs
            .readdirSync(paths.refs)
            .filter((name) => name.startsWith(prefix));
    }
    catch {
        observeUnsafePath(paths.refs, `provider-retry-reservations:${session.investigationId}`, observations);
        return;
    }
    const targets = new Set(names.map((name) => path.join(paths.refs, name)));
    for (let attempt = 2; attempt <= session.blindInvocationIds.length; attempt += 1) {
        targets.add(path.join(paths.refs, `${prefix}${attempt}.json`));
    }
    for (const target of [...targets].sort()) {
        observeUnsafePath(target, `provider-retry-reservation:${path.basename(target)}`, observations);
    }
}
function bindProviderInvocationResolutionState(paths, session, retryBindings) {
    const scan = scanProviderInvocationLifecycles(paths);
    const projections = [...scan.projections];
    const pendingRetry = retryBindings.find(({ status }) => status === 'pending') ?? null;
    const allowedReservedSnapshotIds = new Set();
    for (const { invocationId } of scan.unsafeInvocations) {
        ...
        else if (isDurablyReservedProviderExecutionPolicySnapshot(paths, invocationId)) {
            allowedReservedSnapshotIds.add(invocationId);
        }
    }
    const unsafeInvocations = scan.unsafeInvocations.filter(({ invocationId }) => !allowedReservedSnapshotIds.has(invocationId));
    const target = projections.filter((projection) => projection.ownerInvestigationId === session.investigationId &&
        projection.changeId === session.changeId);
    if (unsafeInvocations.some(({ invocationId, ownerInvestigationId }) => ownerInvestigationId === session.investigationId ||
        invocationId === pendingRetry?.invocationId)) {
        throw providerInvocationUnsafe();
    }
    const globalPendingProjection = pendingRetry === null
        ? undefined
        : projections.find(({ invocationId }) => invocationId === pendingRetry.invocationId);
    if (globalPendingProjection !== undefined &&
    ...
    const targetSurveys = target.filter((projection) => projection.purpose === 'survey');
    ...
    if (canonicalJson(targetSurveyIds) !== canonicalJson(expectedSurveyIds.sort())) {
        throw providerInvocationUnsafe();
    }
    const boundNonces = session.blindInvocationIds.map((invocationId) => {
        const projection = targetSurveys.find((candidate) => candidate.invocationId === invocationId);
    ...
    const retryByAttempt = new Map(retryBindings.map((binding) => [binding.attempt, binding]));
    for (const [index, invocationId] of session.blindInvocationIds.entries()) {
        ...
        const previous = targetSurveys.find((candidate) => candidate.invocationId === session.blindInvocationIds[index - 1]);
        if (retry === undefined ||
    ...
    if (pendingRetry !== null) {
        const previous = targetSurveys.find((candidate) => candidate.invocationId === pendingRetry.previousInvocationId);
        const pendingProjection = targetSurveys.find((candidate) => candidate.invocationId === pendingRetry.invocationId);
    ...
    return {
        ...
        repositoryProviderLeases: projections
            .filter((projection) => projection.state === 'leased' && projection.lease !== null)
            .map((projection) => ({
...
function assertPendingProviderExecutionPolicySnapshot(paths, pending) {
...
function isDurablyReservedProviderExecutionPolicySnapshot(paths, invocationId) {
    const directory = path.join(paths.invocations, invocationId);
    try {
        if (canonicalJson(fs.readdirSync(directory).sort()) !==
            canonicalJson(['execution-policy.json'])) {
            return false;
        }
        for (const name of fs.readdirSync(paths.refs).sort()) {
            if (!name.endsWith('.investigation-start.json') &&
                !/\.provider-retry-[1-9][0-9]*\.json$/.test(name)) {
                continue;
            }
            const value = readPrivateCanonicalJson(paths, path.join(paths.refs, name), providerInvocationUnsafe);
            if (!isRecord(value) || !isRecord(value.request)) {
                continue;
            }
            const retryV2 = value.kind === 'provider-retry-reservation' &&
                value.schemaVersion === 2;
            const expectedKeys = value.kind === 'investigation-start-reservation'
                ? [
                    'baseline',
                    'branch',
                    'changeId',
        ...
        for (const name of fs.readdirSync(paths.refs).sort()) {
            ...
            if (node.type !== 'plan-review-request-reservation' ||
...
function digestPrivateDirectoryEntries(paths, directory, makeError) {
    if (!walkPrivateDirectory(paths, directory, makeError, false)) {
        throw makeError();
    }
    const manifestContent = readPrivateFile(path.join(directory, 'manifest.json'), makeError);
    let manifest;
    try {
    ...
    const snapshotArtifacts = isRecord(manifest) &&
        manifest.kind === 'plan-review-manifest' &&
    ...
    let request;
    try {
        request = recreateProviderInvocationRequest(JSON.parse(readPrivateFile(path.join(directory, 'request.json'), makeError)));
    }
    catch {
        throw makeError();
    }
    ...
    const stateContent = readPrivateFile(path.join(directory, 'state.json'), makeError);
    const state = JSON.parse(stateContent);
    let retentionReceipt = null;
    try {
        if (isRecord(state) &&
            ...
            retentionReceipt = readCompleteProviderRetentionReceipt(paths, path.basename(directory), {
        ...
        else if (readProviderRetentionReceipt(paths, path.basename(directory)) !== null) {
            throw makeError();
        }
    }
    catch {
    ...
    const runtimeRequired = isRecord(state) &&
        isRecord(state.result) &&
        state.result.runtimeObservation !== null;
    const retainedRuntimeArtifacts = BLIND_PROVIDER_RUNTIME_FILES.map((name) => retentionReceipt === null
        ? null
        : providerRetentionArtifact(retentionReceipt, `runtime/${name}`));
    const receiptCarriesRuntime = retainedRuntimeArtifacts.every((artifact) => artifact !== null);
    if (retainedRuntimeArtifacts.some((artifact) => artifact !== null) &&
    ...
    if (!runtimeStats) {
        ...
        if (receiptCarriesRuntime) {
            for (let index = 0; index < BLIND_PROVIDER_RUNTIME_FILES.length; index += 1) {
                files.push({
        ...
        if (receiptCarriesRuntime
            ? runtimeNames.some((name) => !expectedRuntimeNames.includes(name))
        ...
        for (let index = 0; index < BLIND_PROVIDER_RUNTIME_FILES.length; index += 1) {
            ...
            const content = readPrivateBuffer(path.join(runtimePath, name), makeError);
    ...
    if (snapshotArtifacts !== null) {
        const retainedReviewRoot = retentionReceipt === null
            ? null
            : providerRetentionArtifact(retentionReceipt, 'review-root');
        const expectedRetainedReviewRoot = providerRetentionReviewRootArtifact(snapshotArtifacts.map((artifact) => ({
            name: `review-root/${artifact.snapshotFile}`,
            digest: artifact.sha256,
            bytes: artifact.byteLength,
        })));
        if (retainedReviewRoot !== null &&
        ...
        if (!reviewRootStats?.isDirectory() ||
        ...
        for (const artifact of snapshotArtifacts) {
            ...
            const content = readPrivateBuffer(path.join(reviewRootPath, artifact.snapshotFile), makeError);
...
function planReviewSnapshotArtifacts(value, makeError) {
    ...
    if (new Set(artifacts.map(({ snapshotFile }) => snapshotFile)).size !==
        artifacts.length) {
        throw makeError();
    }
    return artifacts;
}
function snapshotLineCount(content) {
...
function assertEvidenceClosureTargetBinding(paths, targetSession, closure) {
    for (const entry of closure.entries) {
        ...
        if (entry.refName === 'propose/planning-materialization') {
            assertMaterializationMatchesOwnerSession(readEvidenceNode(paths, entry.nodeId), owner);
            continue;
        }
        if (entry.refName === 'propose/plan-review-request') {
            const materialization = entry.dependencies.find((dependency) => dependency.role === 'materialization');
            if (!materialization) {
                throw humanResolutionStateInvalid();
            }
            assertMaterializationMatchesOwnerSession(readEvidenceNode(paths, materialization.nodeId), owner);
            continue;
        }
...
function assertMaterializationMatchesOwnerSession(node, session) {
    const output = node.output;
    const exemption = 'kind' in session && session.kind === 'propose-exemption-session';
    const expectedType = exemption
        ? 'propose-exemption-planning-materialization'
        : 'propose-planning-materialization';
    const semanticReceipt = !exemption &&
    ...
    const revisionMatches = isRecord(output) &&
        (exemption
...
function digestBoundStartReservation(paths, session, makeError) {
    ...
    let request;
    let firstInvocationRequest;
    try {
        request = recreateProviderInvocationRequest(value.request);
        firstInvocationRequest = recreateProviderInvocationRequest(readPrivateCanonicalJson(paths, path.join(paths.invocations, session.blindInvocationIds[0], 'request.json'), makeError));
    }
    catch {
        throw makeError();
    }
    if (request.requestDigest !== value.requestDigest ||
...
function observeUnsafePath(filePath, object, observations) {
...
function normalizeAmbiguityObservations(observations) {
...
function createPrivateRawFile(paths, filePath, content, makeError) {
    ensurePrivateInvestigationDirectory(paths, path.dirname(filePath), makeError);
...
function assertCanonicalResolutionEnvelopeBytes(content) {
...
function assertHumanResolutionGrantId(value) {
...
function exactResolutionTimestamp(value) {
...
function isBoundedResolutionText(value, maxBytes) {
    return (typeof value === 'string' &&
        value.trim() === value &&
        value.length > 0 &&
...
function isSortedUniqueStrings(values) {
    ...
    return (values.length === sorted.length &&
...
function isCheckpointKind(value) {
    return (value === 'main-terms' ||
...
function isProviderInvocationFailureShape(value) {
    const hasExecutionKind = isRecord(value) && Object.hasOwn(value, 'executionKind');
...
function isReadOnlyProbeShape(value) {
    try {
    ...
    try {
        ...
        if (value.schemaVersion === 2 || value.schemaVersion === 3) {
            ...
            const attemptReservation = {
                ...
                providerCostMicros: loaded.policy.retryAccounting.reservations[request.providerId]
                    .providerCostMicros,
                providerTokens: loaded.policy.retryAccounting.reservations[request.providerId]
                    .providerTokens,
            };
            if (canonicalJson(value.retryAccounting) !==
...
function assertProviderRepairAuthorityArtifact(name, value, request, makeError) {
    if (!isRecord(value)) {
        throw makeError();
    }
...
function isProviderInvocationLifecycleLease(value) {
    return (value === null ||
        (isRecord(value) &&
            hasExactKeys(value, [
                'generation',
...
function sessionUnsafe() {
    return workflowError('INVESTIGATION_SESSION_UNSAFE', 'Investigation session storage is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function sessionInvalid() {
    return workflowError('INVESTIGATION_SESSION_INVALID', 'Investigation session is malformed or internally inconsistent.', ExitCode.staleState);
}
function sessionTransitionInvalid() {
    return workflowError('INVESTIGATION_SESSION_TRANSITION_INVALID', 'Investigation session transition is not monotonic.', ExitCode.staleState);
}
function investigationCasMismatch(expected, observed) {
    return workflowError('INVESTIGATION_CAS_MISMATCH', 'Investigation session changed during compare-and-swap.', ExitCode.conflict, { details: { expectedRevision: expected, observedRevision: observed } });
}
...
function humanResolutionInvalid(message) {
    return workflowError('HUMAN_RESOLUTION_INVALID', message, ExitCode.guard);
}
function humanResolutionStateInvalid() {
    return workflowError('HUMAN_RESOLUTION_STATE_UNSAFE', 'The workflow state cannot be uniquely bound for human resolution.', ExitCode.unsafeEnvironment);
}
function humanResolutionRecoveryAmbiguous() {
    return workflowError('HUMAN_RESOLUTION_RECOVERY_AMBIGUOUS', 'Human resolution recovery observed state outside the exact journal.', ExitCode.staleState);
}
function providerInvocationUnsafe() {
    return workflowError('HUMAN_RESOLUTION_PROVIDER_STATE_UNSAFE', 'Referenced provider state is missing, unsafe, or non-canonical.', ExitCode.unsafeEnvironment);
}
function startReservationUnsafe() {
    return workflowError('HUMAN_RESOLUTION_START_RESERVATION_UNSAFE', 'Referenced investigation start reservation is unsafe or non-canonical.', ExitCode.unsafeEnvironment);
}
function humanResolutionObjectUnsafe() {
    return workflowError('HUMAN_RESOLUTION_NODE_UNSAFE', 'Human resolution overlay storage is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function humanResolutionRefUnsafe() {
    return workflowError('HUMAN_RESOLUTION_REF_UNSAFE', 'Human resolution ref storage is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function humanResolutionGrantUnsafe() {
    return workflowError('HUMAN_RESOLUTION_GRANT_UNSAFE', 'Human resolution grant storage is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function humanResolutionGrantUnavailable(grantId) {
    return workflowError('HUMAN_RESOLUTION_GRANT_UNAVAILABLE', `Human resolution grant ${grantId} is unavailable or already terminal.`, ExitCode.conflict);
}
function humanResolutionGrantNotFound(grantId) {
    return workflowError('HUMAN_RESOLUTION_GRANT_NOT_FOUND', `Human resolution grant ${grantId} does not exist.`, ExitCode.conflict);
}
function humanResolutionJournalUnsafe() {
    return workflowError('HUMAN_RESOLUTION_JOURNAL_UNSAFE', 'Human resolution journal is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function humanResolutionArchiveUnsafe() {
    return workflowError('HUMAN_RESOLUTION_ARCHIVE_UNSAFE', 'Human resolution evidence archive is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function humanResolutionReceiptUnsafe() {
    return workflowError('HUMAN_RESOLUTION_RECEIPT_UNSAFE', 'Human resolution receipt is unsafe or malformed.', ExitCode.unsafeEnvironment);
}
function sessionLockInvalid() {
    return workflowError('INVESTIGATION_SESSION_LOCK_INVALID', 'Investigation session lock ownership changed during the transition.', ExitCode.staleState);
}
