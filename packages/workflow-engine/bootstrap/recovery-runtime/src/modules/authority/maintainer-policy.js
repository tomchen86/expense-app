...
export function loadMaintainerPolicy(repositoryRoot) {
    const policyPath = path.join(repositoryRoot, 'workflow/maintainer-policy.json');
    let value;
    try {
...
export function parseMaintainerPolicy(value) {
    if (!isRecord(value) ||
        ...
        !/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+\.git$/.test(value.repository.origin) ||
    ...
    try {
        ...
        throw invalidPolicy('Maintainer policy contains an invalid authority path.');
    }
    ...
    if (sealedImmutablePaths.some((immutablePath) => !bootstrapEligiblePaths.some((eligiblePath) => matchesAllowedPath(immutablePath, eligiblePath)))) {
        throw invalidPolicy('Every sealed immutable path must be bootstrap-eligible.');
    }
    const signers = value.trustedSigners.map(parseSigner);
    assertSortedUnique(signers.map(({ identity }) => identity), 'trusted signer identities');
    if (new Set(signers.map(({ publicKey }) => publicKey)).size !==
        signers.length ||
...
export function isAuthorityPathEligible(policy, filePath) {
    return (policy.bootstrapEligiblePaths.some((eligible) => matchesAllowedPath(filePath, eligible)) &&
        (policy.phase !== 'sealed' ||
            !policy.sealedImmutablePaths.some((immutable) => matchesAllowedPath(filePath, immutable))));
}
function parseSigner(value) {
    if (!isRecord(value) ||
        ...
        throw invalidPolicy('Maintainer policy contains an invalid trusted signer.');
    }
    return {
...
function assertSortedUnique(values, label) {
    ...
    if (values.length !== sorted.length ||
        ...
        throw invalidPolicy(`Maintainer policy ${label} must be sorted and unique.`);
    }
}
...
function invalidPolicy(message, details) {
    return workflowError('MAINTAINER_POLICY_INVALID', message, ExitCode.guard, details ? { details } : {});
}
