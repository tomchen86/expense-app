 ...
 */
export function createPlanTarget(input) {
    ...
    if (typeof input.schemaName !== 'string' ||
    ...
    for (const component of input.components) {
        ...
        if (seenIdentities.has(view.identityDigest)) {
            throw targetInvalid('Plan target contains a duplicate component identity.');
        }
        ...
        if (view.role !== 'requirement-clause' && seenLocations.has(location)) {
            throw targetInvalid('Plan target contains a duplicate component location.');
        }
 ...
 */
export function assertPlanTarget(value) {
    ...
    if (typeof value.schemaName !== 'string' ||
    ...
    const views = value.components.map((component) => assertComponentView(component, value.changeId));
    const roleCounts = new Map();
    const seenIdentities = new Set();
    ...
    for (const view of views) {
        ...
        roleCounts.set(role, (roleCounts.get(role) ?? 0) + 1);
        if (seenIdentities.has(view.identityDigest)) {
            throw targetInvalid('Serialized plan target contains a duplicate component identity.');
        }
        if (seenDigests.has(view.componentDigest)) {
            throw targetInvalid('Serialized plan target contains a duplicate component digest.');
        }
        seenIdentities.add(view.identityDigest);
        seenDigests.add(view.componentDigest);
        const location = componentLocationKey(view);
        if (view.role !== 'requirement-clause' && seenLocations.has(location)) {
            throw targetInvalid('Serialized plan target contains a duplicate component location.');
        }
        seenLocations.add(location);
    }
    assertComponentCardinality(roleCounts);
    ...
    if (planTargetDigest(target) !== target.targetDigest) {
        throw targetInvalid('Serialized plan target digest does not match its content.');
    }
    return deepFreeze(target);
}
function canonicalizeComponent(component, changeId) {
    ...
    const rule = COMPONENT_RULES[role];
    if (component.kind !== rule.kind) {
        throw targetInvalid(`Plan target ${role} component kind is unexpected.`);
    }
    const componentPath = assertComponentPath(role, component.path, changeId);
    ...
    return {
        ...
        path: componentPath,
        identityDigest,
        componentDigest: sha256(canonicalJson({
...
function componentSemantic(component) {
    switch (component.kind) {
        case 'structured-json': {
            ...
            let value;
            try {
                value = canonicalJson(projectStructuredSemantics(JSON.parse(canonicalJson(component.value))));
            }
            catch {
                throw targetInvalid('Structured component value is not canonical JSON.');
            }
        ...
        case 'tasks-markdown': {
            assertExactKeys(component, ['kind', 'role', 'path', 'content']);
            return {
                content: normalizeTaskCompletion(normalizeLf(assertContent(component.content))),
            };
        }
        case 'requirement-clause': {
            ...
            if (!isHeadingIdentity(component.requirement) ||
        ...
        case 'mixed-markdown': {
            ...
            if (!isDenseArray(component.authoredRegions) ||
            ...
            if (typeof projection.renderer !== 'string' ||
            ...
            const sourceNodes = projection.sourceNodes.map((node) => {
                ...
                if (!isDigest(node.nodeId) || !isDigest(node.resultDigest)) {
                    throw targetInvalid('Mixed component source node digest is malformed.');
                }
                if (seenSourceNodes.has(node.nodeId)) {
                    throw targetInvalid('Mixed component contains a duplicate source node identity.');
                }
...
function componentIdentityDigest(component, componentPath) {
    const identity = {
 ...
 */
function projectStructuredSemantics(value) {
...
function assertComponentView(value, changeId) {
    if (!isPlainRecord(value) ||
        !hasExactKeys(value, [
            'kind',
            'role',
    ...
    if (typeof role !== 'string' || !(role in COMPONENT_RULES)) {
        throw targetInvalid('Serialized plan target component role is not enumerated.');
    }
    const rule = COMPONENT_RULES[role];
    const kind = value.kind;
    if (typeof kind !== 'string' || kind !== rule.kind) {
        throw targetInvalid(`Serialized plan target ${role} component kind is unexpected.`);
    }
    const componentPath = assertComponentPath(role, value.path, changeId);
    if (!isDigest(value.identityDigest) || !isDigest(value.componentDigest)) {
        throw targetInvalid('Serialized plan target component digest is malformed.');
    }
    if (role !== 'requirement-clause' && role !== 'policy') {
        ...
        if (value.identityDigest !== expectedIdentity) {
            throw targetInvalid('Serialized plan target component identity does not match its role and path.');
        }
    }
    return {
        kind,
...
function assertComponentPath(role, value, changeId) {
    ...
    if (fixedPath !== undefined && componentPath !== fixedPath) {
        throw targetInvalid(`Plan target ${role} component path is not role-consistent.`);
    }
    if ((role === 'delta-spec' || role === 'requirement-clause') &&
        !isDeltaSpecPath(componentPath, changeRoot)) {
        throw targetInvalid(`Plan target ${role} component path is not role-consistent.`);
    }
    if (role === 'policy' && !componentPath.startsWith('workflow/')) {
        throw targetInvalid('Plan target policy component path is not role-consistent.');
    }
    return componentPath;
}
function assertRepositorySafePath(value) {
    if (typeof value !== 'string' ||
        ...
        throw targetInvalid('Plan target component path must be a canonical repository-relative path.');
    }
    return value;
}
function isDeltaSpecPath(candidate, changeRoot) {
...
function isHeadingIdentity(value) {
...
function normalizeTaskCompletion(content) {
...
function assertComponentCardinality(roleCounts) {
    for (const [role, rule] of Object.entries(COMPONENT_RULES)) {
        const count = roleCounts.get(role) ?? 0;
        if (rule.cardinality === 'one' && count !== 1) {
            throw targetInvalid(`Plan target requires exactly one ${role} component.`);
        }
        if (rule.cardinality === 'many' && count < 1) {
            throw targetInvalid(`Plan target requires at least one ${role} component.`);
        }
    }
}
function assertCanonicalComponentOrder(views) {
    for (let index = 1; index < views.length; index += 1) {
        if (compareComponentViews(views[index - 1], views[index]) >= 0) {
            throw targetInvalid('Serialized plan target components are not canonically ordered.');
        }
    }
}
function componentLocationKey(view) {
    return canonicalJson([view.role, view.path]);
}
function compareComponentViews(left, right) {
    return compareUtf8(canonicalJson([left.role, left.path, left.identityDigest]), canonicalJson([right.role, right.path, right.identityDigest]));
}
function compareSourceNodes(left, right) {
    return compareUtf8(canonicalJson([left.nodeId, left.resultDigest]), canonicalJson([right.nodeId, right.resultDigest]));
}
function compareUtf8(left, right) {
    return Buffer.compare(Buffer.from(left, 'utf8'), Buffer.from(right, 'utf8'));
}
function planTargetDigest(value) {
...
function targetInvalid(message) {
