...
import { ensurePrivateInvestigationDirectory, readPrivateCanonicalJson, withPrivateRuntimeLock, } from './investigation-session-store.js';
...
const REVIEWED_CANDIDATE_REAL_ROOTS = Object.freeze({
 ...
 */
export function createProviderRunnerForTesting(host) {
 ...
 */
export function preflightBuiltInProvider(providerId, options) {
    return createProviderRunner(realProviderRunnerHost()).preflight(providerId, options);
}
/**
 ...
 */
export function runBuiltInProvider(input, options) {
...
function createProviderRunner(host) {
    function preflight(providerId, options) {
        ...
        for (const candidate of candidates) {
            ...
            if (!isRegularExecutable(identity.mode) ||
        ...
        const environment = createProviderExecutionEnvironment(providerId, process.execPath, options.temporaryDirectory, options.sourceEnvironment);
        const probe = (executable, args) => host.runProbe({
            executable,
            args,
        ...
        for (const executable of resolved) {
            ...
            if (!probeSucceeded(versionOutcome) ||
    ...
    function run(input, options) {
        ...
        if (fileDigestAtBaseline(input.repositoryRoot, git.head, 'workflow/config.json') !==
            readPlainFileDigest(path.join(input.repositoryRoot, 'workflow', 'config.json'), configBaselineMismatch)) {
            throw configBaselineMismatch();
        }
        const paths = investigationRuntimePaths(git.gitCommonDirectory, config.runtimeDirectory);
        ...
        if (canonicalDirectoryOrThrow(input.invocationDirectory) !== expectedDirectory) {
        ...
        const manifestValue = readPrivateCanonicalJson(paths, path.join(expectedDirectory, 'manifest.json'), inputManifestMismatch);
        if (sha256(canonicalJson(manifestValue)) !== input.request.inputManifestDigest) {
            throw inputManifestMismatch();
        }
        const expectedReviewSnapshotRoot = path.join(expectedDirectory, 'review-root');
        ...
        const ownerWorkflowId = input.acceptanceBinding?.ownerWorkflowId ??
        ...
        const currentManifestValue = assembleProviderPromptManifest(contextStoreRoot, input.request, manifestValue, ownerWorkflowId);
        const managedPrompt = renderManagedProviderPrompt(input.request, currentManifestValue, reviewSnapshotRoot, readProviderRepairPrompt(paths, input.request));
        ...
        if (input.request.baseCommit !== git.head ||
        ...
        if (sha256(canonicalJson(input.semanticOutputSchema)) !==
        ...
        try {
            ...
            const semanticOutputPath = path.join(runtimeDirectory, 'semantic-output.json');
            const schemaContent = canonicalJson(input.semanticOutputSchema);
            const promptIdentity = createExclusiveRuntimeFile(promptPath, managedPrompt);
            const schemaIdentity = createExclusiveRuntimeFile(schemaPath, schemaContent);
            const semanticOutputIdentity = createExclusiveRuntimeFile(semanticOutputPath, '');
            ...
            const governedRuntimeInputs = [
                ...input.governedRuntimeInputs,
                ...(input.acceptanceBinding === undefined
                    ? []
                    : providerAcceptanceGovernedRuntimeInputs(paths, input.acceptanceBinding)),
                {
            ...
            //    model execution.
            const before = captureGovernedProviderProjection(input.repositoryRoot, governedRuntimeInputs);
            ...
            assertExclusiveRuntimeFile(semanticOutputPath, semanticOutputIdentity, '');
...
const PROVIDER_PURPOSE_INSTRUCTIONS = Object.freeze({
    ...
    'task-diff-review': Object.freeze([
        ...
        'When subject.documentationRequirement.required is true, review the complete changeBaseTree-to-candidateTree change and return exactly one documentationAssessment: updated with every changed documentation path, generated-verified with exact source/generated paths and check evidence, no-impact with a concrete rationale only when no documentation path changed, or needs-changes with exact documentation remediation paths. Omission is rejected. When it is false or absent, omit documentationAssessment.',
...
function fileDigestAtBaseline(repositoryRoot, baseCommit, repositoryPath) {
...
function readPlainFileDigest(filePath, makeError) {
...
function canonicalDirectoryOrThrow(directory) {
    try {
 ...
 */
function isReviewedRealPath(candidatePath, realPath) {
    if (realPath === candidatePath) {
        return true;
    }
    return (REVIEWED_CANDIDATE_REAL_ROOTS[candidatePath] ?? []).some((installRoot) => isWithinDirectory(installRoot, realPath));
}
function advertisesFlags(requiredFlags, helpText) {
    return requiredFlags.every((flag) => helpText.includes(flag));
}
function indicatesAuthenticated(providerId, outcome) {
    ...
    return [text, outcome.stderr.trim()].some((line) => /^logged in(?:\s+using\b.*)?$/i.test(line));
}
...
function enforceProcessOutcome(outcome, timeoutMs) {
    assertProviderProcessSucceeded(outcome, timeoutMs);
}
function enforceRawOutputLimit(outcome, aggregateOutputBytes) {
    const bytes = Buffer.byteLength(outcome.stdout, 'utf8') +
 ...
 */
function readNativeSemanticOutput(providerId, outcome, semanticOutputPath, aggregateOutputBytes) {
...
function parseClaudeNativeOutput(stdout) {
    let parsed;
    try {
        parsed = JSON.parse(stdout);
    }
    catch {
        throw nativeOutputInvalid(nativeOutputRepair('NATIVE_JSON_PARSE_FAILED', 'Provider native output was not valid JSON; return one complete object matching the target schema.'));
    }
    if (!isRecord(parsed) ||
        parsed.type !== 'result' ||
        parsed.subtype !== 'success' ||
        !('structured_output' in parsed)) {
        throw nativeOutputInvalid(nativeOutputRepair('NATIVE_WRAPPER_INVALID', 'Provider native output did not contain the required successful structured-output wrapper; return one complete object matching the target schema.'));
    }
    return parsed.structured_output;
}
function readCodexSemanticFile(semanticOutputPath, consumedBytes, aggregateOutputBytes) {
    ...
    try {
        ...
        let read = 0;
        while (read < size) {
            const bytesRead = fs.readSync(descriptor, buffer, read, size - read, read);
            if (bytesRead <= 0) {
                break;
            }
            read += bytesRead;
...
function parseJsonOrInvalid(content) {
    try {
...
function validateSemanticOutput(input, value) {
    const validator = input.outputValidator;
    if (validator.id !== input.request.outputSchema.id ||
    ...
    if (valid !== true) {
        const validationErrors = collectSchemaValidationErrors(input.semanticOutputSchema, frozen);
        throw nativeOutputInvalid({
                ...
                ? [
                    {
                        ...
                        message: 'Output matched the structural schema but failed the bound semantic validator.',
                    },
                ]
                : validationErrors,
        });
...
function collectSchemaValidationErrors(schema, value) {
    const errors = [];
    const visit = (candidate, current, pointer) => {
        if (errors.length >= 64 || !isRecord(candidate))
            return;
        if (Array.isArray(candidate.enum)) {
            const match = candidate.enum.some((allowed) => canonicalJson(allowed) === canonicalJson(current));
            if (!match) {
                errors.push({
                    path: pointer || '/',
                    code: 'ENUM_MISMATCH',
        ...
        if (candidate.type === 'array') {
            ...
            if (Number.isSafeInteger(candidate.minItems) &&
            ...
            if (Number.isSafeInteger(candidate.maxItems) &&
            ...
            if (candidate.items !== undefined) {
                current.forEach((child, index) => visit(candidate.items, child, appendJsonPointer(pointer, String(index))));
            }
            return;
        }
 ...
 */
function acquireProviderSlot(paths, policyMaxConcurrent, invocationId) {
    return withPrivateRuntimeLock(paths, path.join(paths.locks, 'provider-slot-acquire.lock'), () => acquireProviderSlotUnderGuard(paths, policyMaxConcurrent, invocationId), 'PROVIDER_CONCURRENCY_SLOT_GUARD_CONFLICT', concurrencySlotFailed);
}
function acquireProviderSlotUnderGuard(paths, policyMaxConcurrent, invocationId) {
    const slotsDirectory = path.join(paths.root, 'provider-slots');
    ensurePrivateInvestigationDirectory(paths, slotsDirectory, concurrencySlotFailed);
    const limit = Number.isInteger(policyMaxConcurrent) && policyMaxConcurrent > 0
...
function readSlotOwner(slotPath) {
...
function reclaimDeadSlot(slotPath, owner) {
...
function claimProviderSlot(slotPath, invocationId) {
    ...
    let descriptor;
    try {
        descriptor = fs.openSync(slotPath, fs.constants.O_WRONLY |
...
function releaseOwnedSlot(slotPath, owned, marker) {
    const linkStats = fs.lstatSync(slotPath, {
        bigint: true,
        throwIfNoEntry: false,
    });
...
function realProviderRunnerHost() {
    return {
...
function spawnBoundedProcess(input, stdinContent) {
    ...
    const spawnErrorCode = result.error !== undefined && !timedOut
...
function createExclusiveRuntimeDirectory(directory) {
    try {
...
function createExclusiveRuntimeFile(filePath, content) {
    let descriptor;
    try {
...
function assertExclusiveRuntimeFile(filePath, identity, expectedContent) {
...
function openNoFollowRead(filePath) {
    try {
        return fs.openSync(filePath, fs.constants.O_RDONLY | noFollowFlag());
    }
    catch {
        throw nativeOutputInvalid(nativeOutputRepair('NATIVE_OUTPUT_FILE_MISSING_OR_UNSAFE', 'Provider native output file was missing or unsafe; return one complete object matching the target schema.'));
    }
}
...
function isRegularExecutable(mode) {
...
function providerUnavailable(providerId, status) {
    return workflowError('PROVIDER_UNAVAILABLE', `Provider "${providerId}" is not available: ${status}.`, ExitCode.unsafeEnvironment, { details: { status } });
}
...
function durableRequestMismatch() {
    return workflowError('PROVIDER_DURABLE_REQUEST_MISMATCH', 'Launch request does not match the durable canonical invocation request.', ExitCode.staleState);
}
function inputManifestMismatch() {
    return workflowError('PROVIDER_INPUT_MANIFEST_MISMATCH', 'Durable input manifest does not match the request manifest digest.', ExitCode.staleState);
}
function policyMismatch() {
    return workflowError('PROVIDER_POLICY_MISMATCH', 'Provider request is not bound to the current adapter policy or its limits.', ExitCode.guard);
}
function policyBaselineMismatch() {
    return workflowError('PROVIDER_POLICY_BASELINE_MISMATCH', 'Live adapter policy bytes differ from the pinned baseline commit.', ExitCode.staleState);
}
function configBaselineMismatch() {
    return workflowError('PROVIDER_CONFIG_BASELINE_MISMATCH', 'Live workflow configuration bytes differ from the pinned baseline commit.', ExitCode.staleState);
}
function providerDisabled(providerId) {
    return workflowError('PROVIDER_DISABLED', `Provider "${providerId}" is disabled by the adapter policy.`, ExitCode.guard);
}
function baselineMismatch() {
    return workflowError('PROVIDER_BASELINE_MISMATCH', 'Provider request baseline does not match the current repository HEAD/tree.', ExitCode.staleState);
}
function outputSchemaUnbound() {
    return workflowError('PROVIDER_OUTPUT_SCHEMA_UNBOUND', 'Provider semantic output schema does not match the request output schema digest.', ExitCode.guard);
}
function runtimeDirectoryUnsafe(directory) {
    return workflowError('PROVIDER_RUNTIME_DIRECTORY_UNSAFE', 'Provider runtime directory is not the configured investigation invocation directory.', ExitCode.unsafeEnvironment, { details: { directory } });
}
function runtimePathUnsafe(filePath) {
    return workflowError('PROVIDER_RUNTIME_PATH_UNSAFE', 'Provider runtime file is not an exclusively created no-follow single-link regular file.', ExitCode.unsafeEnvironment, { details: { path: filePath } });
}
function concurrencyLimitExceeded() {
    return workflowError('PROVIDER_CONCURRENCY_LIMIT', 'Provider concurrency limit reached for this repository.', ExitCode.conflict);
}
function concurrencySlotFailed() {
    return workflowError('PROVIDER_CONCURRENCY_SLOT_FAILED', 'Unable to acquire a repository-wide provider concurrency slot.', ExitCode.unsafeEnvironment);
}
function executableIdentityDrift() {
    return workflowError('PROVIDER_EXECUTABLE_IDENTITY_DRIFT', 'Provider executable identity changed around the invocation.', ExitCode.unsafeEnvironment);
}
function governedProjectionDrift(changedCategories) {
    return workflowError('PROVIDER_GOVERNED_PROJECTION_DRIFT', 'Governed projection changed during the provider invocation.', ExitCode.verification, { details: { changedCategories } });
}
...
function nativeOutputRepair(reasonCode, message) {
    return {
        repairKind: 'schema',
...
function nativeOutputInvalid(repair = nativeOutputRepair('NATIVE_OUTPUT_INVALID', 'Provider native output was malformed; return one complete object matching the target schema.')) {
    return workflowError('PROVIDER_NATIVE_OUTPUT_INVALID', 'Provider native output was malformed or failed its bound output schema.', ExitCode.verification, { details: { repair } });
}
