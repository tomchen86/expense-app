 ...
 */
export function providerExecutionPolicySnapshot(request) {
    if (request.schemaVersion !== 1 ||
        request.invocationId.length === 0 ||
        request.policyDigest.length === 0) {
        throw executionError('PROVIDER_EXECUTION_REQUEST_INVALID', 'Provider execution policy requires a validated provider request.', ExitCode.guard);
    }
...
export function providerExecutionEnvironmentDigest(request) {
...
export function projectProviderInvocationExecution(input) {
    ...
    if (request.invocationId !== record.invocationId ||
        ...
        throw executionError('PROVIDER_EXECUTION_BINDING_INVALID', 'Provider invocation and request do not project to the same Attempt.', ExitCode.guard);
    }
    const deadline = new Date(Date.parse(record.createdAt) + 14 * 24 * 60 * 60 * 1_000).toISOString();
...
export function createExecutionJob(input) {
    ...
    if (workflow.status !== 'active') {
        throw executionError('EXECUTION_WORKFLOW_NOT_ACTIVE', 'A Job may only be created for an active Workflow.', ExitCode.guard);
    }
    ...
    const createdAt = assertTimestamp(input.createdAt ?? new Date().toISOString());
    if (semanticSpec.contractVersion !== workflow.contractVersion ||
        semanticSpec.stage !== stage ||
        semanticSpec.contextDigest !== workflow.contextDigest) {
        throw executionInvalid('Semantic Job specification is not bound to its Workflow.');
    }
...
export function createReplacementAttempt(input) {
    ...
    const createdAt = assertTimestamp(input.createdAt ?? new Date().toISOString());
    if (workflow.status !== 'active') {
        throw executionError('EXECUTION_WORKFLOW_NOT_ACTIVE', 'A replacement Attempt requires an active Workflow.', ExitCode.guard);
    }
    if (job.workflowId !== workflow.workflowId ||
        ...
        throw executionError('EXECUTION_CONTEXT_STALE', 'The Job or previous Attempt is not in the current Workflow context.', ExitCode.staleState);
    }
    if (job.acceptedAttemptId !== null || job.status === 'succeeded') {
        throw executionError('JOB_RESULT_ALREADY_ACCEPTED', 'A Job with an accepted result cannot create a replacement Attempt.', ExitCode.conflict);
    }
    if (!['failed-retryable', 'timed-out'].includes(previous.status)) {
        throw executionError('ATTEMPT_NOT_REPLACEABLE', 'The previous Attempt is not in a replaceable terminal state.', ExitCode.guard);
    }
    if (previous.failure === null) {
        throw executionError('ATTEMPT_FAILURE_REQUIRED', 'A replacement Attempt requires the previous FailureDescriptor.', ExitCode.guard);
    }
    if ((input.retryMode === 'repair') !==
        (previous.failure.retryClass === 'repairable')) {
        throw executionError('RETRY_MODE_MISMATCH', 'The retry mode does not match the previous failure classification.', ExitCode.guard);
    }
    if (job.attemptCount >= job.retryPolicy.maxAttempts &&
        input.grantId === undefined) {
        throw executionError('RETRY_BUDGET_EXHAUSTED', 'The Job attempt budget is exhausted.', ExitCode.guard);
    }
    const attemptId = assertIdentifier(input.attemptId, 'attemptId');
    ...
    const changedFields = diffJson(previous.policySnapshot, policySnapshot, '', new Set(['/schemaVersion']));
    if (input.retryMode === 'execution-policy-change' &&
        changedFields.length === 0) {
        throw executionError('RETRY_POLICY_CHANGE_EMPTY', 'execution-policy-change requires at least one changed policy field.', ExitCode.usage);
    }
    if (input.retryMode === 'same-input' && changedFields.length !== 0) {
        throw executionError('RETRY_MODE_MISMATCH', 'same-input cannot change the execution policy.', ExitCode.usage);
    }
    const repairContext = input.repairContext === undefined
        ? null
        : assertRepairContext(input.repairContext, job);
    if ((input.retryMode === 'repair') !== (repairContext !== null)) {
        throw executionError('RETRY_REPAIR_CONTEXT_INVALID', 'A repair Attempt requires, and only a repair Attempt accepts, repair context.', ExitCode.usage);
    }
    const strategyChanges = normalizeUniqueStrings(input.strategyChanges ?? [], 'strategyChanges');
    if (input.retryMode === 'strategy-change' && strategyChanges.length === 0) {
        throw executionError('RETRY_STRATEGY_CHANGE_EMPTY', 'strategy-change requires a declared strategy change.', ExitCode.usage);
    }
    const grantId = input.grantId === undefined
        ? null
    ...
    const nextJob = {
        ...
        repairAttemptCount: job.repairAttemptCount + (input.retryMode === 'repair' ? 1 : 0),
        updatedAt: createdAt,
    };
    return deepFreeze({
...
export function executionPolicyChangedFields(previous, current) {
    const before = assertExecutionPolicy(previous);
    const after = assertExecutionPolicy(current);
    return deepFreeze(diffJson(before, after, '', new Set(['/schemaVersion'])));
}
export function leaseAttempt(inputAttempt, input) {
    const attempt = assertAttemptRecord(inputAttempt);
    if (attempt.status !== 'created' || attempt.lease !== null) {
        throw executionError('ATTEMPT_LEASE_CONFLICT', 'Only an unleased created Attempt can be leased.', ExitCode.conflict);
    }
    ...
    return deepFreeze(assertAttemptRecord({
        ...attempt,
        status: 'leased',
...
export function acceptAttemptResult(input) {
    ...
    const completedAt = assertTimestamp(input.completedAt ?? new Date().toISOString());
    if (input.expectedAcceptedAttemptId !== job.acceptedAttemptId) {
        throw executionError('JOB_RESULT_CAS_CONFLICT', 'The Job accepted-result compare-and-swap precondition changed.', ExitCode.staleState);
    }
    assertAttemptFence(attempt, input.leaseGeneration, input.leaseToken, completedAt);
    if (attempt.jobId !== job.jobId ||
        attempt.workflowId !== job.workflowId ||
        job.workflowId !== workflow.workflowId) {
        throw executionInvalid('Result identities do not bind to one Workflow and Job.');
    }
    ...
    if (workflow.status !== 'active' ||
        ...
        const staleJob = job.acceptedAttemptId === null
            ? assertJobRecord({
                ...job,
                status: 'stale',
                updatedAt: completedAt,
        ...
        return deepFreeze({
            ...
            result: createAttemptResult(staleAttempt, outputDigest, 'stale', completedAt),
        });
    }
    if (job.acceptedAttemptId !== null) {
        ...
        return deepFreeze({
            ...
            job,
            attempt: duplicate,
            result: createAttemptResult(duplicate, outputDigest, 'late-duplicate', completedAt),
        });
    }
    if (!['leased', 'running'].includes(attempt.status)) {
        throw executionError('ATTEMPT_RESULT_INELIGIBLE', 'The Attempt state is not eligible to submit a result.', ExitCode.guard);
    }
    ...
    return deepFreeze({
        ...
        result: createAttemptResult(acceptedAttempt, outputDigest, 'accepted', completedAt),
    });
}
export function createFailureDescriptor(input) {
    ...
    if (!['none', 'idempotent-confirmed', 'unknown'].includes(input.sideEffectState)) {
        throw executionInvalid('Failure sideEffectState is unknown.');
    }
    const paths = normalizeFailurePaths(input.paths);
    const validatorPath = input.validatorPath === null
        ? null
...
export function classifyExecutionFailure(input) {
    ...
    if (classification === undefined) {
        throw executionError('EXECUTION_FAILURE_KIND_UNKNOWN', 'The execution failure kind is not registered.', ExitCode.usage);
    }
...
export function decideRetry(input) {
    ...
    const now = assertTimestamp(input.now ?? new Date().toISOString());
    if (!Number.isSafeInteger(input.sameFingerprintCount) ||
    ...
    if (workflow.currentEpoch !== job.epoch ||
        workflow.currentEpoch !== attempt.epoch ||
        workflow.contextDigest !== job.contextDigest ||
        workflow.contextDigest !== attempt.contextDigest) {
        return {
    ...
    const requiredStrategyReason = failure.code === 'PROVIDER_TOOL_UNAVAILABLE'
    ...
    const grant = input.boundedGrantRequest === undefined
        ? undefined
    ...
    if (withinBudget) {
        const retryMode = inferRetryMode(failure, attempt, input.currentExecutionPolicy);
        const decision = {
            retryable: true,
            automatic: true,
            retryMode,
...
export function projectExecutionFailureState(input) {
    ...
    if (decision.requiredGrant !== undefined) {
        if (decision.requiredGrant.workflowId !== workflow.workflowId ||
    ...
    else if (failure.code === 'NEEDS_USER_DECISION' ||
    ...
    else if (failure.sideEffectState === 'unknown' ||
        ...
        if (failure.code === 'PROVIDER_RATE_LIMIT') {
            blocker = {
                ...
                ...(failure.retryAfterMs === null
                    ? {}
                    : {
                        retryAt: new Date(Date.parse(failure.observedAt) + failure.retryAfterMs).toISOString(),
                    }),
            };
        }
        else if (failure.code === 'PROVIDER_CAPACITY' ||
    ...
    return deepFreeze({
        ...
        job: assertJobRecord({
            ...job,
            status,
            updatedAt: new Date(Math.max(Date.parse(job.updatedAt), Date.parse(failure.observedAt))).toISOString(),
        }),
    });
}
...
export function assertReadOnlyProbe(input) {
    if (!isPlainObject(input) ||
        !hasOnlyKeys(input, ['kind', 'target', 'timeoutMs'])) {
        throw probeNotAllowed();
    }
    if (typeof input.kind !== 'string' ||
        !READ_ONLY_PROBE_KINDS.has(input.kind) ||
    ...
    if (requiresTarget !== Object.hasOwn(input, 'target') ||
        (requiresTarget &&
    ...
    if (kind === 'file-read' &&
        !isSafeRepositoryRelativePath(input.target)) {
        throw probeNotAllowed();
    }
    if (kind === 'binary-exists' &&
        !/^[A-Za-z0-9][A-Za-z0-9._+-]{0,127}$/.test(input.target)) {
        throw probeNotAllowed();
    }
    if (kind === 'validator-error' &&
        !/^[A-Z][A-Z0-9_]{1,127}$/.test(input.target)) {
        throw probeNotAllowed();
    }
    if (kind === 'dependency-availability' &&
        !/^(?:@[A-Za-z0-9][A-Za-z0-9._-]{0,63}\/)?[A-Za-z0-9][A-Za-z0-9._-]{0,127}(?:\/[A-Za-z0-9][A-Za-z0-9._-]{0,127})*$/.test(input.target)) {
        throw probeNotAllowed();
    }
    const probe = {
        kind,
...
export function projectLegacyProviderInvocation(input) {
    ...
    if (record.schemaVersion !== 1 ||
        ...
        throw executionError('LEGACY_INVOCATION_INVALID', 'The legacy ProviderInvocationRecord cannot be projected safely.', ExitCode.guard);
    }
    const workflowId = assertIdentifier(record.investigationId, 'workflowId');
    const epoch = assertPositiveInteger(input.epoch, 'epoch');
    const contractVersion = assertPositiveInteger(input.contractVersion, 'contractVersion');
    ...
    if (executionPolicy.provider !== record.providerId) {
        throw executionInvalid('Legacy projection provider policy does not match the record.');
    }
    const createdAt = assertTimestamp(record.createdAt);
    const updatedAt = assertTimestamp(record.updatedAt);
    const attemptId = assertIdentifier(`attempt-legacy-${record.invocationId}`, 'attemptId');
    ...
    const jobIdentityDigest = digestCanonical({
        ...
        ...(input.semanticJobIdentityDigest === undefined
            ? {}
            : {
                semanticJobIdentityDigest: assertDigest(input.semanticJobIdentityDigest),
            }),
    }).slice('sha256:'.length, 'sha256:'.length + 32);
    const jobId = assertIdentifier(`job-legacy-${record.purpose}-${jobIdentityDigest}`, 'jobId');
    const semanticSpec = {
        schemaVersion: 1,
        contractVersion,
    ...
    const requestDigest = digestCanonical({
        ...
        ...(record.mandateBinding === undefined
            ? {}
            : {
                mandateBinding: assertExecutionJobMandateBinding(record.mandateBinding),
            }),
        semanticSpec,
    });
    ...
    const failure = legacyFailure(record, semanticSpec.inputDigest, input.environmentDigest);
...
export function assertJobRecord(value) {
    ...
    if (semanticSpec.stage !== value.stage ||
    ...
    const attemptCount = assertNonnegativeInteger(value.attemptCount, 'attemptCount');
    if (attemptCount < 1) {
...
export function requireExecutionJobMandateBinding(input) {
    const job = assertJobRecord(input);
    if (job.mandateBinding === undefined) {
        throw executionError('EXECUTION_JOB_MANDATE_REQUIRED', 'A legacy Execution Job without a durable Task Mandate binding is read-only.', ExitCode.guard);
    }
    return assertExecutionJobMandateBinding(job.mandateBinding);
}
export function assertAttemptRecord(value) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'attemptId',
            'attemptNumber',
        ...
        !RETRY_MODES.has(value.retryMode) ||
        !ATTEMPT_RETENTION.has(value.retention)) {
    ...
    if (value.retryOf !== null)
        assertIdentifier(value.retryOf, 'retryOf');
    ...
    normalizeUniqueStrings(value.strategyChanges, 'strategyChanges', true);
    if (value.grantId !== null)
        assertIdentifier(value.grantId, 'grantId');
    ...
    if ((['failed-retryable', 'failed-terminal', 'timed-out'].includes(value.status) &&
        value.failure === null) ||
        (['created', 'leased', 'running', 'succeeded'].includes(value.status) &&
            value.failure !== null)) {
        throw executionInvalid('Attempt terminal failure state and FailureDescriptor disagree.');
    }
    assertNonnegativeInteger(value.leaseGeneration, 'leaseGeneration');
    assertAttemptLease(value.lease, value.leaseGeneration);
...
function assertExecutionPolicy(value) {
    ...
    assertBoundedPositiveInteger(value.maxOutputBytes, 'maxOutputBytes', 268_435_456);
...
function assertRetryPolicy(value) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'deadline',
            'maxAttempts',
    ...
    assertBoundedPositiveInteger(value.maxCumulativeRuntimeMs, 'maxCumulativeRuntimeMs', 31_536_000_000);
    assertBoundedNonnegativeInteger(value.maxProviderCostMicros, 'maxProviderCostMicros', Number.MAX_SAFE_INTEGER);
    assertBoundedNonnegativeInteger(value.maxProviderTokens, 'maxProviderTokens', Number.MAX_SAFE_INTEGER);
    assertBoundedPositiveInteger(value.maxSameFailureFingerprint, 'maxSameFailureFingerprint', 100);
    assertBoundedNonnegativeInteger(value.maxRepairAttempts, 'maxRepairAttempts', 100);
...
function assertFailureDescriptor(value) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'code',
            'environmentDigest',
    ...
    const rebuilt = createFailureDescriptor({
        ...
        paths: value.paths,
        validatorPath: value.validatorPath,
        inputDigest: value.inputDigest,
        environmentDigest: value.environmentDigest,
        observedAt: value.observedAt,
        retryAfterMs: value.retryAfterMs === null ? undefined : value.retryAfterMs,
    });
...
function assertEnvironmentSnapshot(value) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'capabilities',
            'provider',
    ...
    if (typeof value.repository.head !== 'string' ||
    ...
    assertBoundedPositiveInteger(value.provider.timeoutMs, 'provider.timeoutMs', 3_600_000);
    const toolNames = [];
...
function assertWorkflowBlocker(value) {
    if (value === null)
        return;
    if (!isPlainObject(value) ||
        !hasOnlyKeys(value, [
        ...
        !WORKFLOW_BLOCKERS.has(value.kind)) {
        throw executionInvalid('Workflow blocker is invalid.');
    }
    assertTimestamp(value.since);
    if (value.jobId !== undefined)
        assertIdentifier(value.jobId, 'blocker.jobId');
    if (value.retryAt !== undefined)
        assertTimestamp(value.retryAt);
    if (value.detailsDigest !== undefined)
        assertDigest(value.detailsDigest);
...
function assertRepairContext(value, job) {
    const context = assertRepairContextShape(value);
    if (context.epoch !== job.epoch ||
        context.contextDigest !== job.contextDigest) {
        throw executionError('RETRY_REPAIR_CONTEXT_STALE', 'Repair context is not bound to the current Job epoch.', ExitCode.staleState);
    }
    return context;
}
function assertRepairContextShape(value) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'contextDigest',
            'epoch',
    ...
    for (const error of value.validationErrors) {
        ...
        if (key <= previousKey) {
            throw executionInvalid('Repair validation errors are not sorted and unique.');
        }
        previousKey = key;
    }
    return deepFreeze(structuredClone(value));
}
function assertChangedFields(value) {
...
function assertAttemptLease(value, generation) {
    if (value === null)
        return;
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
...
function assertExecutionBudgetGrantRequest(value, job) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'createdAt',
            'epoch',
    ...
    for (const change of value.requestedChanges) {
        ...
        if (pointer <= previousChange ||
            canonicalJson(change.from) === canonicalJson(change.to)) {
            throw executionInvalid('Execution budget requested changes are not canonical.');
        }
        previousChange = pointer;
    }
    assertBoundedString(value.rationale, 'grant rationale');
    assertBoundedPositiveInteger(value.expiresAfterAttempts, 'grant expiresAfterAttempts', 16);
    assertTimestamp(value.createdAt);
    if (value.workflowId !== job.workflowId ||
        value.epoch !== job.epoch ||
        value.jobId !== job.jobId ||
        canonicalJson(requestBinding) !== canonicalJson(jobBinding)) {
        throw executionError('EXECUTION_GRANT_REQUEST_BINDING_INVALID', 'The bounded GrantRequest is not bound to the Job.', ExitCode.guard);
    }
    return deepFreeze(structuredClone(value));
}
function assertAttemptFence(attempt, generation, token, completedAt) {
    if (attempt.lease === null ||
        ...
        throw executionError('ATTEMPT_FENCE_REJECTED', 'The Attempt result was produced by an expired or stale lease holder.', ExitCode.staleState);
    }
}
export function createAttemptResult(inputAttempt, inputOutputDigest, acceptance, inputCompletedAt) {
    const attempt = assertAttemptRecord(inputAttempt);
...
function isWithinAutomaticBudget(input) {
    ...
    return (job.attemptCount < job.retryPolicy.maxAttempts &&
...
function inferRetryMode(failure, attempt, currentPolicyInput) {
    if (failure.retryClass === 'repairable')
        return 'repair';
    if (currentPolicyInput !== undefined) {
        const currentPolicy = assertExecutionPolicy(currentPolicyInput);
        if (canonicalJson(currentPolicy) !== canonicalJson(attempt.policySnapshot)) {
            return 'execution-policy-change';
        }
    }
    return 'same-input';
}
function noRetry(reasonCode) {
    return deepFreeze({
        retryable: false,
        automatic: false,
        retryMode: 'none',
...
function grantDecision(requiredGrant, reasonCode) {
    return deepFreeze({
        retryable: true,
        automatic: false,
        retryMode: 'none',
...
function legacyAttemptStatus(record) {
    ...
    if (record.failure?.executionKind === 'needs-user-decision' ||
...
function legacyJobStatus(record) {
    if (record.state === 'prepared')
        return 'queued';
    if (record.state === 'leased')
        return 'running';
...
function legacyFailure(record, inputDigest, environmentDigest) {
    if (record.failure === null)
        return null;
    const code = /^[A-Z][A-Z0-9_]{1,127}$/.test(record.failure.code)
        ? record.failure.code
        : 'LEGACY_PROVIDER_FAILURE';
    const classification = record.failure.executionKind === undefined
...
function classifyLegacyProviderFailure(code, kind) {
    ...
    const source = code.includes('NETWORK')
        ? 'network'
...
function parseCanonicalRecord(raw, assertRecord) {
    if (typeof raw !== 'string' ||
        Buffer.byteLength(raw, 'utf8') > MAX_RECORD_BYTES ||
        !raw.endsWith('\n') ||
        raw.endsWith('\n\n')) {
        throw executionError('EXECUTION_RECORD_NONCANONICAL', 'Execution record bytes are not canonical.', ExitCode.guard);
    }
    let parsed;
    try {
        ...
        throw executionError('EXECUTION_RECORD_INVALID', 'Execution record JSON is invalid.', ExitCode.guard);
    }
...
function diffJson(before, after, basePath = '', ignored = new Set()) {
...
function assertEnvironmentRecordValue(value) {
    canonicalJson(value);
}
...
function assertPrimitiveRecord(value, label) {
    if (!isPlainObject(value))
        throw executionInvalid(`${label} is invalid.`);
    assertSortedObjectKeys(value, label);
    for (const [key, entry] of Object.entries(value)) {
...
function normalizeUniqueStrings(value, label, requireCanonical = false) {
    if (!Array.isArray(value))
        throw executionInvalid(`${label} is invalid.`);
    for (const entry of value)
        assertBoundedString(entry, label);
...
function normalizeFailurePaths(value) {
    if (!Array.isArray(value))
        throw executionInvalid('Failure paths are invalid.');
    const normalized = value.map((entry) => {
        if (entry.startsWith('/'))
            return assertJsonPointer(entry, 'failure path');
...
function assertExecutionJobMandateBinding(value) {
    if (!isPlainObject(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'mandateTaskId',
...
function assertBoundedPositiveInteger(value, label, maximum) {
    const result = assertPositiveInteger(value, label);
    if (result > maximum)
        throw executionInvalid(`${label} exceeds its bound.`);
    return result;
}
function assertBoundedNonnegativeInteger(value, label, maximum) {
...
function probeNotAllowed() {
    return executionError('EXECUTION_PROBE_NOT_ALLOWED', 'The requested environment probe is not on the read-only allowlist.', ExitCode.guard);
}
function executionError(code, message, exitCode) {
...
const WORKFLOW_BLOCKERS = new Set([
    'retry-delay',
    'provider-capacity',
    'human-grant',
    'human-input',
...
const ATTEMPT_RETENTION = new Set([
    'active',
    'debug',
    'pinned',
]);
...
const READ_ONLY_PROBE_KINDS = new Set([
    'repository-head',
