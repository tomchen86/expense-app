 ...
 */
export const RECOVERY_TRUST_ROOT_RESTORE_NAMESPACE = 'HARNESS_RECOVERY_RESTORE_OPERATIONAL_TRUST_ROOT_V1';
...
const SSH_PUBLIC_KEY = /^(?:ssh-ed25519|ssh-rsa|ecdsa-sha2-nistp(?:256|384|521)|sk-ssh-ed25519@openssh\.com|sk-ecdsa-sha2-nistp256@openssh\.com) [A-Za-z0-9+/]+={0,2}$/;
const POINTER_FILE = /^[0-9]{20}\.json$/;
const ROOT_FILE = /^[0-9a-f]{64}\.json$/;
const OPERATION_DIRECTORY = /^[0-9a-f]{64}$/;
const PREPARATION = /^\.([a-z0-9][a-z0-9.-]*\.json)\.([0-9a-f]{64})\.([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.prepare$/;
...
const ROOT_KEYS = [...ROOT_PAYLOAD_KEYS, 'rootDigest'];
const SIGNER_KEYS = ['fingerprint', 'identity', 'publicKey'];
...
const GRANT_KEYS = [...UNSIGNED_GRANT_KEYS, 'grantId'];
const ENVELOPE_KEYS = ['payload', 'replacement', 'signature'];
const POINTER_KEYS = [
    'activatedAt',
    'authorityDescriptorDigest',
    'authorityGeneration',
...
export function recoveryOperationalTrustRootDigest(value) {
    return canonicalDigest(normalizeRootPayload(value));
}
export function recoveryOperationalTrustRootRestoreGrantId(value) {
    return `trust-root-restore-${digestHex(canonicalJson(normalizeUnsignedGrant(value)))}`;
}
export function canonicalRecoveryOperationalTrustRootRestoreStatement(payloadValue, replacementValue) {
...
export function executeRecoveryOperationalTrustRootRestore(externalEnvelopePath, privateStoreRoot, boundary, dependencies) {
    ...
    const envelope = verifyEnvelope(readExternalEnvelope(externalEnvelopePath), dependencies);
    const payload = envelope.payload;
    const envelopeDigest = canonicalDigest(envelope);
    const expectedReservation = createReservation(envelope, envelopeDigest);
    const observedPaths = operationPaths(privateStoreRoot, payload.grantId, false);
    const hasAdmissionEvidence = observedPaths !== null &&
        hasExactPublicationEvidence(observedPaths.operationDirectory, 'reservation.json', expectedReservation);
    if (!hasAdmissionEvidence) {
        assertGrantWindow(payload, dependencies.now);
        assertPointerCas(readActiveTrustRootChain(privateStoreRoot, dependencies.authorityDescriptor, dependencies.authorityExpectations, dependencies.verifyHumanSignature, dependencies.externalAuditRoot, payload.grantId), payload);
    }
    const paths = operationPaths(privateStoreRoot, payload.grantId, true);
    if (paths === null)
        throw stateCorrupt();
    ensureImmutable(paths.reservation, expectedReservation, dependencies, 'reservation-published');
    let record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, payload.grantId);
    ...
    const rootPath = path.join(paths.roots, `${payload.replacementRootDigest.slice('sha256:'.length)}.json`);
    ensureImmutable(rootPath, envelope.replacement, dependencies, 'root-published');
    const pointer = createPointer(payload, envelopeDigest, record.reservation);
    const activeBefore = readActiveTrustRootChain(privateStoreRoot, dependencies.authorityDescriptor, dependencies.authorityExpectations, dependencies.verifyHumanSignature, dependencies.externalAuditRoot, payload.grantId);
    if (activeBefore?.pointer.pointerDigest !== pointer.pointerDigest) {
        assertPointerCas(activeBefore, payload);
        const pointerPath = path.join(paths.pointers, pointerFileName(pointer.generation));
        ensureImmutable(pointerPath, pointer, dependencies, 'pointer-published');
    }
    const active = readActiveTrustRootChain(privateStoreRoot, dependencies.authorityDescriptor, dependencies.authorityExpectations, dependencies.verifyHumanSignature, dependencies.externalAuditRoot, payload.grantId);
    if (active === null ||
        active.pointer.pointerDigest !== pointer.pointerDigest ||
        active.root.rootDigest !== envelope.replacement.rootDigest) {
        throw stateCorrupt();
    }
    record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, payload.grantId);
    if (record.receipt === null) {
        ensureImmutable(paths.receipt, createReceipt(payload, pointer, record.reservation.reservedAt), dependencies, 'receipt-published');
    }
    record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, payload.grantId);
    if (record.receipt === null)
        throw stateCorrupt();
    if (record.terminal === null) {
        ensureImmutable(paths.terminal, createTerminal(payload, envelopeDigest, record.receipt, record.reservation.reservedAt), dependencies, 'terminal-published');
    }
...
export function readRecoveryOperationalTrustRootActive(privateStoreRoot, descriptorValue, expectations, dependencies) {
    if (!isRecord(dependencies) ||
        ...
        throw restoreError('RECOVERY_TRUST_ROOT_VERIFIER_REQUIRED', 'Active operational trust-root reads require the pinned Recovery Authority signature verifier.', ExitCode.unsafeEnvironment);
    }
    ...
    assertExternalAuditBoundary(dependencies.externalAuditRoot, dependencies.boundary);
    return readActiveTrustRootChain(privateStoreRoot, descriptorValue, expectations, dependencies.verifyHumanSignature, dependencies.externalAuditRoot, null);
}
function readActiveTrustRootChain(privateStoreRoot, descriptorValue, expectations, verifyHumanSignature, externalAuditRoot, allowedPendingGrantId) {
    const descriptor = verifyRecoveryAuthorityDescriptor(descriptorValue, expectations);
    ...
    assertNoForeignUnpublishedPreparation(paths, privateStoreRoot, allowedPendingGrantId);
    const names = fs
    ...
    for (const [index, name] of names.entries()) {
        ...
        if (name !== pointerFileName(generation))
            throw stateCorrupt();
        const pointer = normalizePointer(readPublished(path.join(paths.pointers, name)));
        if (pointer.repositoryId !== descriptor.repositoryIdentity.repositoryId ||
        ...
        const rootPath = path.join(paths.roots, `${pointer.rootDigest.slice('sha256:'.length)}.json`);
        const root = normalizeRoot(readPublished(rootPath));
        if (root.repositoryId !== descriptor.repositoryIdentity.repositoryId ||
            root.generation !== pointer.generation ||
            root.rootDigest !== pointer.rootDigest) {
            throw stateCorrupt();
        }
        assertPersistedPointerAuthority(privateStoreRoot, descriptor, expectations, verifyHumanSignature, externalAuditRoot, pointer, root, allowedPendingGrantId);
        previous = pointer;
        activeRoot = root;
    }
    if (previous === null || activeRoot === null)
        throw stateCorrupt();
    return deepFreeze({ pointer: previous, root: activeRoot });
}
function assertNoForeignUnpublishedPreparation(paths, privateStoreRoot, allowedPendingGrantId) {
    const allowedTargets = new Set();
    if (allowedPendingGrantId !== null) {
        const operation = operationPaths(privateStoreRoot, allowedPendingGrantId, false);
        if (operation !== null &&
            fs.lstatSync(operation.reservation, { throwIfNoEntry: false }) !==
                undefined) {
            const reservation = normalizeReservation(readPublished(operation.reservation));
            const payload = reservation.envelope.payload;
            allowedTargets.add(`${payload.replacementRootDigest.slice('sha256:'.length)}.json`);
...
function assertNoForeignPendingRestore(paths, privateStoreRoot, allowedPendingGrantId) {
    const allowedDirectory = allowedPendingGrantId === null ? null : digestHex(allowedPendingGrantId);
    for (const name of fs.readdirSync(paths.operations).sort()) {
        ...
        if (fs.lstatSync(reservationPath, { throwIfNoEntry: false }) === undefined) {
            if (name !== allowedDirectory)
                throw transitionIncomplete();
            continue;
        }
        const reservation = normalizeReservation(readPublished(reservationPath));
        if (digestHex(reservation.grantId) !== name)
            throw stateCorrupt();
        const record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, reservation.grantId);
        if (record.auditAcknowledgement === null &&
...
function assertPersistedPointerAuthority(privateStoreRoot, descriptor, expectations, verifyHumanSignature, externalAuditRoot, pointer, root, allowedPendingGrantId) {
    const record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, pointer.grantId);
    ...
    assertRecoveryAuthorityDomain(descriptor, expectations, RECOVERY_TRUST_ROOT_RESTORE_NAMESPACE, payload.signerFingerprint);
    let verified = false;
    try {
        verified = verifyHumanSignature(canonicalRecoveryOperationalTrustRootRestoreStatement(payload, root), envelope.signature, payload.humanSigner, RECOVERY_TRUST_ROOT_RESTORE_NAMESPACE);
    }
    catch {
        verified = false;
    }
    if (!verified) {
        throw restoreError('RECOVERY_TRUST_ROOT_PERSISTED_SIGNATURE_INVALID', 'Active operational trust-root lineage does not retain a valid Recovery Authority signature.', ExitCode.verification);
    }
    if (record.auditAcknowledgement === null &&
...
export function readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, grantIdValue) {
    ...
    const auditAcknowledgement = readOptionalPublished(paths.auditAcknowledgement, normalizeAuditAcknowledgement);
...
function verifyEnvelope(value, dependencies) {
    ...
    if (payload.repositoryId !== descriptor.repositoryIdentity.repositoryId ||
        ...
        payload.humanSigner !== descriptor.signer.identity ||
        payload.signerFingerprint !== descriptor.signer.fingerprint) {
        throw restoreError('RECOVERY_TRUST_ROOT_AUTHORITY_BINDING_MISMATCH', 'Trust-root restore does not bind the exact loaded Recovery Authority, runtime, and audit root.', ExitCode.verification);
    }
    assertRecoveryAuthorityDomain(descriptor, dependencies.authorityExpectations, RECOVERY_TRUST_ROOT_RESTORE_NAMESPACE, payload.signerFingerprint);
    let verified = false;
    try {
        verified = dependencies.verifyHumanSignature(canonicalRecoveryOperationalTrustRootRestoreStatement(payload, replacement), envelope.signature, payload.humanSigner, RECOVERY_TRUST_ROOT_RESTORE_NAMESPACE);
    }
    catch {
        verified = false;
    }
    if (!verified) {
        throw restoreError('RECOVERY_TRUST_ROOT_SIGNATURE_INVALID', 'Trust-root restore signature is invalid for its independent Recovery Authority domain.', ExitCode.verification);
    }
    return envelope;
}
function finishAudit(privateStoreRoot, paths, payload, dependencies) {
    let record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, payload.grantId);
    ...
    const audit = record.audit ??
        createAuditRecord(payload, record.reservation.envelopeDigest, record.receipt, record.terminal, record.reservation.reservedAt);
    ...
    ensureImmutable(paths.auditAcknowledgement, createAuditAcknowledgement(audit, record.reservation.reservedAt), dependencies, 'audit-ack-published');
    record = readRecoveryOperationalTrustRootRestoreRecord(privateStoreRoot, payload.grantId);
    if (record.auditAcknowledgement === null)
        throw stateCorrupt();
    return record;
}
function createReservation(envelope, envelopeDigest) {
    const payload = {
...
function createPointer(payload, envelopeDigest, reservation) {
    const value = {
...
function createReceipt(payload, pointer, completedAt) {
    const value = {
...
function createTerminal(payload, envelopeDigest, receipt, consumedAt) {
    const value = {
...
function createAuditRecord(payload, envelopeDigest, receipt, terminal, recordedAt) {
...
function normalizeRoot(value) {
    ...
    if (rootDigest !== expected)
        throw rootInvalid();
    return deepFreeze({ ...payload, rootDigest: expected });
}
function normalizeRootPayload(value) {
    if (!isRecord(value) ||
    ...
    const sorted = [...trustedSigners].sort((left, right) => left.identity.localeCompare(right.identity));
    if (trustedSigners.some((signer, index) => signer.identity !== sorted[index]?.identity) ||
...
function normalizeUnsignedGrant(value) {
    if (!isRecord(value) ||
...
function normalizePointer(value) {
    ...
    if (value.pointerDigest !== pointerDigest)
        throw stateCorrupt();
...
function normalizeReservation(value) {
    ...
    if (envelope.payload.grantId !== value.grantId ||
...
function normalizeReceipt(value) {
    ...
    if (value.receiptDigest !== canonicalDigest(payload))
        throw stateCorrupt();
...
function normalizeTerminal(value) {
    ...
    if (value.terminalDigest !== canonicalDigest(payload))
        throw stateCorrupt();
...
function normalizeAudit(value) {
    ...
    if (value.recordDigest !== canonicalDigest(payload))
        throw stateCorrupt();
    return deepFreeze({ ...payload, recordDigest: value.recordDigest });
}
function normalizeAuditAcknowledgement(value) {
    if (!isRecord(value) ||
...
function assertReplacementBinding(payload, replacement) {
    if (replacement.repositoryId !== payload.repositoryId ||
...
function assertRecordChain(reservation, receipt, terminal, audit, acknowledgement) {
    const payload = reservation.envelope.payload;
    if (receipt !== null) {
        if (receipt.grantId !== payload.grantId ||
...
function assertPointerCas(active, payload) {
    const generation = active?.pointer.generation ?? 0;
    const digest = active?.pointer.pointerDigest ?? null;
    if (generation !== payload.expectedGeneration ||
        digest !== payload.expectedActivePointerDigest) {
        throw restoreError('RECOVERY_TRUST_ROOT_POINTER_CAS_MISMATCH', 'Trust-root restore does not match the exact active generation and pointer digest.', ExitCode.staleState);
    }
}
function assertGrantWindow(payload, nowValue) {
    const now = exactDate(nowValue).getTime();
    if (now < Date.parse(payload.issuedAt)) {
        throw restoreError('RECOVERY_TRUST_ROOT_GRANT_NOT_YET_VALID', 'Trust-root restore Grant is not yet valid.', ExitCode.staleState);
    }
    if (now >= Date.parse(payload.expiresAt)) {
        throw restoreError('RECOVERY_TRUST_ROOT_GRANT_EXPIRED', 'Trust-root restore Grant has expired.', ExitCode.staleState);
    }
}
function statePaths(privateStoreRoot, create) {
...
function operationPaths(privateStoreRoot, grantIdValue, create) {
    if (!GRANT_ID.test(grantIdValue))
        throw grantInvalid();
    const paths = statePaths(privateStoreRoot, create);
    if (paths === null)
        return null;
    const operationDirectory = path.join(paths.operations, digestHex(grantIdValue));
    if (create) {
        ensurePrivateDirectory(paths.operations, operationDirectory);
    }
    else if (fs.lstatSync(operationDirectory, { throwIfNoEntry: false }) === undefined) {
        return null;
    }
    else {
...
function assertStateClosure(paths) {
    ...
    assertPreparedDirectoryClosure(paths.pointers, (name) => POINTER_FILE.test(name));
    for (const entry of fs.readdirSync(paths.operations, {
        ...
        assertPrivateDirectory(path.join(paths.operations, entry.name), stateCorrupt);
    }
}
function assertOperationClosure(paths) {
    assertStateClosure(paths);
...
function assertPreparedDirectoryClosure(directory, allowedTarget) {
    assertPrivateDirectory(directory, stateCorrupt);
    for (const name of fs.readdirSync(directory).sort()) {
        ...
        const match = PREPARATION.exec(name);
        if (match === null || !allowedTarget(match[1]))
            throw stateCorrupt();
        ...
        else if (fs.lstatSync(path.join(directory, match[1]), {
...
function hasExactPublicationEvidence(directory, targetName, value) {
    assertPrivateDirectory(directory, stateCorrupt);
    const target = path.join(directory, targetName);
    if (fs.lstatSync(target, { throwIfNoEntry: false }) !== undefined) {
...
function ensureImmutable(filePath, value, dependencies, publishedPhase) {
    ...
    if (existing.length === 1) {
        ...
        preparation = path.join(directory, `.${targetName}.${digest}.${crypto.randomUUID()}.prepare`);
    }
...
function completePreparation(preparation, expected, dependencies, targetName) {
    const observed = fs.lstatSync(preparation, { throwIfNoEntry: false });
    let descriptor;
    try {
        if (observed === undefined) {
            descriptor = fs.openSync(preparation, fs.constants.O_RDWR |
            ...
            descriptor = fs.openSync(preparation, fs.constants.O_RDWR | (fs.constants.O_NOFOLLOW ?? 0));
            const opened = fs.fstatSync(descriptor);
            if (!sameFileSnapshot(observed, opened, [1]))
                throw stateCorrupt();
        }
        ...
        if (existing.length > 0) {
            ...
            if (read !== existing.length)
                throw stateCorrupt();
        }
        if (!isBufferPrefix(existing, expected))
            throw stateCorrupt();
        ...
        while (offset < expected.length) {
            const written = fs.writeSync(descriptor, expected, offset, expected.length - offset, offset);
            if (written < 1)
                throw storeUnsafe();
            offset += written;
        }
        ...
        if (!sameInode(openedBefore, openedAfter) ||
            openedAfter.nlink !== 1 ||
        ...
        if (pathAfterHook === undefined ||
            !sameFileSnapshot(openedAfter, pathAfterHook, [1])) {
            throw stateCorrupt();
        }
    }
    catch (error) {
        if (hasWorkflowCode(error) || error instanceof Error)
            throw error;
        throw storeUnsafe();
    }
    finally {
...
function readOptionalPublished(filePath, normalize) {
    if (fs.lstatSync(filePath, { throwIfNoEntry: false }) === undefined) {
        return null;
    }
    return normalize(readPublished(filePath));
}
function matchingPreparations(directory, targetName) {
    const matches = [];
    for (const name of fs.readdirSync(directory).sort()) {
        const match = PREPARATION.exec(name);
        if (match?.[1] === targetName)
            matches.push(path.join(directory, name));
    }
    return matches;
}
function readPreparation(filePath) {
...
function assertExactPreparation(filePath, expected, allowedLinkCounts) {
    const observed = readPrivateFile(filePath, allowedLinkCounts);
    if (!observed.equals(expected))
        throw stateCorrupt();
}
function readPrivateFile(filePath, allowedLinkCounts) {
...
function readExternalEnvelope(filePath) {
    const before = fs.lstatSync(filePath, { throwIfNoEntry: false });
    if (before === undefined ||
        !before.isFile() ||
        before.isSymbolicLink() ||
...
function parseCanonicalBytes(bytes, malformed = stateCorrupt) {
    try {
...
function assertBoundaries(externalEnvelopePath, privateStoreRoot, boundary) {
    ...
    if (!isExactAbsolutePath(externalEnvelopePath) ||
        pathIsWithin(worktree, externalEnvelopePath) ||
        pathIsWithin(gitCommon, externalEnvelopePath)) {
        throw restoreError('RECOVERY_TRUST_ROOT_EXTERNAL_SOURCE_FORBIDDEN', 'Pre-signed trust-root replacement must come from outside the repository worktree and Git common directory.', ExitCode.unsafeEnvironment);
    }
}
function assertPrivateStoreBoundary(privateStoreRoot, boundary) {
    if (!isRecord(boundary) ||
        !hasExactKeys(boundary, ['gitCommonDirectory', 'repositoryWorktreeRoot'])) {
        throw boundaryInvalid();
    }
    ...
    const gitCommon = exactDirectoryPath(boundary.gitCommonDirectory);
    assertPrivateDirectory(privateStoreRoot, storeUnsafe);
    if (privateStoreRoot === gitCommon ||
        !pathIsWithin(gitCommon, privateStoreRoot)) {
        throw restoreError('RECOVERY_TRUST_ROOT_STORE_BOUNDARY_MISMATCH', 'Trust-root restore store must be an exact private descendant of the Git common directory.', ExitCode.unsafeEnvironment);
    }
}
function assertExternalAuditBoundary(externalAuditRoot, boundary) {
    const worktree = exactDirectoryPath(boundary.repositoryWorktreeRoot);
    const gitCommon = exactDirectoryPath(boundary.gitCommonDirectory);
    if (pathIsWithin(worktree, externalAuditRoot) ||
        pathIsWithin(gitCommon, externalAuditRoot)) {
        throw restoreError('RECOVERY_TRUST_ROOT_AUDIT_ROOT_UNSAFE', 'Trust-root restore audit root must remain outside the repository worktree and Git common directory.', ExitCode.unsafeEnvironment);
    }
}
function assertDependencies(dependencies) {
    if (!isRecord(dependencies) ||
        ...
        throw restoreError('RECOVERY_TRUST_ROOT_DEPENDENCIES_INVALID', 'Trust-root restore requires exact out-of-band authority, time, signature, and audit dependencies.', ExitCode.unsafeEnvironment);
    }
    assertPrivateDirectory(dependencies.externalAuditRoot, auditRootUnsafe);
}
function exactDirectoryPath(value) {
    if (!isExactAbsolutePath(value))
        throw boundaryInvalid();
...
function assertPrivateDirectory(directory, errorFactory) {
...
function pointerFileName(generation) {
...
function exactDate(value) {
    if (!(value instanceof Date) || !Number.isFinite(value.getTime())) {
        throw restoreError('RECOVERY_TRUST_ROOT_TIME_INVALID', 'Trust-root restore requires a valid injected time.', ExitCode.unsafeEnvironment);
    }
    return new Date(value.getTime());
}
function isBufferPrefix(prefix, value) {
...
function restoreError(code, message, exitCode) {
    return workflowError(code, message, exitCode);
}
function grantInvalid() {
    return restoreError('RECOVERY_TRUST_ROOT_GRANT_INVALID', 'Trust-root restore Grant or canonical replacement bundle is malformed.', ExitCode.guard);
}
function rootInvalid() {
    return restoreError('RECOVERY_TRUST_ROOT_REPLACEMENT_INVALID', 'Operational trust-root replacement does not match its exact schema or digest.', ExitCode.guard);
}
function stateCorrupt() {
    return restoreError('RECOVERY_TRUST_ROOT_STATE_CORRUPT', 'Trust-root restore durable state failed integrity verification.', ExitCode.verification);
}
function storeUnsafe() {
    return restoreError('RECOVERY_TRUST_ROOT_STORE_UNSAFE', 'Trust-root restore durable store is unavailable or unsafe.', ExitCode.unsafeEnvironment);
}
function externalFileUnsafe() {
    return restoreError('RECOVERY_TRUST_ROOT_EXTERNAL_FILE_UNSAFE', 'Pre-signed trust-root replacement must be one exact private regular file.', ExitCode.unsafeEnvironment);
}
function boundaryInvalid() {
    return restoreError('RECOVERY_TRUST_ROOT_BOUNDARY_INVALID', 'Trust-root restore requires exact repository and Git common boundaries.', ExitCode.unsafeEnvironment);
}
function auditRootUnsafe() {
    return restoreError('RECOVERY_TRUST_ROOT_AUDIT_ROOT_UNSAFE', 'Trust-root restore external audit root must be an exact private directory.', ExitCode.unsafeEnvironment);
}
function alreadyConsumed() {
    return restoreError('RECOVERY_TRUST_ROOT_GRANT_ALREADY_CONSUMED', 'Trust-root restore Grant is one-shot and has already been consumed.', ExitCode.conflict);
}
function transitionIncomplete() {
    return restoreError('RECOVERY_TRUST_ROOT_TRANSITION_INCOMPLETE', 'Operational trust-root transition is not durably consumed and audited.', ExitCode.conflict);
}
function hasWorkflowCode(error) {
