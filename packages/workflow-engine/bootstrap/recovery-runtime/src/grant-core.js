...
export function createGrantChallenge(requested, registry, options) {
    ...
    if (!STABLE_ID.test(requested.sourceModuleId) ||
    ...
    if (expiresAt.getTime() <= now.getTime() ||
        ...
        throw grantInvalid('GRANT_CHALLENGE_BOUNDS_INVALID', 'Grant challenge expiry is invalid.');
    }
    const challengeId = options.challengeId ?? crypto.randomUUID();
    if (!UUID_V4.test(challengeId)) {
        throw grantInvalid('GRANT_CHALLENGE_ID_INVALID', 'Grant challenge ID is invalid.');
    }
    const facts = freezeCanonical(requested.facts);
    const choices = requested.candidates.map((candidate) => createChoice(candidate, registry));
    if (new Set(choices.map(({ choiceId }) => choiceId)).size !== choices.length) {
        throw grantInvalid('GRANT_CHOICE_DUPLICATE', 'Grant challenge choices must be unique.');
    }
    if (!choices.some(({ resolutionKind }) => resolutionKind === 'non-retry')) {
        throw grantInvalid('GRANT_NON_RETRY_RESOLUTION_REQUIRED', 'Grant challenge requires at least one non-retry resolution.');
    }
    const payload = {
        schemaVersion: 1,
        ...
        stateBinding: freezeCanonical(requested.stateBinding),
        choices,
...
export function createApprovalSubject(challenge, decision, options = {}) {
    challenge = assertGrantChallenge(challenge);
    const now = exactDate(options.now ?? new Date());
    if (exactTimestamp(challenge.expiresAt).getTime() < now.getTime()) {
        throw workflowError('GRANT_CHALLENGE_EXPIRED', 'Grant challenge has expired.', ExitCode.staleState);
    }
    const choice = challenge.choices.find(({ choiceId }) => choiceId === decision.choiceId);
    if (choice === undefined) {
        throw grantInvalid('GRANT_CHOICE_NOT_FOUND', 'Selected grant choice is unavailable.');
    }
    if (!GRANT_APPROVAL_METHODS.includes(decision.approvalMethod)) {
        throw grantInvalid('GRANT_APPROVAL_METHOD_INVALID', 'Selected grant approval method is unavailable.');
    }
    if (!STABLE_ID.test(decision.reasonCode) ||
        !choice.allowedReasonCodes.includes(decision.reasonCode)) {
        throw grantInvalid('GRANT_REASON_NOT_ALLOWED', 'Selected grant reason is not allowed for this choice.');
    }
    const reason = canonicalReason(decision.reason, choice.reasonRequired);
    if (!SESSION_NONCE.test(decision.sessionNonce)) {
        throw grantInvalid('GRANT_SESSION_NONCE_INVALID', 'Grant approval session nonce is invalid.');
    }
    return deepFreeze({
        schemaVersion: 1,
...
export function assertGrantChallenge(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
        ...
        throw grantInvalid('GRANT_CHALLENGE_INVALID', 'Grant challenge is malformed.');
    }
    ...
    if (sha256(canonicalJson(facts)) !== value.factsDigest) {
        throw grantInvalid('GRANT_CHALLENGE_INVALID', 'Grant challenge facts do not match their digest.');
    }
    const choices = value.choices.map(assertGrantChoice);
    if (new Set(choices.map(({ choiceId }) => choiceId)).size !== choices.length ||
        !choices.some(({ resolutionKind }) => resolutionKind === 'non-retry')) {
        throw grantInvalid('GRANT_CHALLENGE_INVALID', 'Grant challenge choices are invalid.');
    }
    const payload = {
        schemaVersion: 1,
        kind: 'grant-challenge.v1',
        ...
        facts,
        factsDigest: value.factsDigest,
        stateBinding: freezeCanonical(value.stateBinding),
        choices,
        issuedAt: value.issuedAt,
        expiresAt: value.expiresAt,
    };
    if (sha256(canonicalJson(payload)) !== value.challengeDigest) {
        throw grantInvalid('GRANT_CHALLENGE_INVALID', 'Grant challenge does not match its digest.');
    }
    return deepFreeze({
        ...payload,
...
export function approvalSubjectDigest(subject) {
    return sha256(canonicalJson(assertApprovalSubject(subject)));
}
export function assertApprovalSubject(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'schemaVersion',
            'kind',
        ...
        !GRANT_APPROVAL_METHODS.includes(value.approvalMethod) ||
        ...
        throw grantInvalid('GRANT_APPROVAL_SUBJECT_INVALID', 'Grant approval subject is malformed.');
    }
    exactTimestamp(value.expiresAt);
    return deepFreeze(freezeCanonical(value));
}
function createChoice(candidate, registry) {
    ...
    if (!STABLE_ID.test(candidate.transitionId) ||
        ...
        throw grantInvalid('GRANT_CANDIDATE_INVALID', 'Grant transition candidate is malformed.');
    }
    const normalized = registry.normalizeParameters(candidate.transitionId, candidate.parameters);
...
function assertGrantChoice(value) {
    if (!isRecord(value) ||
        !hasExactKeys(value, [
            'choiceId',
            'transitionId',
        ...
        !value.allowedReasonCodes.every((reasonCode) => typeof reasonCode === 'string' && STABLE_ID.test(reasonCode)) ||
    ...
    if (sha256(canonicalJson(parameters)) !== value.parameterDigest) {
        throw grantInvalid('GRANT_CHOICE_INVALID', 'Grant choice parameters do not match their digest.');
    }
    const choicePayload = {
        transitionId: value.transitionId,
        parameters,
        parameterDigest: value.parameterDigest,
        parameterSchemaDigest: value.parameterSchemaDigest,
        consequenceDigest: value.consequenceDigest,
    ...
    if (sha256(canonicalJson(choicePayload)) !== value.choiceId) {
        throw grantInvalid('GRANT_CHOICE_INVALID', 'Grant choice does not match its digest.');
    }
    return deepFreeze({
...
function canonicalReason(value, required) {
...
function grantInvalid(code, message) {
