...
const RECOVERY_RECORD_TEMPORARY =
  /^\.([0-9a-f]{64}\.json)\.(absent|[0-9a-f]{64})\.([0-9a-f]{64})\.([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.tmp$/;
const LEGACY_RECOVERY_RECORD_TEMPORARY =
  /^\.([0-9a-f]{64}\.json)\.([1-9][0-9]*)\.([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.tmp$/;
const PAYLOAD_KEYS = [
...
export type ControlPlaneRecoverySourceState =
  'RECOVERY_VERIFIED' | 'SWITCHED' | 'ROLLBACK_REQUIRED';

export interface ControlPlaneRecoveryGrantPayload {
  ...
  sourceTransactionState: ControlPlaneRecoverySourceState;
...
export interface ControlPlaneRecoveryGrantEnvelope {
  payload: ControlPlaneRecoveryGrantPayload;
  signature: string;
}

export interface ControlPlaneRecoveryReceipt {
  ...
  controlPlaneJournalDigestBefore: Sha256Digest;
  controlPlaneJournalDigestAfter: Sha256Digest;
  supervisorStateDigestBefore: Sha256Digest;
  supervisorStateDigestAfter: Sha256Digest;
  result: 'rolled-back';
  completedAt: string;
  receiptDigest: Sha256Digest;
}

export interface ControlPlaneRecoveryFailure {
  kind: 'control-plane-recovery-failure.v1';
  recoveryGrantId: string;
  sourceControlPlaneGrantId: string;
  stage: 'restart-verification';
  errorCode: string;
  selectedClosureDigest: Sha256Digest;
  selectedArtifactId: string;
  supervisorStateDigest: Sha256Digest;
  supervisorGeneration: number;
  controlPlaneJournalDigest: Sha256Digest;
  failedAt: string;
  failureDigest: Sha256Digest;
}

export interface PersistedControlPlaneRecoveryGrant {
  kind: 'persisted-control-plane-recovery-grant.v1';
  state: 'reserved' | 'completion-pending' | 'consumed' | 'expired' | 'failed';
  envelope: ControlPlaneRecoveryGrantEnvelope;
  envelopeDigest: Sha256Digest;
  prestateDigest: Sha256Digest;
  receipt: ControlPlaneRecoveryReceipt | null;
  failure: ControlPlaneRecoveryFailure | null;
  createdAt: string;
  updatedAt: string;
  recordDigest: Sha256Digest;
}

export type ControlPlaneRecoveryAuditEvent =
  'authorized' | 'expired' | 'failed' | 'rolled-back' | 'consumed';

export interface ControlPlaneRecoveryAuditRecord {
  ...
  operation: 'rollback-control-plane';
  sequence: 0 | 1 | 2;
  event: ControlPlaneRecoveryAuditEvent;
...
export interface ControlPlaneRecoveryStateBinding {
  repositoryId: string;
  sourceControlPlaneGrantId: string;
  previousClosureDigest: Sha256Digest;
  currentClosureDigest: Sha256Digest;
...
export function createControlPlaneRecoveryGrantPayload(
  binding: ControlPlaneRecoveryStateBinding & {
    issuedAt: string;
    humanSigner: string;
  },
): ControlPlaneRecoveryGrantPayload {
  const issuedAt = exactIso(binding.issuedAt, 'HARNESS_RECOVERY_GRANT_INVALID');
  const expiresAt = new Date(
    Date.parse(issuedAt) + CONTROL_PLANE_RECOVERY_GRANT_TTL_MS,
  ).toISOString();
...
export function canonicalControlPlaneRecoveryGrantPayload(
  payload: ControlPlaneRecoveryGrantPayload,
): string {
  return canonicalJson(normalizePayload(payload));
}

export function verifyControlPlaneRecoveryGrant(
  envelope: ControlPlaneRecoveryGrantEnvelope,
  input: {
    now: Date;
  ...
  if (
    ...
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_INVALID',
      'Recovery Grant envelope is malformed.',
    );
  }
  const payload = normalizePayload(envelope.payload);
  let verified = false;
  try {
    verified = input.verifyHumanSignature(
  ...
  if (!verified) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_SIGNATURE_INVALID',
      'Recovery Grant signature is invalid for HARNESS_RECOVERY_GRANT_V1.',
    );
  }
  const now = exactDate(input.now, 'HARNESS_RECOVERY_GRANT_CLOCK_INVALID');
  if (
    (input.enforceLive ?? true) &&
    now.getTime() < Date.parse(payload.issuedAt)
  ) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_NOT_YET_VALID',
      'Recovery Grant activation window has not started.',
      ExitCode.staleState,
    );
  }
  if (
    ...
    throw recoveryError(
      ...
      'Recovery Grant activation window has expired.',
      ExitCode.staleState,
    );
  }
...
export function controlPlaneRecoveryPrestateDigest(
  payload: ControlPlaneRecoveryGrantPayload,
): Sha256Digest {
...
export function createControlPlaneRecoveryReceipt(input: {
  ...
  supervisorStateDigestAfter: Sha256Digest;
  completedAt: string;
}): ControlPlaneRecoveryReceipt {
  const payload = normalizePayload(input.payload);
  assertDigest(input.poststateDigest, 'HARNESS_RECOVERY_RECEIPT_INVALID');
  assertDigest(
    input.controlPlaneJournalDigestAfter,
    'HARNESS_RECOVERY_RECEIPT_INVALID',
  );
  assertDigest(
    input.supervisorStateDigestAfter,
    'HARNESS_RECOVERY_RECEIPT_INVALID',
  );
  const receiptPayload = {
    kind: 'control-plane-recovery-receipt.v1' as const,
...
export function createControlPlaneRecoveryFailure(input: {
  ...
  assertDigest(
    input.controlPlaneJournalDigest,
    'HARNESS_RECOVERY_FAILURE_INVALID',
  );
  if (
    !Number.isSafeInteger(input.supervisorGeneration) ||
    input.supervisorGeneration < payload.supervisorGeneration
  ) {
    throw recoveryError(
      'HARNESS_RECOVERY_FAILURE_INVALID',
      'Recovery failure supervisor generation is invalid.',
    );
  }
  const failurePayload = {
    kind: 'control-plane-recovery-failure.v1' as const,
    recoveryGrantId: payload.grantId,
    sourceControlPlaneGrantId: payload.sourceControlPlaneGrantId,
    stage: 'restart-verification' as const,
...
export function createControlPlaneRecoveryAuditRecord(input: {
  ...
  receiptDigest: Sha256Digest | null;
  recordedAt: string;
}): ControlPlaneRecoveryAuditRecord {
  const payload = normalizePayload(input.payload);
  ...
  const sequence: 0 | 1 | 2 =
    input.event === 'authorized'
      ...
      : input.event === 'rolled-back' ||
        ...
        : 2;
  const identity = {
    kind: 'control-plane-recovery-audit-id.v1' as const,
    recoveryGrantId: payload.grantId,
    event: input.event,
    sequence,
  };
  const recordPayload = {
    kind: 'control-plane-recovery-audit.v1' as const,
...
export function reservePersistedControlPlaneRecoveryGrant(
  storageRoot: string,
  envelope: ControlPlaneRecoveryGrantEnvelope,
  at: string,
): PersistedControlPlaneRecoveryGrant {
  const payload = normalizePayload(envelope.payload);
  ...
  if (fs.lstatSync(target, { throwIfNoEntry: false })) {
    const existing = readPersistedControlPlaneRecoveryGrant(
      storageRoot,
      payload.grantId,
    );
    ...
    if (existing.state === 'consumed') throw recoveryAlreadyConsumed();
    if (existing.state === 'failed') throw recoveryAlreadyFailed();
...
export function consumePersistedControlPlaneRecoveryGrant(
  storageRoot: string,
  input: {
    recoveryGrantId: string;
  ...
  const current = readPersistedControlPlaneRecoveryGrant(
    storageRoot,
    input.recoveryGrantId,
  );
  if (current.state === 'consumed') throw recoveryAlreadyConsumed();
  if (current.state === 'failed') throw recoveryAlreadyFailed();
  if (current.state === 'expired') {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_EXPIRED',
      'Expired Recovery Grant cannot be consumed.',
      ExitCode.staleState,
    );
  }
  if (current.state !== 'completion-pending' || current.receipt === null) {
    throw recoveryError(
      'HARNESS_RECOVERY_CONSUMPTION_NOT_PREPARED',
      'Recovery Grant consumption requires a durable completion receipt before terminalization.',
      ExitCode.conflict,
    );
  }
  if (current.recordDigest !== input.expectedRecordDigest) {
    throw recoveryError(
      'HARNESS_RECOVERY_RECORD_CAS_MISMATCH',
      'Recovery Grant reservation changed before consumption.',
      ExitCode.staleState,
    );
  }
  ...
  replaceAtomic(
    recoveryRecordPath(storageRoot, input.recoveryGrantId, false),
...
export function preparePersistedControlPlaneRecoveryGrantConsumption(
  storageRoot: string,
  input: {
    recoveryGrantId: string;
    expectedRecordDigest: Sha256Digest;
  ...
  if (current.state === 'expired') {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_EXPIRED',
      'Expired Recovery Grant cannot prepare consumption.',
      ExitCode.staleState,
    );
  }
  const receipt = normalizeReceipt(input.receipt);
  if (
    receipt.recoveryGrantId !== current.envelope.payload.grantId ||
    receipt.prestateDigest !== current.prestateDigest
  ) {
    throw recoveryError(
      'HARNESS_RECOVERY_RECEIPT_INVALID',
      'Recovery receipt does not consume the exact reserved prestate.',
    );
  }
  if (current.state === 'completion-pending') {
    if (canonicalJson(current.receipt) !== canonicalJson(receipt)) {
      throw recoveryError(
        'HARNESS_RECOVERY_RECEIPT_INVALID',
        'Prepared Recovery Grant completion differs from the exact durable receipt.',
      );
    }
    return current;
  }
  if (current.recordDigest !== input.expectedRecordDigest) {
    throw recoveryError(
      'HARNESS_RECOVERY_RECORD_CAS_MISMATCH',
      'Recovery Grant reservation changed before completion preparation.',
      ExitCode.staleState,
    );
  }
  const recordPayload = {
...
export function failPersistedControlPlaneRecoveryGrant(
  storageRoot: string,
  input: {
    recoveryGrantId: string;
    expectedRecordDigest: Sha256Digest;
    failure: ControlPlaneRecoveryFailure;
  },
): PersistedControlPlaneRecoveryGrant {
  const current = readPersistedControlPlaneRecoveryGrant(
    storageRoot,
  ...
  if (current.state !== 'reserved') {
    throw recoveryError(
      'HARNESS_RECOVERY_FAILURE_STATE_INVALID',
      'Only a reserved Recovery Grant can terminalize after restart verification failure.',
      ExitCode.conflict,
    );
  }
  if (current.recordDigest !== input.expectedRecordDigest) {
    throw recoveryError(
      'HARNESS_RECOVERY_RECORD_CAS_MISMATCH',
      'Recovery Grant reservation changed before failure terminalization.',
      ExitCode.staleState,
    );
  }
  const failure = normalizeFailure(input.failure);
  if (
    failure.recoveryGrantId !== current.envelope.payload.grantId ||
    failure.sourceControlPlaneGrantId !==
      current.envelope.payload.sourceControlPlaneGrantId
  ) {
    throw recoveryError(
      'HARNESS_RECOVERY_FAILURE_INVALID',
      'Recovery failure does not bind the exact reserved Grant.',
    );
  }
  const recordPayload = {
    kind: current.kind,
...
export function expirePersistedControlPlaneRecoveryGrant(
  storageRoot: string,
  input: {
    recoveryGrantId: string;
    expectedRecordDigest: Sha256Digest;
  ...
  if (current.state !== 'reserved') {
    throw recoveryError(
      'HARNESS_RECOVERY_COMPLETION_OBLIGATORY',
      'A Recovery Grant with a durable completion receipt cannot expire before terminal audit completion.',
      ExitCode.conflict,
    );
  }
  if (current.recordDigest !== input.expectedRecordDigest) {
    throw recoveryError(
      'HARNESS_RECOVERY_RECORD_CAS_MISMATCH',
      'Recovery Grant reservation changed before expiry terminalization.',
      ExitCode.staleState,
    );
  }
  const expiredAt = exactIso(
    input.expiredAt,
    'HARNESS_RECOVERY_RECORD_INVALID',
  );
  if (Date.parse(expiredAt) < Date.parse(current.envelope.payload.expiresAt)) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_NOT_EXPIRED',
      'Recovery Grant cannot terminalize before its activation window ends.',
      ExitCode.conflict,
    );
  }
  const recordPayload = {
...
export function readPersistedControlPlaneRecoveryGrant(
  storageRoot: string,
  recoveryGrantId: string,
): PersistedControlPlaneRecoveryGrant {
  const value = readCanonicalRecord(
    recoveryRecordPath(storageRoot, recoveryGrantId, false),
  );
  if (
    !isRecord(value) ||
    !hasExactKeys(value, [
  ...
  if (
    payload.grantId !== recoveryGrantId ||
  ...
  const createdAt = exactIso(
    value.createdAt,
    'HARNESS_RECOVERY_RECORD_CORRUPT',
  );
  ...
  if (Date.parse(updatedAt) < Date.parse(createdAt))
    throw recoveryRecordCorrupt();
  const receipt =
    value.receipt === null ? null : normalizeReceipt(value.receipt);
  const failure =
    value.failure === null ? null : normalizeFailure(value.failure);
  if (
    ((value.state === 'reserved' || value.state === 'expired') &&
  ...
  return deepFreeze(
    structuredClone(value as unknown as PersistedControlPlaneRecoveryGrant),
  );
}

export function findPersistedControlPlaneRecoveryGrantForSource(
  storageRoot: string,
  sourceControlPlaneGrantId: string,
): PersistedControlPlaneRecoveryGrant | null {
  ...
  if (
    entries.some(
      (entry) =>
        !RECOVERY_RECORD_NAME.test(entry) &&
  ...
  const matches = entries
    ...
    .map((entry) =>
      readPersistedControlPlaneRecoveryGrant(
        storageRoot,
        recoveryGrantIdFromRecordPath(directory, entry),
      ),
    )
    .filter(
      (record) =>
        record.envelope.payload.sourceControlPlaneGrantId ===
        sourceControlPlaneGrantId,
    );
  const active = matches.filter(
    ({ state }) => state !== 'expired' && state !== 'failed',
  );
  if (active.length > 1) throw recoveryRecordCorrupt();
...
export function throwControlPlaneRecoveryAlreadyConsumed(): never {
  throw recoveryAlreadyConsumed();
}

export function throwControlPlaneRecoveryFailed(): never {
  throw recoveryAlreadyFailed();
}

function normalizePayload(raw: unknown): ControlPlaneRecoveryGrantPayload {
  if (!isRecord(raw) || !hasExactKeys(raw, PAYLOAD_KEYS)) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_INVALID',
      'Recovery Grant payload has unknown, missing, or malformed fields.',
    );
  }
  if (
    raw.kind !== 'harness-recovery-grant.v1' ||
    ...
    !['RECOVERY_VERIFIED', 'SWITCHED', 'ROLLBACK_REQUIRED'].includes(
      String(raw.sourceTransactionState),
    ) ||
    ...
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_INVALID',
      'Recovery Grant payload has invalid authority or state bindings.',
    );
  }
  ...
  if (ttl <= 0 || ttl > CONTROL_PLANE_RECOVERY_GRANT_TTL_MS) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_TTL_INVALID',
      'Recovery Grant must use a positive activation window of at most five minutes.',
    );
  }
  const expectedGrantId = recoveryGrantId({
    ...
    sourceTransactionState:
      raw.sourceTransactionState as ControlPlaneRecoverySourceState,
  ...
  if (raw.grantId !== expectedGrantId) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_INVALID',
      'Recovery Grant identity does not match its signed activation identity.',
    );
  }
  return {
    kind: raw.kind,
...
function normalizeReceipt(raw: unknown): ControlPlaneRecoveryReceipt {
  if (
    !isRecord(raw) ||
    !hasExactKeys(raw, [
      'completedAt',
    ...
    throw recoveryError(
      'HARNESS_RECOVERY_RECEIPT_INVALID',
      'Control-plane recovery receipt is malformed.',
    );
  }
  ...
  if (receiptDigest !== canonicalDigest(payload)) {
    throw recoveryError(
      'HARNESS_RECOVERY_RECEIPT_INVALID',
      'Control-plane recovery receipt digest is invalid.',
    );
  }
  return structuredClone(raw) as unknown as ControlPlaneRecoveryReceipt;
}

function normalizeFailure(raw: unknown): ControlPlaneRecoveryFailure {
  if (
    !isRecord(raw) ||
    !hasExactKeys(raw, [
      'controlPlaneJournalDigest',
    ...
    throw recoveryError(
      'HARNESS_RECOVERY_FAILURE_INVALID',
      'Persisted Recovery Grant failure is malformed.',
    );
  }
  ...
  if (failureDigest !== canonicalDigest(payload)) {
    throw recoveryError(
      'HARNESS_RECOVERY_FAILURE_INVALID',
      'Persisted Recovery Grant failure digest is invalid.',
    );
  }
  return structuredClone({
    ...raw,
    failedAt,
  }) as unknown as ControlPlaneRecoveryFailure;
}

function recoveryGrantId(
  input: ControlPlaneRecoveryStateBinding & {
    issuedAt: string;
    expiresAt: string;
    humanSigner: string;
  },
  ...
  assertIdentifier(
    input.sourceControlPlaneGrantId,
    'HARNESS_RECOVERY_GRANT_INVALID',
  );
...
function recoveryRecordsDirectory(
  storageRoot: string,
  create: boolean,
): string {
  ...
  if (root !== storageRoot) {
    throw recoveryError(
      'HARNESS_RECOVERY_STATE_UNSAFE',
      'Recovery state root must be an exact absolute path.',
  ...
  if (create && !fs.lstatSync(directory, { throwIfNoEntry: false })) {
    try {
      ...
      if (!isNodeCode(error, 'EEXIST')) throw recoveryStateUnsafe();
    }
  }
  return directory;
}
...
function recoveryRecordPath(
  ...
  if (!RECOVERY_GRANT_ID.test(recoveryGrantId)) {
    throw recoveryError(
      'HARNESS_RECOVERY_GRANT_INVALID',
      'Recovery Grant ID is malformed.',
    );
  }
  ...
  return path.join(
    directory,
    `${rawDigest(`control-plane-recovery\0${recoveryGrantId}`).slice('sha256:'.length)}.json`,
  );
}

function recoveryGrantIdFromRecordPath(
  directory: string,
  entry: string,
): string {
  ...
  if (entry !== expected) throw recoveryRecordCorrupt();
  return grantId;
}
...
function readCanonicalRecord(
  ...
  if (inspected.value === null) throw recoveryRecordCorrupt();
  return inspected.value;
}

function inspectPrivateRecordFile(
  filePath: string,
  allowedLinkCounts: readonly number[],
): { bytes: string; raw: Buffer; stats: fs.Stats; value: unknown | null } {
  ...
  try {
    ...
    if (isWorkflowCode(error)) throw error;
    throw recoveryRecordCorrupt();
  } finally {
...
function replaceAtomic(
  filePath: string,
  bytes: string,
  expectedOldRecordDigest: Sha256Digest | null,
  nextRecordDigest: Sha256Digest,
): void {
  const directory = path.dirname(filePath);
  const expectation: RecoveryRecordReconcileExpectation = {
  ...
  const temporary = path.join(
    ...
    `.${path.basename(filePath)}.${expected}.${nextRecordDigest.slice('sha256:'.length)}.${crypto.randomUUID()}.tmp`,
  );
  try {
...
function assertExactPublishedRecord(
  filePath: string,
  expectedRecordDigest: Sha256Digest,
...
interface RecoveryRecordReconcileExpectation {
  targetName: string;
  expectedOldRecordDigest: Sha256Digest | null;
  nextRecordDigest: Sha256Digest;
  bytes: string;
...
function reconcileRecoveryRecordTemporaries(
  directory: string,
  expectation?: RecoveryRecordReconcileExpectation,
): void {
  assertPrivateDirectory(directory);
  ...
  for (const match of recognized) {
    ...
    if (
      expectation?.targetName === targetName &&
      match.current !== null &&
    ...
    if (inspected.stats.nlink !== 1) throw recoveryRecordCorrupt();
    ...
    if (!isRecord(inspected.value)) {
      ...
      if (match.current !== null) {
        ...
        if (!targetStats && namedExpected === 'absent') continue;
    ...
    if (!targetStats) {
      ...
      if (match.current?.[2] === 'absent') continue;
      throw recoveryRecordCorrupt();
    }
  ...
  if (
    ...
    if (stats.nlink !== 1) throw recoveryRecordCorrupt();
  }
}

function reconcileCleanupQuarantine(recordsDirectory: string): void {
  const quarantineRoot = path.join(
    path.dirname(recordsDirectory),
  ...
  for (const entry of fs.readdirSync(quarantineRoot).sort()) {
    ...
    const ambiguousTargets = contents.filter((name) =>
      /^ambiguous-target-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(
        name,
      ),
    );
    ...
    const authoritative = contents.filter(
      (name) => !ambiguousTargets.includes(name),
    );
    if (
      canonicalJson(authoritative) !== canonicalJson(['residue']) &&
    ...
    const target = path.join(
      recordsDirectory,
      expectedRecoveryRecordName(inspected.value),
    );
    detachQuarantinedHardLink(
      operationDirectory,
      residue,
      target,
      inspected.value,
...
interface RecoveryPreparationMatch {
  current: RegExpExecArray | null;
  legacy: RegExpExecArray | null;
}

function recoverExactPartialFinal(
  directory: string,
  expectation: RecoveryRecordReconcileExpectation | undefined,
): void {
  if (expectation === undefined) return;
  const target = path.join(directory, expectation.targetName);
  const stats = fs.lstatSync(target, { throwIfNoEntry: false });
  if (stats === undefined || stats.nlink !== 1) return;
...
function expectationMatchesPreparation(
  expectation: RecoveryRecordReconcileExpectation | undefined,
  targetName: string,
  match: RecoveryPreparationMatch,
): boolean {
  if (expectation === undefined || expectation.targetName !== targetName) {
    return false;
  }
  if (match.legacy !== null) return true;
  const expected =
    expectation.expectedOldRecordDigest === null
      ? 'absent'
      : expectation.expectedOldRecordDigest.slice('sha256:'.length);
  return (
    match.current?.[2] === expected &&
...
function assertPreparedRecord(
  value: unknown,
  targetName: string,
  namedNextDigest: string | undefined,
...
function isStrictBytePrefix(prefix: Buffer, exact: string): boolean {
...
function isRecoverableLegacyFinalPrefix(
  prefix: Buffer,
  exact: string,
): boolean {
  if (!isStrictBytePrefix(prefix, exact)) return false;
  let expected: unknown;
  try {
    expected = JSON.parse(exact) as unknown;
  } catch {
    return false;
  }
  if (
...
function unlinkExactSingleFile(filePath: string, expected: fs.Stats): void {
...
function unlinkExactAlias(alias: string, anchor: string): void {
  ...
  if (!isRecord(value)) throw recoveryRecordCorrupt();
  const quarantined = quarantineVerifiedPath(alias, aliasStats, [2], {
    filePath: anchor,
    stats: anchorBefore,
  });
  detachQuarantinedHardLink(
    quarantined.operationDirectory,
    quarantined.residue,
    anchor,
    value,
  );
...
function quarantineVerifiedPath(
  filePath: string,
  expected: fs.Stats,
  allowedLinkCounts: readonly number[],
  anchor?: { filePath: string; stats: fs.Stats },
): { operationDirectory: string; residue: string } {
  ...
  const operationDirectory = fs.mkdtempSync(
    path.join(quarantineRoot, 'cleanup-'),
  );
...
function detachQuarantinedHardLink(
  ...
  if (path.basename(target) !== expectedName) throw recoveryRecordCorrupt();
  ...
  if (
    canonicalJson(residueValue) !== canonicalJson(value) ||
  ...
  if (targetBeforePublish !== undefined) {
    ...
    if (inspectedTarget.value === null) {
      ...
      const ambiguous = path.join(
        operationDirectory,
        `ambiguous-target-${crypto.randomUUID()}`,
      );
      try {
  ...
  if (targetBeforePublish === undefined) {
    ...
    assertExactPublishedRecord(
      target,
      value.recordDigest as Sha256Digest,
      bytes,
    );
  } else {
...
function expectedRecoveryRecordName(value: Record<string, unknown>): string {
  if (
    !isRecord(value.envelope) ||
    !isRecord(value.envelope.payload) ||
    typeof value.envelope.payload.grantId !== 'string' ||
...
function assertDigest(
  ...
  if (!isDigest(value)) throw recoveryError(code, 'Expected a SHA-256 digest.');
}
...
function recoveryAlreadyConsumed(): Error & { code: string } {
  return recoveryError(
    'HARNESS_RECOVERY_GRANT_ALREADY_CONSUMED',
    'Recovery Grant is a consumed one-shot tombstone and cannot replay.',
    ExitCode.conflict,
  );
}

function recoveryAlreadyFailed(): Error & { code: string } {
  return recoveryError(
    'HARNESS_RECOVERY_GRANT_FAILED',
    'Recovery Grant terminalized after restart verification failed and cannot replay.',
    ExitCode.conflict,
  );
}

function recoveryRecordCorrupt(): Error & { code: string } {
  return recoveryError(
    'HARNESS_RECOVERY_RECORD_CORRUPT',
    'Persisted Recovery Grant or receipt failed canonical integrity verification.',
    ExitCode.verification,
  );
}

function recoveryStateUnsafe(): Error & { code: string } {
  return recoveryError(
    'HARNESS_RECOVERY_STATE_UNSAFE',
    'Recovery state must remain in exact private non-symlink storage.',
    ExitCode.unsafeEnvironment,
  );
}
...
function recoveryError(
  code: string,
  message: string,
  exitCode: ExitCodeValue = ExitCode.guard,
): ReturnType<typeof workflowError> {
  return workflowError(code, message, exitCode);
}

function isWorkflowCode(error: unknown): boolean {
