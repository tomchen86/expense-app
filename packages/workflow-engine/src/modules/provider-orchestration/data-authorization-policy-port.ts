 ...
 * remains a caller-owned type parameter so the core contract does not own a
 * built-in provider registry or host execution configuration.
 */
...
/**
 * Core-owned current-policy reader port. Historical policy readers are
 * deliberately excluded so an old snapshot grammar cannot become live
 * authorization.
 */
