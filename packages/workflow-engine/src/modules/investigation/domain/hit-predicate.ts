...
import type { ScanHitContextWindow } from './investigation-scanner.ts';
...
export type HitPredicate =
  | Readonly<{ kind: 'contains'; literal: string }>
  | Readonly<{ kind: 'containsToken'; literal: string }>
  | Readonly<{ kind: 'beforeMatchContains'; literal: string }>
  | Readonly<{ kind: 'afterMatchContains'; literal: string }>
  | Readonly<{ kind: 'all'; operands: readonly HitPredicate[] }>
  | Readonly<{ kind: 'any'; operands: readonly HitPredicate[] }>
  | Readonly<{ kind: 'not'; operand: HitPredicate }>;

export type HitPredicateSubject = Readonly<{
  window: ScanHitContextWindow | null;
  matchOffset: number;
  matchLength: number;
}>;
...
export function parseHitPredicate(value: unknown): HitPredicate {
 ...
 */
export function evaluateHitPredicate(
  predicate: HitPredicate,
  subject: HitPredicateSubject,
): boolean {
...
function evaluate(
  ...
  switch (predicate.kind) {
    ...
    case 'not':
      ...
      return (
        subject.window?.truncated === false &&
...
function containsToken(text: string, literal: string): boolean {
  let from = 0;
  for (;;) {
...
function sliceBeforeMatch(text: string, subject: HitPredicateSubject): string {
  const offset = subject.matchOffset - (subject.window?.byteOffset ?? 0);
  return offset <= 0 ? '' : text.slice(0, offset);
}

function sliceAfterMatch(text: string, subject: HitPredicateSubject): string {
  const offset =
    subject.matchOffset -
...
function parseNode(
  ...
  if ((LEAF_OPERATORS as readonly string[]).includes(operator)) {
    return Object.freeze({
      kind: operator as (typeof LEAF_OPERATORS)[number],
  ...
  if (operator === 'all' || operator === 'any') {
    ...
    return Object.freeze({
      ...
      operands: Object.freeze(
        operand.map((entry) => parseNode(entry, depth + 1, budget)),
      ),
    });
  }
  if (operator === 'not') {
...
function predicateInvalid(message: string) {
