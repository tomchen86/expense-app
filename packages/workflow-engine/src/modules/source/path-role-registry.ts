...
export type PathRoleRegistry = Readonly<{
  schemaVersion: 1;
  kind: 'path-role-registry';
  /** Sorted longest-pattern-first so the narrowest claim resolves. */
  rules: ReadonlyArray<Readonly<{ pattern: string; role: PathRole }>>;
}>;

export type PathRoleResolution = Readonly<
  | { registered: true; role: PathRole; pattern: string }
  | { registered: false; role: null; pattern: null }
>;

const ROLE_SET = new Set<string>(PATH_ROLES);
 ...
 */
const COMPRESSIBLE_ROLES = new Set<PathRole>(['ordinary']);

export function parsePathRoleRegistry(value: unknown): PathRoleRegistry {
  if (
    !isRecord(value) ||
  ...
  const claimed = new Map<string, PathRole>();
  const rules: Array<{ pattern: string; role: PathRole }> = [];
  for (const [role, patterns] of Object.entries(value.roles)) {
    if (!ROLE_SET.has(role)) {
      throw registryInvalid(
        `Path role registry declares unknown role ${role}.`,
      );
    }
    ...
    for (const raw of patterns) {
      ...
      try {
        ...
        throw registryInvalid(
          `Path role ${role} declares an unusable pattern ${JSON.stringify(raw)}.`,
        );
      }
      const existing = claimed.get(pattern);
      if (existing !== undefined) {
        throw registryInvalid(
          `Pattern ${pattern} is claimed by both ${existing} and ${role}.`,
        );
      }
      claimed.set(pattern, role as PathRole);
      rules.push({ pattern, role: role as PathRole });
    }
  }
  if (rules.length === 0) {
...
export function resolvePathRole(
 ...
 * than assurance.
 */
export function compressionEligible(resolution: PathRoleResolution): boolean {
...
function registryInvalid(message: string) {
