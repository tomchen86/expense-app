...
/**
 * A path identity that preserves the exact raw tracked bytes plus a best-effort
 * UTF-8 rendering. `utf8` is null when the raw bytes are not valid UTF-8; a
 * text-oriented selector then cannot match, but a raw-byte selector still can.
 */
export type MutationPathIdentity = {
  rawBase64: string;
  utf8: string | null;
};

/**
 * Reviewed selectors. `path-prefix` and `exact-path` match the decoded UTF-8
 * identity segment-aware; `raw-prefix` matches the exact leading tracked bytes so
 * an invalid-UTF-8 path is still classifiable.
 */
export type MutationClassSelector =
  | { kind: 'path-prefix'; path: string }
  | { kind: 'exact-path'; path: string }
  | { kind: 'raw-prefix'; rawBase64: string };

export type MutationClassRule = {
  ruleId: string;
  mutationClass: MutationClass;
  selector: MutationClassSelector;
};

export type MutationClassPolicy = {
  policyDigest: string;
  rules: MutationClassRule[];
};
...
export function createMutationClassPolicy(input: {
  ...
  if (
    ...
    input === null ||
    Array.isArray(input) ||
    !hasExactKeys(input as unknown as Record<string, unknown>, ['rules'])
  ) {
 ...
 */
export function assertMutationClassPolicy(policy: MutationClassPolicy): {
  rules: MutationClassRule[];
  policyDigest: string;
} {
  if (
  ...
  if (
    ...
    throw policyInvalid(
      'Mutation-class policy digest does not match its rules.',
    );
  }
...
export function classifyMutationPath(
  policy: MutationClassPolicy,
  path: MutationPathIdentity,
): MutationClassification {
  const { rules } = assertMutationClassPolicy(policy);
  const identity = assertPathIdentity(path);
  const matched = new Set<MutationClass>();
...
export function assertPathIdentity(
  value: unknown,
  invalid: ErrorFactory = policyInvalid,
): { raw: Buffer; utf8: string | null } {
...
function normalizePolicyRules(rules: unknown): {
  rules: MutationClassRule[];
  policyDigest: string;
} {
  ...
  const normalized: MutationClassRule[] = [];
  for (const rule of rules) {
    if (
      typeof rule !== 'object' ||
      rule === null ||
      Array.isArray(rule) ||
      !hasExactKeys(rule as Record<string, unknown>, [
        'ruleId',
        'mutationClass',
        'selector',
      ]) ||
    ...
    normalized.push(structuredClone(rule) as MutationClassRule);
  }
  normalized.sort((left, right) =>
    left.ruleId < right.ruleId ? -1 : left.ruleId > right.ruleId ? 1 : 0,
  );
  const policyDigest = sha256(
    canonicalJson({ schema: POLICY_SCHEMA, rules: normalized }),
  );
  return { rules: normalized, policyDigest };
}

function isValidSelector(value: unknown): value is MutationClassSelector {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    return false;
  }
  ...
  switch (selector.kind) {
    ...
    case 'raw-prefix':
      if (
        !hasExactKeys(selector, ['kind', 'rawBase64']) ||
...
function selectorMatches(
  selector: MutationClassSelector,
  identity: { raw: Buffer; utf8: string | null },
): boolean {
  switch (selector.kind) {
    case 'path-prefix':
      return (
    ...
    case 'raw-prefix': {
      ...
      return (
        identity.raw.byteLength >= prefix.byteLength &&
...
function assertCanonicalNonemptyBase64(
  value: unknown,
  invalid: ErrorFactory = policyInvalid,
): Buffer {
