...
export type DiscriminationVerdict = Readonly<{
  admissible: boolean;
  memberCount: number;
...
export function assessPredicateDiscrimination(
  predicate: HitPredicate,
  members: readonly HitPredicateSubject[],
  controls: readonly HitPredicateSubject[],
  options: { threshold?: number } = {},
): DiscriminationVerdict {
  const threshold = options.threshold ?? DISCRIMINATION_THRESHOLD;
  if (!Number.isFinite(threshold) || threshold < 0 || threshold > 1) {
    throw workflowError(
      'PREDICATE_DISCRIMINATION_INVALID',
      'A discrimination threshold is a proportion between zero and one.',
      ExitCode.usage,
    );
  }

  const reasons: string[] = [];
  const membersMatched = members.filter((member) =>
    evaluateHitPredicate(predicate, member),
  ).length;
  ...
  const comparable = controls.filter(
    (control) => control.window !== null && control.window.utf8 !== null,
  );
  const controlRejected = comparable.filter(
    (control) => !evaluateHitPredicate(predicate, control),
  ).length;
  const rejectionRate =
    comparable.length === 0 ? 0 : controlRejected / comparable.length;
  ...
  if (membersMatched !== members.length) {
    reasons.push(
      `member-not-matched:${members.length - membersMatched} of ${members.length} members fail the predicate`,
    );
  }
  if (comparable.length === 0) {
    reasons.push(
      'no-control-hits:discrimination cannot be shown against an empty comparison set',
    );
    ...
    reasons.push(
      'vacuous-predicate:no control hit is rejected, so the predicate is true of every hit',
    );
  } else if (rejectionRate < threshold) {
    reasons.push(
      `weak-discrimination:${controlRejected}/${comparable.length} controls rejected, below the ${threshold} bar`,
    );
  }
