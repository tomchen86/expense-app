...
import {
  ...
  activeBootstrapSidecarPromotionPinTxIds,
  inFlightPersistedEngineAdoptionTxIds,
  ...
  recordBootstrapSidecarWorkspaceMaterialized,
...
export type BootstrapHumanSignatureVerifier = (
  payload: string,
  signature: string,
  signer: string,
  namespace: string,
...
export interface BootstrapDependencies {
  ...
  testHooks?: {
    ...
    afterWorktreeReceiptPersistedBeforeSidecar?: () => void;
    afterAdoptionReceiptPersistedBeforeSidecar?: () => void;
  };
}
...
export interface PersistedWipBundle {
  ...
  tracked: {
    paths: string[];
    workingTreePatchBase64: string;
    stagedPatchBase64: string;
    workingTreePatchDigest: Sha256Digest;
    stagedPatchDigest: Sha256Digest;
  };
  untracked: PersistedUntrackedFile[];
  session: {
    snapshotPath: string;
    contentBase64: string;
    contentDigest: Sha256Digest;
  ...
  statusPorcelainBase64: string;
  capturedAt: string;
  bundleDigest: Sha256Digest;
}

export interface WipRestoreReceipt {
...
export interface ChildWorktreeMaterializationReceipt {
...
export interface LocalEngineBinding {
...
export interface BootstrapAdoptionStepReceipt {
  kind: 'bootstrap-adoption-step-receipt.v1';
  txId: string;
  action: string;
  beforeJournalDigest: Sha256Digest;
  afterJournalDigest: Sha256Digest;
...
export function discoverBootstrapUntrackedAllowlist(
  repositoryRoot: string,
): string[] {
 ...
 */
export function persistTrustedBootstrapSessionSnapshot(
  storageRoot: string,
  session: unknown,
): string {
  ...
  if (content.length > MAX_SESSION_BYTES) {
    throw workflowError(
      'BOOTSTRAP_SESSION_SNAPSHOT_TOO_LARGE',
      'Durable workflow session exceeds the bootstrap size limit.',
      ExitCode.guard,
    );
  }
  ...
  const observed = readPrivateBytes(
    snapshotPath,
    'BOOTSTRAP_SESSION_SNAPSHOT_NOT_FOUND',
    MAX_SESSION_BYTES,
  );
  if (!observed.equals(content)) {
    throw workflowError(
      'BOOTSTRAP_SESSION_SNAPSHOT_CONFLICT',
      'Content-addressed durable session snapshot differs from its source.',
      ExitCode.staleState,
    );
  }
  return snapshotPath;
...
export function capturePersistedWipIntervention(
  storageRoot: string,
  input: {
    repositoryRoot: string;
    parent: Parameters<typeof persistInterventionPlan>[1]['parent'];
    interventionChangeId: string;
    childWorkspacePath: string;
    changeRef: string;
    untrackedAllowlist: string[];
  ...
  if (Buffer.byteLength(input.pendingIntent, 'utf8') > MAX_SESSION_BYTES) {
    throw workflowError(
      'BOOTSTRAP_PENDING_INTENT_INVALID',
      'Pending intent exceeds the bootstrap size limit.',
      ExitCode.guard,
    );
  }
  ...
  if (
    workingTreePatch.length > MAX_PATCH_BYTES ||
    stagedPatch.length > MAX_PATCH_BYTES
  ) {
    throw workflowError(
      'BOOTSTRAP_TRACKED_PATCH_TOO_LARGE',
      'Tracked WIP patch exceeds the bootstrap size limit.',
      ExitCode.guard,
    );
  }
  const trackedPaths = parseNulPaths(
    gitBuffer(repositoryRoot, ['diff', '--name-only', '-z', 'HEAD', '--']),
  );
  ...
  if (canonicalJson(allowlist) !== canonicalJson(observedUntracked)) {
    throw workflowError(
      'BOOTSTRAP_UNTRACKED_ALLOWLIST_MISMATCH',
      'Untracked allowlist must exactly cover every non-ignored untracked path.',
      ExitCode.guard,
      { details: { expected: observedUntracked, actual: allowlist } },
    );
  }
  const untracked = captureUntrackedFiles(repositoryRoot, allowlist);
  const sessionSnapshotPath = canonicalPlainFile(
    assertPersistenceOwnedPath(
      storageRoot,
      input.sessionSnapshotPath,
      'BOOTSTRAP_SESSION_SNAPSHOT_OUTSIDE_STATE',
    ),
    MAX_SESSION_BYTES,
    'BOOTSTRAP_SESSION_SNAPSHOT_UNSAFE',
  );
  ...
  const bundlePayload = {
    kind: 'harness-wip-bundle.v1' as const,
  ...
  const bundle: PersistedWipBundle = {
    ...bundlePayload,
  ...
  const intervention = persistInterventionPlan(storageRoot, {
    ...
    childWorkspace: {
      ...
      childWorkspacePath: canonicalPlannedWorkspacePath(
...
export function readPersistedWipBundle(
  ...
  try {
    ...
    if (
      ...
      ) !== bundle.tracked.workingTreePatchDigest ||
    ...
    if (
      canonicalJson(bundle.tracked.paths) !==
...
export function restorePersistedWipBundle(
  storageRoot: string,
  input: {
    parentChangeId: string;
    repositoryRoot: string;
  ...
  const bundle = readPersistedWipBundle(
    storageRoot,
    intervention.checkpoint.checkpointId,
  );
  const repositoryRoot = canonicalRepositoryRoot(input.repositoryRoot);
  const sessionPath = assertPersistenceOwnedPath(
    storageRoot,
    input.sessionSnapshotPath,
    'BOOTSTRAP_SESSION_SNAPSHOT_OUTSIDE_STATE',
  );
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_RESTORE_BINDING_MISMATCH',
      'Restore target differs from the signed intervention checkpoint.',
      ExitCode.verification,
    );
  }
  assertCheckpointMatchesBundle(intervention, bundle);
  if (
    gitText(repositoryRoot, ['rev-parse', 'HEAD']).trim() !== bundle.baseOid
  ) {
    throw workflowError(
      'BOOTSTRAP_RESTORE_BASE_MISMATCH',
      'Parent HEAD changed since checkpoint capture.',
      ExitCode.staleState,
    );
  }
  ...
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_RESTORE_WORKTREE_NOT_CLEAN',
      'WIP restore requires the exact clean checkpoint base.',
      ExitCode.conflict,
    );
  }
  if (!alreadyRestored) {
...
export function materializeInterventionChildWorktree(
  storageRoot: string,
  input: {
    parentChangeId: string;
    repositoryRoot: string;
  ...
  dependencies: BootstrapDependencies,
): ChildWorktreeMaterializationReceipt {
  const verifier = requireHumanVerifier(dependencies);
  const now = bootstrapNow(dependencies);
  const intervention = readPersistedIntervention(
    storageRoot,
  ...
  if (repositoryRoot !== metadata.parentWorkspacePath) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKTREE_PARENT_MISMATCH',
      'Child worktree request is not bound to the persisted parent workspace.',
      ExitCode.verification,
    );
  }
  ...
  const receiptPath = bootstrapReceiptPath(
    storageRoot,
    `worktree-${metadata.workspaceId}`,
  );
  if (fs.existsSync(receiptPath)) {
    const receipt =
      readBootstrapReceipt<ChildWorktreeMaterializationReceipt>(receiptPath);
  ...
  if (fs.lstatSync(metadata.childWorkspacePath, { throwIfNoEntry: false })) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKTREE_PATH_OCCUPIED',
      'Child worktree path became occupied after reservation.',
      ExitCode.conflict,
    );
  }
  if (gitRefExists(repositoryRoot, metadata.changeRef)) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKTREE_REF_EXISTS',
      'Reserved child change ref already exists.',
      ExitCode.conflict,
    );
  }
  ...
  const receiptPayload = {
    kind: 'intervention-child-worktree-materialized.v1' as const,
  ...
  const receipt: ChildWorktreeMaterializationReceipt = {
    ...receiptPayload,
    receiptDigest: canonicalDigest(receiptPayload),
  };
  persistBootstrapReceipt(
...
export function initializeLocalEngineBinding(
  storageRoot: string,
  bindingPath: string,
  input: {
    parentChangeId: string;
  ...
  const parentWorkspacePath = canonicalRepositoryRoot(
    input.parentWorkspacePath,
  );
  const expectedParentBranch = `refs/heads/work/${input.parentChangeId}`;
  if (
    input.parentBranch !== expectedParentBranch ||
    ...
    throw workflowError(
      'BOOTSTRAP_BINDING_PARENT_SCOPE_INVALID',
      'Local binding requires the exact parent worktree branch and artifact path.',
      ExitCode.verification,
    );
  }
  const now = exactDate(input.now, 'BOOTSTRAP_CLOCK_INVALID');
  const payload = {
    kind: 'local-parent-session-metadata.v2' as const,
    parentChangeId: input.parentChangeId,
    parentWorkspacePath,
    ...
    blocker: {
      kind: 'harness-intervention' as const,
      checkpointId: input.checkpointId,
      blockedBy: input.interventionChangeId,
    },
    interventionState: 'active' as const,
    generation: 1,
    updatedAt: now.toISOString(),
  };
  const binding: LocalEngineBinding = {
    ...payload,
...
export function readLocalEngineBinding(
  bindingPath: string,
): LocalEngineBinding {
  const value = readCanonicalPrivateJson(
    canonicalAbsolutePath(bindingPath),
    'BOOTSTRAP_BINDING_NOT_FOUND',
    64 * 1024,
  );
  if (
    !isRecord(value) ||
  ...
  const binding = value as unknown as LocalEngineBinding;
  ...
  if (
    ...
    binding.activeArtifact.executablePath !==
      localEngineArtifactPath(
        bindingStateRoot(bindingPath),
        binding.activeArtifact.artifactId,
      ) ||
...
export function abandonPersistedIntervention(
  ...
  return withInterventionParentOperation(
    ...
    input.parentChangeId,
    () =>
      abandonPersistedInterventionUnderParentLock(
        storageRoot,
        bindingPath,
        input,
      ),
...
function abandonPersistedInterventionUnderParentLock(
  storageRoot: string,
  bindingPath: string,
  input: {
    parentChangeId: string;
...
export function executePersistedAdoptionStep(
  ...
  receipt: BootstrapAdoptionStepReceipt;
} {
  const verifier = requireHumanVerifier(dependencies);
  const now = bootstrapNow(dependencies);
  ...
  if (current.artifactRecordDigest !== undefined) {
    const storedArtifact = readStoredInterventionEngineArtifact(
      storageRoot,
      current.journal.artifactId,
    );
    if (
      ...
      throw workflowError(
        'BOOTSTRAP_ENGINE_ARTIFACT_RECORD_MISMATCH',
        'Bootstrap execution requires the exact persisted artifact record bound by the adoption.',
        ExitCode.verification,
      );
    }
  }
  if (current.journal.journalDigest !== input.expectedJournalDigest) {
    throw workflowError(
      'BOOTSTRAP_ADOPTION_CAS_MISMATCH',
      'Adoption journal changed before bootstrap execution.',
      ExitCode.staleState,
    );
  }
  const intervention = readPersistedIntervention(
  ...
  if (
    ...
    verifyPersistedMaintenanceGrantForRecovery(
      current.maintenanceGrantEnvelope,
      intervention,
      current.journal,
      verifier,
    );
  ...
  if (
    input.artifact.artifactId !== current.journal.artifactId ||
    ...
    throw workflowError(
      'BOOTSTRAP_ENGINE_ARTIFACT_MISMATCH',
      'Engine artifact differs from the persisted adoption journal.',
      ExitCode.verification,
    );
  }
  const bindingPath = assertPersistenceOwnedPath(
    storageRoot,
    input.bindingPath,
    'BOOTSTRAP_BINDING_OUTSIDE_STATE',
  );
  ...
  if (
    binding.parentChangeId !== current.journal.parentChangeId ||
    binding.parentWorkspacePath !==
    ...
    throw workflowError(
      ...
      'Local engine binding does not belong to this adoption transaction.',
      ExitCode.verification,
    );
  }
  const materializedExecutable = materializeLocalEngineExecutable(
    storageRoot,
    intervention,
    input.artifact,
    input.executablePath,
  ...
  let next: PersistedEngineAdoptionRecord;
  ...
  switch (recovered.decision.action) {
    ...
    case 'update-engine-binding': {
      const updated = ensureBindingEngine(
        bindingPath,
        binding,
        current.journal.fromEngineDigest,
        current.journal.toEngineDigest,
        input.at,
      );
    ...
    case 'start-new-engine': {
      ...
      const probe = runEngineExecutable(
      ...
      if (
        ...
        throw workflowError(
          'BOOTSTRAP_ENGINE_START_FAILED',
          'New engine bootstrap probe did not report a compatible start.',
          ExitCode.verification,
        );
      }
      effectsPerformed = true;
    ...
    case 'run-health-check': {
      assertBindingEngine(binding, current.journal.toEngineDigest);
      const health = runEngineExecutable(
        materializedExecutable,
        input.artifact.executableDigest,
        '--health-check',
      );
      if (
        ...
        throw workflowError(
          'BOOTSTRAP_ENGINE_HEALTH_INVALID',
          'Engine health check returned an invalid contract.',
          ExitCode.verification,
        );
      }
      effectsPerformed = true;
    ...
    case 'finalize-commit': {
      assertBindingEngine(binding, current.journal.toEngineDigest);
      const finalized = ensureBindingAdoptionFinalized(
        bindingPath,
        binding,
        current.journal.checkpointId,
        current.journal.interventionChangeId,
        input.at,
      );
    ...
    case 'rollback-engine-binding': {
      const rolledBack = ensureBindingEngine(
        bindingPath,
        binding,
        current.journal.toEngineDigest,
        current.journal.fromEngineDigest,
    ...
    case 'none': {
      assertBindingEngine(
        binding,
        recovered.decision.authoritativeEngineDigest,
      );
  ...
  const recordedAt =
    recovered.decision.action === 'none'
      ...
      : input.at;
  const receiptPayload = {
    kind: 'bootstrap-adoption-step-receipt.v1' as const,
    txId: input.txId,
  ...
  const receipt: BootstrapAdoptionStepReceipt = {
    ...receiptPayload,
    receiptDigest: canonicalDigest(receiptPayload),
  };
  persistBootstrapReceipt(
    storageRoot,
    `adoption-${input.txId}-${beforeJournalDigest}-${recovered.decision.action}`,
    receipt,
  );
  if (
    next.journal.state === 'COMMITTED' &&
    recovered.decision.action !== 'none'
  ) {
...
function assertCheckpointMatchesBundle(
  ...
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_WIP_BUNDLE_CHECKPOINT_MISMATCH',
      'WIP bundle bytes do not match the persisted checkpoint.',
      ExitCode.verification,
    );
  }
}

function verifyRestoredBundle(
  repositoryRoot: string,
  sessionPath: string,
  bundle: PersistedWipBundle,
): void {
  ...
  if (
    rawDigest(working) !== bundle.tracked.workingTreePatchDigest ||
    ...
    throw workflowError(
      'BOOTSTRAP_RESTORE_VERIFICATION_FAILED',
      'Restored WIP does not reproduce the captured bytes.',
      ExitCode.verification,
    );
  }
  for (const entry of bundle.untracked) {
    ...
    if (rawDigest(fs.readFileSync(target)) !== entry.contentDigest) {
      throw workflowError(
        'BOOTSTRAP_RESTORE_VERIFICATION_FAILED',
        'Restored untracked file digest mismatch.',
        ExitCode.verification,
      );
    }
  }
...
function restoreUntrackedFiles(
  repositoryRoot: string,
  entries: PersistedUntrackedFile[],
): void {
  for (const entry of entries) {
    ...
    if (fs.lstatSync(target, { throwIfNoEntry: false })) {
      throw workflowError(
        'BOOTSTRAP_RESTORE_UNTRACKED_COLLISION',
        `Restore target already exists: ${entry.path}`,
        ExitCode.conflict,
      );
    }
...
function captureUntrackedFiles(
  ...
  return allowlist.map((relativePath) => {
    ...
    if (!stats?.isFile() || stats.isSymbolicLink()) {
      throw workflowError(
        'BOOTSTRAP_UNTRACKED_FILE_UNSAFE',
        `Allowlisted untracked path is not a regular file: ${relativePath}`,
        ExitCode.guard,
      );
    }
    if (stats.size > MAX_UNTRACKED_FILE_BYTES) {
      throw workflowError(
        'BOOTSTRAP_UNTRACKED_FILE_TOO_LARGE',
        `Allowlisted untracked file is too large: ${relativePath}`,
        ExitCode.guard,
      );
    }
    ...
    if (total > MAX_UNTRACKED_TOTAL_BYTES) {
      throw workflowError(
        'BOOTSTRAP_UNTRACKED_TOTAL_TOO_LARGE',
        'Allowlisted untracked bytes exceed the total limit.',
        ExitCode.guard,
      );
    }
    return {
...
function validateUntrackedAllowlist(values: string[]): string[] {
  if (!Array.isArray(values)) {
    throw workflowError(
      'BOOTSTRAP_UNTRACKED_ALLOWLIST_INVALID',
      'Untracked allowlist must be an array.',
      ExitCode.usage,
    );
  }
  ...
  if (canonicalJson(values) !== canonicalJson(normalized)) {
    throw workflowError(
      'BOOTSTRAP_UNTRACKED_ALLOWLIST_INVALID',
      'Untracked allowlist must be sorted and unique.',
      ExitCode.usage,
    );
  }
  return normalized;
...
function listUntracked(repositoryRoot: string): string[] {
...
function parseNulPaths(output: Buffer): string[] {
  ...
  if (output.at(-1) !== 0) {
    throw workflowError(
      'BOOTSTRAP_GIT_PATH_OUTPUT_INVALID',
...
function persistWipBundle(
  ...
  if (content.length > MAX_BUNDLE_BYTES) {
    throw workflowError(
      'BOOTSTRAP_WIP_BUNDLE_TOO_LARGE',
      'WIP bundle exceeds the persistence size limit.',
      ExitCode.guard,
    );
  }
  ...
  if (fs.existsSync(target)) {
    if (
      ...
      throw workflowError(
        'BOOTSTRAP_WIP_BUNDLE_COLLISION',
        'Checkpoint bundle path contains different bytes.',
        ExitCode.verification,
      );
    }
    return;
...
function verifyMaterializedWorktree(
  repositoryRoot: string,
  metadata: PersistedInterventionRecord['childWorkspace'],
): void {
  const childRoot = canonicalRepositoryRoot(metadata.childWorkspacePath);
  if (childRoot !== metadata.childWorkspacePath) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKTREE_VERIFICATION_FAILED',
      'Materialized child root differs from the reservation.',
      ExitCode.verification,
    );
  }
  const commonParent = fs.realpathSync(
    gitText(repositoryRoot, ['rev-parse', '--git-common-dir'])
      .trim()
      .startsWith('/')
  ...
  const childCommon = fs.realpathSync(
    path.isAbsolute(childCommonRaw)
      ? childCommonRaw
      : path.join(childRoot, childCommonRaw),
  );
  if (
    childCommon !== commonParent ||
    gitText(childRoot, ['rev-parse', 'HEAD']).trim() !== metadata.baseOid ||
    gitText(childRoot, ['symbolic-ref', 'HEAD']).trim() !== metadata.changeRef
  ) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKTREE_VERIFICATION_FAILED',
      'Materialized child worktree failed repository/ref/base verification.',
      ExitCode.verification,
    );
  }
}
...
function assertMaterializationReceipt(
  receipt: ChildWorktreeMaterializationReceipt,
  repositoryRoot: string,
  metadata: PersistedInterventionRecord['childWorkspace'],
): void {
  if (
    receipt.kind !== 'intervention-child-worktree-materialized.v1' ||
    ...
    throw workflowError(
      'BOOTSTRAP_RECEIPT_CORRUPT',
      'Child worktree receipt does not match its reservation.',
      ExitCode.verification,
    );
  }
}

function assertEngineArtifactIntegrity(artifact: EngineArtifact): void {
  const artifactKeys = [
    'artifactId',
  ...
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_ENGINE_ARTIFACT_CORRUPT',
      'Engine artifact failed structural verification.',
      ExitCode.verification,
    );
  }
  ...
  if (!isDigest(artifactId) || canonicalDigest(payload) !== artifactId) {
    throw workflowError(
      'BOOTSTRAP_ENGINE_ARTIFACT_CORRUPT',
      'Engine artifact failed content-addressed verification.',
      ExitCode.verification,
    );
  }
}

function materializeLocalEngineExecutable(
  storageRoot: string,
  intervention: PersistedInterventionRecord,
  artifact: EngineArtifact,
  ...
  const executablePath = canonicalPlainFile(
    requestedExecutablePath,
    MAX_UNTRACKED_FILE_BYTES,
    'BOOTSTRAP_ENGINE_EXECUTABLE_UNSAFE',
  );
  const childRoot = intervention.childWorkspace.childWorkspacePath;
  if (!isPathInside(childRoot, executablePath)) {
    throw workflowError(
      'BOOTSTRAP_ENGINE_EXECUTABLE_OUTSIDE_CHILD',
      'Engine executable must come from the intervention child workspace.',
      ExitCode.guard,
    );
  }
  ...
  if (sourceStats.nlink !== 1) {
    throw workflowError(
      'BOOTSTRAP_ENGINE_EXECUTABLE_UNSAFE',
      'Engine executable source must have exactly one link.',
  ...
  try {
    ...
    if (
      ...
      throw workflowError(
        'BOOTSTRAP_ENGINE_EXECUTABLE_UNSAFE',
        'Engine executable source changed while being opened.',
        ExitCode.unsafeEnvironment,
      );
    }
  } finally {
  ...
  if (rawDigest(executableBytes) !== artifact.executableDigest) {
    throw workflowError(
      'BOOTSTRAP_ENGINE_EXECUTABLE_DIGEST_MISMATCH',
      'Engine executable bytes differ from the adopted artifact.',
      ExitCode.verification,
    );
  }
...
function runEngineExecutable(
  ...
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_ENGINE_COMMAND_FAILED',
      `Engine ${mode} command failed.`,
      ExitCode.verification,
    );
  }
  let value: unknown;
  try {
    ...
    throw workflowError(
      'BOOTSTRAP_ENGINE_COMMAND_OUTPUT_INVALID',
      `Engine ${mode} command did not emit JSON.`,
      ExitCode.verification,
    );
  }
  if (!isRecord(value)) {
    throw workflowError(
      'BOOTSTRAP_ENGINE_COMMAND_OUTPUT_INVALID',
      `Engine ${mode} command output must be an object.`,
      ExitCode.verification,
    );
  }
  return value;
...
function ensureBindingEngine(
  bindingPath: string,
  current: LocalEngineBinding,
  expectedFrom: Sha256Digest,
  target: Sha256Digest,
  at: string,
): LocalEngineBinding {
  ...
  if (current.engineDigest !== expectedFrom) {
    throw workflowError(
      'BOOTSTRAP_BINDING_ENGINE_MISMATCH',
      'Local engine binding is neither the expected source nor target.',
      ExitCode.staleState,
    );
  }
...
function ensureBindingAdoptionFinalized(
  bindingPath: string,
  current: LocalEngineBinding,
  checkpointId: Sha256Digest,
  interventionChangeId: string,
  ...
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_PARENT_SESSION_CAS_MISMATCH',
      'Parent session blocker changed before adoption finalization.',
      ExitCode.staleState,
    );
  }
  const { recordDigest: _recordDigest, ...currentPayload } = current;
  const payload = {
    ...currentPayload,
    blocker: null,
    interventionState: 'adopted' as const,
    generation: current.generation + 1,
    updatedAt: at,
  };
  const next: LocalEngineBinding = {
...
function assertBindingEngine(
  binding: LocalEngineBinding,
  expected: Sha256Digest,
): void {
  if (binding.engineDigest !== expected) {
    throw workflowError(
      'BOOTSTRAP_BINDING_ENGINE_MISMATCH',
      'Local engine binding does not select the journal engine.',
      ExitCode.staleState,
    );
  }
}

function isHarnessInterventionBlocker(
  value: unknown,
  checkpointId: Sha256Digest,
  interventionChangeId: string,
): boolean {
...
function assertAbandonableProjection(
  ...
  const inFlightTxIds = inFlightPersistedEngineAdoptionTxIds(
    storageRoot,
    intervention.parent.changeId,
  );
  const promotionTxIds = activeBootstrapSidecarPromotionPinTxIds(
    storageRoot,
    intervention.parent.changeId,
  );
  if (promotionTxIds.length > 0) {
    throw workflowError(
      'INTERVENTION_ABANDONMENT_PROMOTION_NOT_TERMINAL',
      'An in-flight repository promotion must be recovered before intervention abandonment.',
      ExitCode.guard,
      { details: { txIds: promotionTxIds } },
    );
  }
  if (localBinding === null) {
    if (inFlightTxIds.length > 0) {
      throw workflowError(
        ...
        'An in-flight adoption must be recovered before intervention abandonment.',
        ExitCode.guard,
        { details: { txIds: inFlightTxIds } },
      );
    }
    return;
  }
...
export function localEngineArtifactPath(
  storageRoot: string,
  artifactId: Sha256Digest,
): string {
...
function bindingStateRoot(bindingPath: string): string {
...
function bootstrapBundleDirectory(storageRoot: string): string {
  return path.join(
    interventionControlPersistencePaths(storageRoot).root,
    'bootstrap-wip-bundles',
  );
}

function bootstrapReceiptDirectory(storageRoot: string): string {
  return path.join(
    interventionControlPersistencePaths(storageRoot).root,
    'bootstrap-receipts',
  );
...
function wipBundlePath(
  storageRoot: string,
  checkpointId: Sha256Digest,
): string {
  return path.join(
    bootstrapBundleDirectory(storageRoot),
    `${checkpointId.slice('sha256:'.length)}.json`,
  );
}

function bootstrapReceiptPath(storageRoot: string, identity: string): string {
  return path.join(
    bootstrapReceiptDirectory(storageRoot),
    `${crypto.createHash('sha256').update(identity).digest('hex')}.json`,
  );
}

function persistBootstrapReceipt(
  storageRoot: string,
  identity: string,
  receipt: unknown,
): void {
  ...
  if (fs.existsSync(target)) {
    if (
      !readPrivateBytes(
        target,
        'BOOTSTRAP_RECEIPT_NOT_FOUND',
        1024 * 1024,
      ).equals(content)
    ) {
      throw workflowError(
        'BOOTSTRAP_RECEIPT_CONFLICT',
        'Bootstrap receipt identity already contains different bytes.',
        ExitCode.conflict,
      );
    }
    return;
...
function readBootstrapReceipt<T>(receiptPath: string): T {
  const value = readCanonicalPrivateJson(
    receiptPath,
    'BOOTSTRAP_RECEIPT_NOT_FOUND',
    1024 * 1024,
  ...
  if (!isRecord(value) || !isDigest(value.receiptDigest)) {
    throw workflowError(
      'BOOTSTRAP_RECEIPT_CORRUPT',
      'Bootstrap receipt failed structural verification.',
      ExitCode.verification,
    );
  }
  const { receiptDigest, ...payload } = value;
  if (canonicalDigest(payload) !== receiptDigest) {
    throw workflowError(
      'BOOTSTRAP_RECEIPT_CORRUPT',
      'Bootstrap receipt failed digest verification.',
      ExitCode.verification,
    );
  }
  return value as T;
...
function canonicalRepositoryRoot(requestedRoot: string): string {
  const absolute = canonicalAbsolutePath(requestedRoot);
  const stats = fs.lstatSync(absolute, { throwIfNoEntry: false });
  if (!stats?.isDirectory() || stats.isSymbolicLink()) {
    throw workflowError(
      'BOOTSTRAP_REPOSITORY_UNSAFE',
      'Bootstrap repository root must be a plain directory.',
      ExitCode.unsafeEnvironment,
    );
  }
  ...
  if (real !== absolute) {
    throw workflowError(
      'BOOTSTRAP_REPOSITORY_UNSAFE',
      'Bootstrap repository root must be canonical.',
      ExitCode.unsafeEnvironment,
    );
  }
  ...
  if (fs.realpathSync(topLevel) !== real) {
    throw workflowError(
      'BOOTSTRAP_REPOSITORY_UNSAFE',
      'Requested path is not the exact Git worktree root.',
      ExitCode.unsafeEnvironment,
    );
  }
  return real;
...
function canonicalPlannedWorkspacePath(
  ...
  if (!fs.existsSync(parent) || fs.realpathSync(parent) !== parent) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKSPACE_PARENT_UNSAFE',
      'Child workspace parent directory must be canonical.',
      ExitCode.unsafeEnvironment,
    );
  }
  if (
    parent !== path.dirname(repositoryRoot) ||
    absolute === repositoryRoot ||
    isPathInside(repositoryRoot, absolute) ||
    isPathInside(absolute, repositoryRoot)
  ) {
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKSPACE_SCOPE_INVALID',
      'Child workspace must be a disjoint sibling of the parent worktree.',
      ExitCode.guard,
    );
  }
  return absolute;
...
function canonicalPlainFile(
  ...
  if (
    ...
    throw workflowError(
      code,
      'Bootstrap file must be a bounded canonical regular file.',
      ExitCode.unsafeEnvironment,
    );
  }
  return absolute;
...
function canonicalAbsolutePath(requestedPath: string): string {
  if (
    ...
    throw workflowError(
      ...
      'Bootstrap path must be an explicit normalized absolute path.',
      ExitCode.usage,
    );
  }
...
function assertPersistenceOwnedPath(
  storageRoot: string,
  requestedPath: string,
  code: string,
): string {
  ...
  if (!isPathInside(root, absolute)) {
    throw workflowError(
      code,
      'Bootstrap mutable state must stay inside the persistence root.',
      ExitCode.guard,
    );
  }
  return absolute;
...
function assertSafeRelativePath(value: string): void {
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_UNTRACKED_PATH_INVALID',
      'Untracked allowlist path is unsafe.',
      ExitCode.usage,
    );
  }
}
...
function safeRepositoryPath(
  ...
  if (!isPathInside(repositoryRoot, target)) {
    throw workflowError(
      'BOOTSTRAP_UNTRACKED_PATH_INVALID',
      'Untracked path escapes the repository root.',
      ExitCode.guard,
    );
  }
  return target;
...
function ensureSafeParentDirectories(
  ...
  for (const part of relative.split(path.sep).filter(Boolean)) {
    ...
    if (!stats) {
      ...
      throw workflowError(
        'BOOTSTRAP_RESTORE_PARENT_UNSAFE',
        'Untracked restore parent is not a plain directory.',
        ExitCode.unsafeEnvironment,
      );
    }
  }
...
function removeRestoredUntracked(
  repositoryRoot: string,
  relativePath: string,
): void {
  const target = safeRepositoryPath(repositoryRoot, relativePath);
...
function removeExactChildWorkspace(
  childPath: string,
  parentRoot: string,
): void {
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_CHILD_WORKTREE_CLEANUP_UNSAFE',
      'Refusing to clean an unvalidated child workspace path.',
      ExitCode.unsafeEnvironment,
    );
  }
...
function gitBuffer(
  ...
  if (result.error || result.status !== 0 || !Buffer.isBuffer(result.stdout)) {
    const stderr = Buffer.isBuffer(result.stderr)
      ...
      : '';
    throw workflowError(
      'BOOTSTRAP_GIT_COMMAND_FAILED',
      `Git command failed: ${args[0] ?? 'unknown'}`,
      ExitCode.verification,
      { details: { status: result.status, stderr } },
    );
  }
  return result.stdout;
}

function gitBufferNoThrow(repositoryRoot: string, args: string[]): void {
...
function gitRefExists(repositoryRoot: string, ref: string): boolean {
  const result = spawnSync(
    'git',
    ['-C', repositoryRoot, 'show-ref', '--verify', '--quiet', ref],
    { encoding: null, env: { ...process.env, LANG: 'C', LC_ALL: 'C' } },
  );
  if (result.status === 0) {
    return true;
  }
  ...
  throw workflowError(
    'BOOTSTRAP_GIT_COMMAND_FAILED',
    'Unable to inspect reserved child change ref.',
    ExitCode.verification,
  );
}

function createPrivateFileExclusive(filePath: string, content: Buffer): void {
...
function assertPrivateLocalEngineExecutable(
  filePath: string,
  expectedDigest: Sha256Digest,
): void {
  ...
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_LOCAL_ENGINE_ARTIFACT_UNSAFE',
  ...
  try {
    ...
    if (
      ...
      throw workflowError(
        'BOOTSTRAP_LOCAL_ENGINE_ARTIFACT_DIGEST_MISMATCH',
...
function writePrivateFileAtomic(filePath: string, content: Buffer): void {
...
function replacePrivateFileAtomic(
  filePath: string,
  content: Buffer,
  ...
  if (current.recordDigest !== expectedRecordDigest) {
    throw workflowError(
      'BOOTSTRAP_BINDING_CAS_MISMATCH',
      'Local engine binding changed before atomic replacement.',
      ExitCode.staleState,
    );
  }
...
function readCanonicalPrivateJson(
  filePath: string,
  notFoundCode: string,
  maxBytes: number,
): unknown {
  ...
  if (!Buffer.from(raw, 'utf8').equals(bytes)) {
    throw workflowError(
      'BOOTSTRAP_PERSISTENCE_OBJECT_CORRUPT',
      'Bootstrap persistence object is not valid UTF-8.',
      ExitCode.verification,
    );
  }
  let value: unknown;
  try {
    ...
    throw workflowError(
      'BOOTSTRAP_PERSISTENCE_OBJECT_CORRUPT',
      'Bootstrap persistence object is not JSON.',
      ExitCode.verification,
    );
  }
  if (`${canonicalJson(value)}\n` !== raw) {
    throw workflowError(
      'BOOTSTRAP_PERSISTENCE_OBJECT_CORRUPT',
      'Bootstrap persistence object is not canonical.',
      ExitCode.verification,
    );
  }
  return value;
...
function readPrivateBytes(
  ...
  if (!stats) {
    throw workflowError(
      notFoundCode,
      'Bootstrap persistence object was not found.',
      ExitCode.conflict,
    );
  }
  if (
    ...
    throw workflowError(
      'BOOTSTRAP_PERSISTENCE_OBJECT_UNSAFE',
      'Bootstrap persistence object is not a bounded private file.',
      ExitCode.unsafeEnvironment,
    );
  }
  const descriptor = fs.openSync(
  ...
  try {
    ...
    if (
      ...
      throw workflowError(
        'BOOTSTRAP_PERSISTENCE_OBJECT_UNSAFE',
        'Bootstrap persistence object changed while being opened.',
        ExitCode.unsafeEnvironment,
      );
    }
...
function readOptionalPlainFile(
  filePath: string,
  maxBytes: number,
): Buffer | null {
  const stats = fs.lstatSync(filePath, { throwIfNoEntry: false });
  ...
  if (!stats.isFile() || stats.isSymbolicLink() || stats.size > maxBytes) {
    throw workflowError(
      'BOOTSTRAP_SESSION_SNAPSHOT_UNSAFE',
      'Existing session snapshot target is unsafe.',
      ExitCode.unsafeEnvironment,
    );
  }
...
function rawDigest(value: Buffer): Sha256Digest {
...
function verifyPersistedMaintenanceGrantForRecovery(
  envelope: HarnessMaintenanceGrantEnvelope,
  intervention: PersistedInterventionRecord,
  journal: PersistedEngineAdoptionRecord['journal'],
  verifier: BootstrapHumanSignatureVerifier,
): void {
  const payload = envelope.payload;
  if (
    payload.grantId !== journal.grantId ||
    ...
    throw workflowError(
      'BOOTSTRAP_RECOVERY_GRANT_INVALID',
      'Persisted recovery grant no longer matches the adoption journal.',
      ExitCode.verification,
    );
  }
  ...
  try {
    ...
    throw workflowError(
      'BOOTSTRAP_RECOVERY_GRANT_INVALID',
      'Persisted recovery grant failed binding or signature verification.',
      ExitCode.verification,
    );
  }
}
...
function requireHumanVerifier(
  dependencies: BootstrapDependencies,
): BootstrapHumanSignatureVerifier {
  if (dependencies.verifyHumanSignature === undefined) {
    throw workflowError(
      'BOOTSTRAP_HUMAN_VERIFIER_REQUIRED',
      'Bootstrap effect requires a trusted human grant verifier.',
      ExitCode.guard,
    );
  }
...
function bootstrapNow(dependencies: BootstrapDependencies): Date {
  return exactDate(
...
function assertGitOid(value: string, code: string): void {
...
function corruptWipBundle() {
  return workflowError(
    'BOOTSTRAP_WIP_BUNDLE_CORRUPT',
    'Persisted WIP bundle failed integrity verification.',
    ExitCode.verification,
  );
}
...
function corruptBinding() {
  return workflowError(
    'BOOTSTRAP_BINDING_CORRUPT',
    'Local engine binding failed integrity verification.',
    ExitCode.verification,
  );
}
