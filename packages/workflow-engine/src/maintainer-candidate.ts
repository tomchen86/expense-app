...
export function createApplyJournal(input: {
  txId: string;
  grantId: string;
  targetRef: string;
  expectedOldCommit: string;
  ...
  const createdAt = exactTimestamp(input.createdAt, 'apply transaction time');
  if (
    !TX_ID.test(input.txId) ||
    !GRANT_ID.test(input.grantId) ||
    !validRef(input.targetRef) ||
    !OBJECT_ID.test(input.expectedOldCommit) ||
    !Number.isSafeInteger(input.expectedRefGeneration) ||
    input.expectedRefGeneration < 0 ||
    !OBJECT_ID.test(input.candidateCommit) ||
    input.candidateCommit === input.expectedOldCommit ||
    !DIGEST.test(input.candidateBundleDigest)
  ) {
    throw candidateError(
      'APPLY_JOURNAL_INVALID',
      'Apply transaction journal input is malformed.',
    );
  }
  return {
    schemaVersion: 1,
...
export function advanceApplyJournal(
  journal: ApplyJournal,
  nextState: ApplyJournalState,
  input: { at: string; observedRef?: string; failureCode?: string },
): ApplyJournal {
  const at = exactTimestamp(input.at, 'apply journal transition time');
  if (!JOURNAL_TRANSITIONS[journal.state]?.includes(nextState)) {
    throw candidateError(
      'APPLY_JOURNAL_TRANSITION_INVALID',
      `Apply journal cannot transition from ${journal.state} to ${nextState}.`,
    );
  }
  if (input.observedRef !== undefined && !OBJECT_ID.test(input.observedRef)) {
    throw candidateError(
      'APPLY_JOURNAL_TRANSITION_INVALID',
      'Apply journal observed ref is malformed.',
    );
  }
  if (
    nextState === 'REF_UPDATED' &&
    input.observedRef !== journal.candidateCommit
  ) {
    throw candidateError(
      'APPLY_JOURNAL_TRANSITION_INVALID',
      'REF_UPDATED requires the exact candidate commit as observed poststate.',
    );
  }
  return {
    ...journal,
    state: nextState,
    observedRef: input.observedRef ?? journal.observedRef,
    failureCode:
      input.failureCode ??
      (nextState === 'FAILED'
...
export type ApplyRecoveryDecision =
  | { action: 'resume-before-cas' }
  | { action: 'terminalize-expired' }
  | { action: 'complete-after-cas' }
  | { action: 'already-complete' }
  | { action: 'manual-reconciliation' };

export function recoverApplyJournal(
  journal: ApplyJournal,
  input: { observedRef: string; now: Date; grantExpiresAt: string },
): ApplyRecoveryDecision {
  if (!OBJECT_ID.test(input.observedRef)) {
    throw candidateError(
      'APPLY_RECOVERY_OBSERVATION_INVALID',
      'Apply recovery observed ref is malformed.',
    );
  }
  const now = exactDate(input.now, 'apply recovery time').getTime();
  ...
  if (
    ...
    input.observedRef === journal.expectedOldCommit
  ) {
...
export type ApplyGrantRuntimeState = {
  grantId: string;
  state:
    | 'available'
    | 'applying'
...
export function terminalizeApplyGrant(
  grant: ApplyGrantRuntimeState,
  terminalState: 'consumed' | 'expired' | 'invalidated' | 'failed',
): ApplyGrantRuntimeState {
  if (
    !GRANT_ID.test(grant.grantId) ||
    !DIGEST.test(grant.candidateBundleDigest) ||
    !DIGEST.test(grant.checksAttestationDigest) ||
    !['available', 'applying'].includes(grant.state) ||
    !['consumed', 'expired', 'invalidated', 'failed'].includes(terminalState)
  ) {
    throw candidateError(
      'APPLY_GRANT_TERMINALIZATION_INVALID',
      'Apply grant cannot enter the requested terminal state.',
    );
  }
  return { ...grant, state: terminalState, reservationId: null };
}
