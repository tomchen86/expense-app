...
const CONVERGENCE_OUTPUT_SCHEMA =
  'expense-app.workflow.evidence-convergence-output.v1';

const REUSE_PROOF_TYPE = 'evidence-reuse-proof';
const REUSE_PROOF_NODE_SCHEMA = 'expense-app.workflow.evidence-reuse-proof.v1';
const REUSE_PROOF_OUTPUT_SCHEMA =
  'expense-app.workflow.evidence-reuse-proof-output.v1';

const CONVERGENCE_EXACT_ROLES = ['newParentNode', 'oldParentNode'] as const;
const CONVERGENCE_PROVENANCE_ROLES = ['newParent', 'oldParent'] as const;
...
/** Every compatibility claim a convergence record exposes about its parents. */
export type ConvergenceBinding = {
  oldParentNode: string;
  newParentNode: string;
  sharedType: string;
  sharedEvaluator: string;
  sharedPolicyDigest: string;
  sharedNodeSchema: string;
  sharedOutputSchema: string;
  sharedResultDigest: string;
  validatorVersion: string;
};

export type ReuseProofBinding = {
  descendantNode: string;
  oldParentNode: string;
  newParentNode: string;
 ...
 */
export function parentsCompatible(a: EvidenceNode, b: EvidenceNode): boolean {
  return (
    a.type === b.type &&
...
export function createConvergenceRecord(input: {
  ...
  const oldParent = assertStoredEvidenceNode(
    input.oldParent,
    convergenceInvalid,
  );
  const newParent = assertStoredEvidenceNode(
    input.newParent,
    convergenceInvalid,
  );
...
export function createDescendantReuseProof(input: {
  ...
  const descendant = assertStoredEvidenceNode(
    input.descendant,
    reuseProofInvalid,
  );
  const oldParent = assertStoredEvidenceNode(
  ...
  const convergence = assertStoredEvidenceNode(
    input.convergenceRecord,
    reuseProofInvalid,
  );
 ...
 */
export function readConvergenceBinding(
  node: EvidenceNode,
): ConvergenceBinding | null {
  if (
    node.type !== CONVERGENCE_TYPE ||
 ...
 */
export function readReuseProofBinding(
  node: EvidenceNode,
): ReuseProofBinding | null {
  if (
    node.type !== REUSE_PROOF_TYPE ||
  ...
  if (
    node.provenanceParentNodeIds.descendant !== descendantNode ||
    node.provenanceParentNodeIds.oldParent !== oldParentNode ||
    node.provenanceParentNodeIds.newParent !== newParentNode ||
...
function digestParentRole(parentRole: string): string {
...
function convergenceIncompatible() {
  return workflowError(
    'EVIDENCE_CONVERGENCE_INCOMPATIBLE',
    'Convergence requires equal type, evaluator, policy, schemas, and result.',
    ExitCode.usage,
  );
}
...
function convergenceInvalid() {
  return workflowError(
    'EVIDENCE_CONVERGENCE_INVALID',
    'Convergence record input is malformed.',
    ExitCode.usage,
  );
}
...
function reuseProofInvalid() {
  return workflowError(
    'EVIDENCE_REUSE_PROOF_INVALID',
    'Descendant reuse proof input is malformed.',
    ExitCode.usage,
  );
}
