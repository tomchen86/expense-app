 ...
 */
export const LEGACY_MIGRATION_CHANGE_ID =
...
export const LEGACY_MIGRATION_POLICY_DIGEST = sha256(
  canonicalJson({
...
export type LegacyPlanTaskState = {
  id: string;
  completed: boolean;
};

export type LegacyPlanMigrationSubject = {
  schemaVersion: 1;
  ...
  taskProjection: LegacyPlanTaskState[];
  preservedArtifactDigests: Record<string, string>;
  replacedArtifactDigests: Record<string, string>;
  guardDigest: string;
  policyDigest: string;
 ...
 * `guardDigest` instead.
 */
function preservedRelativePaths(
...
export function deriveLegacyPlanMigrationSubject(input: {
  ...
  const metadata = readTextAtCommit(
    repositoryRoot,
    baseline.head,
    `${changePrefix}/.openspec.yaml`,
  );
  if (metadata === null) {
    throw notEligible(
      'Legacy migration requires a committed legacy change tree.',
    );
  }
  const legacyMetadata = parseLegacyMetadata(metadata);

  const governingCommit = resolveGoverningLegacyGeneration(
    repositoryRoot,
    changeId,
    baseline.head,
  );
  if (
    ...
    throw notEligible(
      'Legacy change metadata drifted from its governing generation.',
    );
  }

  const tasksMarkdown = readTextAtCommit(
    repositoryRoot,
    baseline.head,
    `${changePrefix}/tasks.md`,
  );
  const guardJson = readTextAtCommit(
    repositoryRoot,
    baseline.head,
    `${changePrefix}/guard.json`,
  );
  if (tasksMarkdown === null || guardJson === null) {
    throw notEligible(
      'Legacy migration requires committed legacy tasks and guard artifacts.',
    );
  }
  ...
  const digestAtBaseline = (relativePath: string): [string, string] => {
    const content = readTextAtCommit(
      repositoryRoot,
      baseline.head,
      `${changePrefix}/${relativePath}`,
    );
    if (content === null) {
  ...
  const preservedArtifactDigests = Object.fromEntries(
    preservedRelativePaths(repositoryRoot, baseline.head, changePrefix).map(
      digestAtBaseline,
    ),
  );
  const replacedArtifactDigests = Object.fromEntries(
    LEGACY_MIGRATION_REPLACED_ARTIFACTS.map(digestAtBaseline),
  );
 ...
 */
export function assertLegacyPlanMigrationSubject(
  value: unknown,
): LegacyPlanMigrationSubject {
  if (
    !isRecord(value) ||
    ...
    LEGACY_MIGRATION_REPLACED_ARTIFACTS.some(
      (relativePath) =>
        !Object.hasOwn(value.replacedArtifactDigests as object, relativePath),
    ) ||
  ...
  const candidate = value as unknown as LegacyPlanMigrationSubject;
 ...
 */
export function legacyMigrationMetadataBytes(
  subject: LegacyPlanMigrationSubject,
): string {
 ...
 */
export function isReplaceableLegacyArtifact(
  subject: LegacyPlanMigrationSubject,
  relativePath: string,
  existing: string,
): boolean {
...
export function assertPreservedLegacyProjection(input: {
  ...
  if (
    canonicalJson(tasks.map(({ id, completed }) => ({ id, completed }))) !==
    canonicalJson(subject.taskProjection)
  ) {
    throw projectionInvalid(
      'A legacy migration must preserve the exact committed checkbox projection.',
    );
  }
  if (sha256(canonicalJson(guard)) !== subject.guardDigest) {
    throw projectionInvalid(
      'A legacy migration must preserve the governed task guard.',
    );
  }
  for (const [relativePath, expectedDigest] of Object.entries(
    subject.preservedArtifactDigests,
  )) {
    const content = entries.get(relativePath);
    if (content === undefined || sha256(content) !== expectedDigest) {
      throw projectionInvalid(
        `A legacy migration must preserve ${relativePath} exactly.`,
      );
    }
  }
  for (const relativePath of entries.keys()) {
    if (
      ...
      throw projectionInvalid(
        `A legacy migration must not add ${relativePath} to the governed plan.`,
      );
    }
  }
}

function resolveGoverningLegacyGeneration(
  repositoryRoot: string,
  changeId: string,
  head: string,
): string {
  ...
  for (const commit of commits) {
    ...
    try {
      trailers = parseManagedTrailers(
        commitFacts(repositoryRoot, commit).message,
      );
    } catch (error) {
  ...
  throw workflowError(
    'LEGACY_MIGRATION_GENERATION_MISSING',
    'No immutable governing legacy plan generation is reachable from the baseline.',
    ExitCode.guard,
  );
}

function parseLegacyMetadata(text: string): { created: string } {
  ...
  if (
    ...
    throw notEligible(
      'Legacy migration requires a change that currently declares expense-app.',
    );
  }
  return { created };
}
...
function readTextAtCommit(
  ...
  const entry = runGit(
    repositoryRoot,
    ['ls-tree', '-z', commit, '--', `:(literal)${filePath}`],
    true,
  );
  if (!entry.trim()) {
    return null;
...
function finalizeSubject(
  subject: Omit<LegacyPlanMigrationSubject, 'subjectDigest'>,
): LegacyPlanMigrationSubject {
...
function notEligible(message: string) {
...
function subjectInvalid() {
  return workflowError(
    'LEGACY_MIGRATION_SUBJECT_INVALID',
    'The durable legacy migration subject is not the exact authorized subject.',
    ExitCode.staleState,
  );
}
...
function projectionInvalid(message: string) {
  return workflowError(
    'LEGACY_MIGRATION_PROJECTION_INVALID',
    message,
    ExitCode.guard,
  );
}
