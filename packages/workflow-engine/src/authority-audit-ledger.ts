...
const PRIVATE_PUBLICATION_NAME =
  /^\.(.+)\.([0-9a-f]{64})\.([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.publish\.tmp$/;
...
export type AuthorityAuditProfile = 'development' | 'protected';
...
export type AuthorityAuditLedgerScope = Readonly<{
  ...
  /** Defaults to development, then becomes immutable for this repository ID. */
...
export const AUTHORITY_AUDIT_PROFILE_POLICIES = deepFreeze({
  development: {
    ...
    destructionMode: 'automatic-retention-only' as const,
  },
  protected: {
    schemaVersion: 1 as const,
    profile: 'protected' as const,
    maxAgeMs: null,
    minimumRecentRecords: null,
    anchorMode: 'backend-receipt' as const,
    destructionMode: 'exact-human-grant-with-tombstone' as const,
  },
});

export type AuthorityAuditProfilePolicy =
  (typeof AUTHORITY_AUDIT_PROFILE_POLICIES)[AuthorityAuditProfile];
...
export type AuthorityAuditLedgerPaths = Readonly<{
  ...
  retentionCheckpoint: string;
  retentionCheckpointNext: string;
...
export type AuthorityAuditMaintenanceHooks = Readonly<{
  testAfterJournalPreparation?: () => void;
  testAfterTombstonePreparation?: () => void;
  testAfterTombstonePublication?: () => void;
  testAfterCheckpointPreparation?: () => void;
  testAfterCheckpointPublication?: () => void;
  testAfterEntryRemoval?: () => void;
}>;

export type AuthorityAuditCompactionResult = Readonly<{
  schemaVersion: 1;
  ...
  prunedThroughRecordDigest: Sha256Digest | null;
  checkpointDigest: Sha256Digest | null;
}>;

export type AuthorityAuditAnchorBackend = Readonly<{
  backendId: string;
  /** Publish to the real remote tag/WORM backend selected by the caller. */
  publish: (canonicalPayload: string) => string;
  /** Independently validates the backend's opaque receipt. */
  verify: (canonicalPayload: string, backendReceipt: string) => boolean;
}>;

export type AuthorityAuditAnchorReceipt = Readonly<{
  schemaVersion: 1;
  ...
  headSequence: number;
  headRecordDigest: Sha256Digest;
  anchoredAt: string;
  backendId: string;
  backendReceipt: string;
}>;

export type AuthorityAuditAnchorPublication = Readonly<{
  receiptDigest: Sha256Digest;
  receiptPath: string;
  receipt: AuthorityAuditAnchorReceipt;
}>;

export type AuthorityAuditAnchorVerification = Readonly<{
  schemaVersion: 1;
  kind: 'authority-audit-anchor-verification.v1';
  repositoryId: Sha256Digest;
  ok: true;
  currentHeadAnchored: boolean;
  receipts: readonly AuthorityAuditAnchorPublication[];
}>;

export type AuditDestructionGrantPayload = Readonly<{
  schemaVersion: 1;
...
export type AuditDestructionGrantEnvelope = Readonly<{
  schemaVersion: 1;
  kind: 'audit-destruction-grant-envelope.v1';
  payload: AuditDestructionGrantPayload;
  signature: string;
}>;

export type AuditDestructionSignatureVerifier = (
  input: Readonly<{
    domain: 'HARNESS_AUDIT_DESTRUCTION_GRANT_V1';
    payload: string;
    signerIdentity: string;
...
type AuthorityAuditProfileState = Readonly<{
  schemaVersion: 1;
...
export type AuthorityAuditProtectedConfiguration = Readonly<{
  schemaVersion: 1;
  kind: 'authority-audit-protected-configuration.v1';
  repositoryId: Sha256Digest;
  backendId: 'filesystem-worm.v1';
  backendRoot: string;
  destructionPublicKeyPem: string;
  destructionPublicKeyDigest: Sha256Digest;
  configurationDigest: Sha256Digest;
}>;

type AuthorityAuditRetentionCheckpoint = Readonly<{
  schemaVersion: 1;
  ...
  policyDigest: Sha256Digest;
  prunedThroughSequence: number;
  prunedThroughRecordDigest: Sha256Digest;
  prunedAt: string;
  reason: 'development-retention' | 'protected-destruction';
  grantDigest: Sha256Digest | null;
  tombstoneDigest: Sha256Digest | null;
  previousCheckpointDigest: Sha256Digest | null;
}>;

type AuthorityAuditDestructionTombstone = Readonly<{
  schemaVersion: 1;
  kind: 'authority-audit-destruction-tombstone.v1';
  repositoryId: Sha256Digest;
  grantDigest: Sha256Digest;
  grantEnvelope: AuditDestructionGrantEnvelope;
  signerIdentity: string;
  expectedHeadSequence: number;
  expectedHeadRecordDigest: Sha256Digest;
  destroyedThroughSequence: number;
  destroyedThroughRecordDigest: Sha256Digest;
  destroyedAt: string;
  reasonDigest: Sha256Digest;
  remoteAnchorDisposition: 'retain' | 'deletion-authorized';
  previousCheckpointDigest: Sha256Digest | null;
...
type AuthorityAuditMaintenanceJournal = Readonly<{
  schemaVersion: 1;
  kind: 'authority-audit-maintenance-journal.v1';
  repositoryId: Sha256Digest;
  checkpoint: AuthorityAuditRetentionCheckpoint;
  checkpointDigest: Sha256Digest;
  tombstone: AuthorityAuditDestructionTombstone | null;
  tombstoneDigest: Sha256Digest | null;
}>;
...
export function configureProtectedAuthorityAuditLedger(
  scope: AuthorityAuditLedgerScope,
  input: Readonly<{
    backendRoot: string;
    destructionPublicKeyPem: string;
  }>,
): AuthorityAuditProtectedConfiguration {
  const prepared = prepareLedger({ ...scope, profile: 'protected' });
  if (prepared.profile !== 'protected') throw developmentProfile();
  const backendRoot = assertProtectedBackendRoot(
    input.backendRoot,
    prepared.scope,
  );
  const destructionPublicKeyPem = canonicalDestructionPublicKey(
    input.destructionPublicKeyPem,
  );
  const core = {
    schemaVersion: 1 as const,
  ...
  if (
    fs.readdirSync(prepared.paths.events).length > 0 ||
  ...
  writeExclusivePrivateFile(
    prepared.paths.protectedConfiguration,
    bytes,
...
export function readProtectedAuthorityAuditConfiguration(
  scope: AuthorityAuditLedgerScope,
): AuthorityAuditProtectedConfiguration {
...
export function createPinnedAuthorityAuditAnchorBackend(
  scope: AuthorityAuditLedgerScope,
): AuthorityAuditAnchorBackend {
  ...
  return Object.freeze({
    ...
    publish(canonicalPayload: string): string {
      ...
      recoverPrivateFilePublications(
        backendPaths.anchors,
        (name) => /^[0-9a-f]{64}\.anchor$/.test(name),
        anchorBackendInvalid,
      );
      publishImmutableObject(
        path.join(backendPaths.anchors, objectName),
        canonicalPayload,
        anchorBackendInvalid,
      );
    ...
    verify(canonicalPayload: string, backendReceipt: string): boolean {
      try {
        ...
        const raw = JSON.parse(backendReceipt) as unknown;
        if (
          !isPlainRecord(raw) ||
        ...
        return (
          readExactPrivateFile(
            path.join(backendPaths.anchors, raw.objectName),
            MAX_METADATA_BYTES,
            anchorBackendInvalid,
          ) === canonicalPayload
        );
...
export function createPinnedAuditDestructionSignatureVerifier(
  scope: AuthorityAuditLedgerScope,
...
function ledgerPathsForScope(
  ...
  return Object.freeze({
    ...
    retentionCheckpointNext: path.join(
      metadata,
      RETENTION_CHECKPOINT_NEXT_NAME,
    ),
...
export function appendAuthorityAuditRecord(
  ...
  const maintenanceNow = assertMaintenanceTimestamp(
  ...
  return withLedgerLock(prepared.paths, prepared.scope.repositoryId, () => {
    ...
    recoverRecordPublications(
      prepared.paths,
      prepared.scope.repositoryId,
      prepared.profile,
    );
    const checkpoint = readRetentionCheckpoint(
      prepared.paths,
      prepared.scope.repositoryId,
      prepared.profile,
    );
    ...
    const previousSequence =
      previous?.record.sequence ?? checkpoint?.prunedThroughSequence ?? 0;
    const previousDigest =
      previous?.recordDigest ?? checkpoint?.prunedThroughRecordDigest ?? null;
...
export function scanAuthorityAuditLedger(
  ...
  return withLedgerLock(prepared.paths, prepared.scope.repositoryId, () => {
    ...
    return Object.freeze({
      ...
      headSequence:
        head?.record.sequence ?? checkpoint?.prunedThroughSequence ?? 0,
      headRecordDigest:
        head?.recordDigest ?? checkpoint?.prunedThroughRecordDigest ?? null,
 ...
 */
export function compactDevelopmentAuthorityAuditLedger(
  scope: AuthorityAuditLedgerScope,
  input: Readonly<{ now: string }>,
  hooks: AuthorityAuditMaintenanceHooks = {},
): AuthorityAuditCompactionResult {
  const now = assertMaintenanceTimestamp(input.now);
  const prepared = prepareLedger(scope);
  if (prepared.profile !== 'development') throw protectedProfile();
  return withLedgerLock(prepared.paths, prepared.scope.repositoryId, () => {
    recoverAuditMaintenance(prepared.paths, prepared.profile);
    recoverRecordPublications(
      prepared.paths,
...
function compactDevelopmentUnderLock(
  prepared: Readonly<{
    scope: AuthorityAuditLedgerScope;
  ...
  if (prepared.profile !== 'development') throw protectedProfile();
  const previousCheckpoint = readRetentionCheckpoint(
    prepared.paths,
    prepared.scope.repositoryId,
    prepared.profile,
  );
  ...
  const maximumPrunable = Math.max(
    0,
    records.length - policy.minimumRecentRecords,
  );
  const cutoff = Date.parse(now) - policy.maxAgeMs;
  let count = 0;
  while (
    count < maximumPrunable &&
...
export function anchorProtectedAuthorityAuditLedger(
  scope: AuthorityAuditLedgerScope,
  input: Readonly<{
    anchoredAt: string;
    backend: AuthorityAuditAnchorBackend;
  }>,
): AuthorityAuditAnchorPublication {
  ...
  return withLedgerLock(prepared.paths, prepared.scope.repositoryId, () => {
    ...
    const receipt: AuthorityAuditAnchorReceipt = deepFreeze({
      schemaVersion: 1,
      kind: 'authority-audit-anchor-receipt.v1',
      repositoryId: prepared.scope.repositoryId,
      profile: 'protected',
    ...
    const receiptPath = path.join(
      prepared.paths.anchors,
      `${receiptDigest.slice('sha256:'.length)}.json`,
    );
...
export function verifyProtectedAuthorityAuditAnchors(
  scope: AuthorityAuditLedgerScope,
  rawBackends: readonly AuthorityAuditAnchorBackend[],
): AuthorityAuditAnchorVerification {
  const prepared = prepareLedger(scope);
  if (prepared.profile !== 'protected') throw developmentProfile();
  const backends = new Map<string, AuthorityAuditAnchorBackend>();
  for (const rawBackend of rawBackends) {
    const backend = assertAnchorBackend(rawBackend);
    if (backends.has(backend.backendId)) throw anchorBackendInvalid();
  ...
  return withLedgerLock(prepared.paths, prepared.scope.repositoryId, () => {
    ...
    const bySequence = new Map(
      scan.records.map((entry) => [entry.record.sequence, entry.recordDigest]),
    );
    ...
    for (const publication of receipts) {
      ...
      if (backend === undefined) throw anchorBackendRequired();
      const payload = canonicalAnchorPayload({
      ...
      if (
        publication.receipt.repositoryId !== prepared.scope.repositoryId ||
    ...
    return deepFreeze({
      ...
      repositoryId: prepared.scope.repositoryId,
      ok: true,
      currentHeadAnchored: receipts.some(
        ({ receipt }) =>
          receipt.headSequence === scan.headSequence &&
...
export function canonicalAuditDestructionGrantPayload(
  rawPayload: AuditDestructionGrantPayload,
): string {
  return `${canonicalJson(assertAuditDestructionGrantPayload(rawPayload))}\n`;
}

export function canonicalAuditDestructionGrantEnvelope(
  rawEnvelope: AuditDestructionGrantEnvelope,
): string {
  return `${canonicalJson(assertAuditDestructionGrantEnvelope(rawEnvelope))}\n`;
}

export function destroyProtectedAuthorityAuditLedger(
  scope: AuthorityAuditLedgerScope,
  rawEnvelope: AuditDestructionGrantEnvelope,
  input: Readonly<{
    now: string;
    verifyHumanSignature: AuditDestructionSignatureVerifier;
  }>,
  hooks: AuthorityAuditMaintenanceHooks = {},
): AuthorityAuditCompactionResult {
  const now = assertMaintenanceTimestamp(input.now);
  ...
  const canonicalPayload = canonicalAuditDestructionGrantPayload(
    envelope.payload,
  );
  if (
    !input.verifyHumanSignature({
      domain: 'HARNESS_AUDIT_DESTRUCTION_GRANT_V1',
  ...
  return withLedgerLock(prepared.paths, prepared.scope.repositoryId, () => {
    ...
    const tombstonePath = path.join(
      prepared.paths.tombstones,
      `${grantDigest.slice('sha256:'.length)}.json`,
    );
    if (fs.lstatSync(tombstonePath, { throwIfNoEntry: false })) {
      ...
      const tombstone = parseDestructionTombstone(
        readExactPrivateFile(
          tombstonePath,
          MAX_METADATA_BYTES,
          destructionTombstoneInvalid,
        ),
      );
    ...
    const scan = scanLedgerUnderLock(prepared);
    if (
      payload.repositoryId !== prepared.scope.repositoryId ||
    ...
    const target = scan.records.find(
      ({ record }) => record.sequence === payload.throughSequence,
    );
    if (
      target === undefined ||
    ...
    const tombstone: AuthorityAuditDestructionTombstone = deepFreeze({
      schemaVersion: 1,
      kind: 'authority-audit-destruction-tombstone.v1',
      repositoryId: prepared.scope.repositoryId,
      grantDigest,
    ...
    executeAuditMaintenance(
      prepared.paths,
      createMaintenanceJournal(checkpoint, tombstone),
      hooks,
    );
    return compactionResult(
      'protected',
      payload.throughSequence -
        (previousCheckpoint?.prunedThroughSequence ?? 0),
      scan.recordCount -
        (payload.throughSequence -
          (previousCheckpoint?.prunedThroughSequence ?? 0)),
      checkpoint,
...
function ensureAuthorityAuditProfile(
  paths: AuthorityAuditLedgerPaths,
  scope: AuthorityAuditLedgerScope,
): AuthorityAuditProfile {
  ...
  const existing =
    existingStats === undefined
      ? null
      : parseAuthorityAuditProfileState(
          readExactPrivateFile(
            paths.profileState,
            MAX_METADATA_BYTES,
            unsafeFilesystem,
  ...
  const state: AuthorityAuditProfileState = deepFreeze({
    schemaVersion: 1,
    kind: 'authority-audit-profile.v1',
  ...
  if (existing === null) {
    if (!isUninitializedAuditLayout(paths)) throw invalidProfile();
    try {
  ...
  if (stored.repositoryId !== scope.repositoryId) throw invalidProfile();
  if (stored.profile !== requested) throw profileMismatch();
  if (
...
function isUninitializedAuditLayout(paths: AuthorityAuditLedgerPaths): boolean {
  return (
    fs.readdirSync(paths.events).length === 0 &&
...
function assertProfileArtifactBindings(
  paths: AuthorityAuditLedgerPaths,
  scope: AuthorityAuditLedgerScope,
  profile: AuthorityAuditProfile,
): void {
...
function assertProtectedBackendRoot(
  raw: unknown,
  scope: AuthorityAuditLedgerScope,
): string {
...
function assertProtectedConfigurationPath(raw: unknown): string {
  if (
    typeof raw !== 'string' ||
    raw.length === 0 ||
...
function canonicalDestructionPublicKey(raw: unknown): string {
  if (
    typeof raw !== 'string' ||
    raw.length === 0 ||
    raw.includes('\0') ||
...
function canonicalProtectedConfiguration(
  configuration: AuthorityAuditProtectedConfiguration,
): string {
  return `${canonicalJson(configuration)}\n`;
}

function parseProtectedConfiguration(
  bytes: string,
): AuthorityAuditProtectedConfiguration {
  ...
  const core = {
    ...
    destructionPublicKeyPem,
    destructionPublicKeyDigest: assertDigest(
      raw.destructionPublicKeyDigest,
      protectedConfigurationInvalid,
    ),
  };
  const configuration = deepFreeze({
    ...core,
    configurationDigest: assertDigest(
      raw.configurationDigest,
      protectedConfigurationInvalid,
    ),
  });
  if (
...
function readProtectedConfiguration(
  paths: AuthorityAuditLedgerPaths,
): AuthorityAuditProtectedConfiguration {
  return parseProtectedConfiguration(
    readExactPrivateFile(
      paths.protectedConfiguration,
      MAX_METADATA_BYTES,
      protectedConfigurationInvalid,
    ),
  );
...
function ensureFilesystemWormBackend(
  configuration: AuthorityAuditProtectedConfiguration,
): Readonly<{ repositories: string; repository: string; anchors: string }> {
  const backendRoot = configuration.backendRoot;
  assertPrivateDirectory(backendRoot);
  for (const entry of fs.readdirSync(backendRoot)) {
    if (entry !== 'repositories') throw anchorBackendInvalid();
  }
...
function parseAuthorityAuditProfileState(
  bytes: string,
): AuthorityAuditProfileState {
...
function authorityAuditPolicyDigest(
  profile: AuthorityAuditProfile,
): Sha256Digest {
  return sha256Digest(canonicalJson(AUTHORITY_AUDIT_PROFILE_POLICIES[profile]));
}

function assertAuditProfile(
  raw: unknown,
  makeError: () => Error,
): AuthorityAuditProfile {
  if (raw !== 'development' && raw !== 'protected') throw makeError();
  return raw;
}

function scanLedgerUnderLock(
  prepared: Readonly<{
    scope: AuthorityAuditLedgerScope;
    paths: AuthorityAuditLedgerPaths;
    profile: AuthorityAuditProfile;
...
function createRetentionCheckpoint(
  input: Readonly<{
    repositoryId: Sha256Digest;
    profile: AuthorityAuditProfile;
    prunedThroughSequence: number;
    ...
    grantDigest: Sha256Digest | null;
    tombstoneDigest: Sha256Digest | null;
    previousCheckpoint: AuthorityAuditRetentionCheckpoint | null;
  }>,
): AuthorityAuditRetentionCheckpoint {
  if (
  ...
  const checkpoint: AuthorityAuditRetentionCheckpoint = deepFreeze({
    schemaVersion: 1,
    kind: 'authority-audit-retention-checkpoint.v1',
...
function createMaintenanceJournal(
  checkpoint: AuthorityAuditRetentionCheckpoint,
  tombstone: AuthorityAuditDestructionTombstone | null,
): AuthorityAuditMaintenanceJournal {
  const tombstoneDigest =
    tombstone === null ? null : sha256Digest(canonicalTombstone(tombstone));
...
function executeAuditMaintenance(
  paths: AuthorityAuditLedgerPaths,
  journal: AuthorityAuditMaintenanceJournal,
  hooks: AuthorityAuditMaintenanceHooks,
): void {
  ...
  writeExclusivePrivateFile(
    paths.maintenanceJournal,
    canonicalMaintenanceJournal(journal),
    maintenanceInvalid,
    hooks.testAfterJournalPreparation,
  );
  fsyncDirectory(paths.metadata);
  completeAuditMaintenance(paths, journal, hooks);
}

function recoverAuditMaintenance(
  paths: AuthorityAuditLedgerPaths,
  profile: AuthorityAuditProfile,
): void {
  ...
  if (journalStats === undefined) {
    if (
      fs.lstatSync(paths.retentionCheckpointNext, { throwIfNoEntry: false })
    ) {
      throw maintenanceInvalid();
    }
    return;
  }
  const journal = parseMaintenanceJournal(
    readExactPrivateFile(
      paths.maintenanceJournal,
      MAX_METADATA_BYTES,
      maintenanceInvalid,
  ...
  if (
    journal.repositoryId !==
      (`sha256:${path.basename(paths.repository)}` as Sha256Digest) ||
...
function completeAuditMaintenance(
  paths: AuthorityAuditLedgerPaths,
  journal: AuthorityAuditMaintenanceJournal,
  hooks: AuthorityAuditMaintenanceHooks,
): void {
  if (journal.tombstone !== null) {
    const tombstonePath = path.join(
      paths.tombstones,
      `${journal.tombstone.grantDigest.slice('sha256:'.length)}.json`,
    );
    publishImmutableObject(
      tombstonePath,
      canonicalTombstone(journal.tombstone),
      destructionTombstoneInvalid,
      hooks.testAfterTombstonePreparation,
    );
...
function publishRetentionCheckpoint(
  paths: AuthorityAuditLedgerPaths,
  next: AuthorityAuditRetentionCheckpoint,
  hooks: AuthorityAuditMaintenanceHooks,
): void {
  const nextBytes = canonicalCheckpoint(next);
  const current = readRetentionCheckpoint(
    paths,
    next.repositoryId,
    next.profile,
  );
  if (current !== null && canonicalCheckpoint(current) === nextBytes) {
    if (
      ...
      const prepared = readExactPrivateFile(
        paths.retentionCheckpointNext,
        MAX_METADATA_BYTES,
        maintenanceInvalid,
      );
      if (prepared !== nextBytes) throw maintenanceInvalid();
  ...
  if (preparedStats === undefined) {
    writeExclusivePrivateFile(
      ...
      nextBytes,
      maintenanceInvalid,
      hooks.testAfterCheckpointPreparation,
    );
    fsyncDirectory(paths.metadata);
  } else if (
    readExactPrivateFile(
  ...
  if (
    canonicalCheckpoint(
      readRetentionCheckpoint(paths, next.repositoryId, next.profile)!,
    ) !== nextBytes
  ) {
    throw maintenanceInvalid();
  }
...
function removePrunedAuditPrefix(
  paths: AuthorityAuditLedgerPaths,
  checkpoint: AuthorityAuditRetentionCheckpoint,
  hooks: AuthorityAuditMaintenanceHooks,
): void {
  const names = fs.readdirSync(paths.records).sort();
  const retainedResultDigests = new Set<Sha256Digest>();
  for (const name of names) {
    const match = FINAL_RECORD_NAME.exec(name);
    if (match === null || match[1] === undefined) throw invalidLedger();
    if (Number(match[1]) <= checkpoint.prunedThroughSequence) continue;
    retainedResultDigests.add(
      parseCanonicalRecord(
        readRecordPath(path.join(paths.records, name), [1]).content,
      ).resultDigest,
    );
  }
  for (const name of names) {
    ...
    if (sequence > checkpoint.prunedThroughSequence) continue;
    ...
    if (
      record.repositoryId !== checkpoint.repositoryId ||
    ...
    const eventPath = path.join(
      paths.events,
      `${record.resultDigest.slice('sha256:'.length)}.json`,
    );
    if (
      !retainedResultDigests.has(record.resultDigest) &&
      fs.lstatSync(eventPath, { throwIfNoEntry: false })
    ) {
      const eventBytes = readExactPrivateFile(
        eventPath,
        MAX_RECORD_BYTES,
        unsafeFilesystem,
      );
  ...
  if (
    fs.readdirSync(paths.records).some((name) => {
      const match = FINAL_RECORD_NAME.exec(name);
      return (
        match !== null && Number(match[1]) <= checkpoint.prunedThroughSequence
      );
    })
  ) {
    throw maintenanceInvalid();
...
function readRetentionCheckpoint(
  paths: AuthorityAuditLedgerPaths,
  repositoryId: Sha256Digest,
  profile: AuthorityAuditProfile,
): AuthorityAuditRetentionCheckpoint | null {
  ...
  const checkpoint = parseRetentionCheckpoint(
    readExactPrivateFile(
      paths.retentionCheckpoint,
      MAX_METADATA_BYTES,
      maintenanceInvalid,
    ),
  );
...
function reconstructProtectedCheckpointHistory(
  paths: AuthorityAuditLedgerPaths,
): AuthorityAuditRetentionCheckpoint[] {
  const tombstones = readDestructionTombstones(paths).sort(
    (left, right) =>
      left.destroyedThroughSequence - right.destroyedThroughSequence,
  );
  const grantIds = new Set<string>();
  const checkpoints: AuthorityAuditRetentionCheckpoint[] = [];
  let previous: AuthorityAuditRetentionCheckpoint | null = null;
...
function protectedCheckpointDigestHistory(
  paths: AuthorityAuditLedgerPaths,
): Map<number, Sha256Digest> {
  return new Map(
    reconstructProtectedCheckpointHistory(paths).map((checkpoint) => [
...
function assertProtectedCheckpointTombstoneBinding(
  paths: AuthorityAuditLedgerPaths,
  checkpoint: AuthorityAuditRetentionCheckpoint,
): void {
  if (
    checkpoint.profile !== 'protected' ||
  ...
  const tombstonePath = path.join(
    paths.tombstones,
    `${checkpoint.grantDigest.slice('sha256:'.length)}.json`,
  );
  let tombstone: AuthorityAuditDestructionTombstone;
  try {
    tombstone = parseDestructionTombstone(
      readExactPrivateFile(
        tombstonePath,
        MAX_METADATA_BYTES,
        destructionTombstoneInvalid,
  ...
  if (
    tombstone.grantDigest !== checkpoint.grantDigest ||
...
function parseRetentionCheckpoint(
  bytes: string,
): AuthorityAuditRetentionCheckpoint {
  const raw = parseJson(bytes, maintenanceInvalid);
  const checkpoint = assertRetentionCheckpoint(raw);
  if (bytes !== canonicalCheckpoint(checkpoint)) throw maintenanceInvalid();
  return checkpoint;
}

function assertRetentionCheckpoint(
  raw: unknown,
): AuthorityAuditRetentionCheckpoint {
  if (
    !isPlainRecord(raw) ||
  ...
  const tombstoneDigest = assertNullableDigest(
    raw.tombstoneDigest,
    maintenanceInvalid,
  );
  if (
  ...
  return deepFreeze({
    ...
    prunedThroughRecordDigest: assertDigest(
      raw.prunedThroughRecordDigest,
      maintenanceInvalid,
    ),
    ...
    grantDigest,
    tombstoneDigest,
    previousCheckpointDigest: assertNullableDigest(
...
function checkpointDigest(
  checkpoint: AuthorityAuditRetentionCheckpoint | null,
): Sha256Digest | null {
...
function parseMaintenanceJournal(
  bytes: string,
): AuthorityAuditMaintenanceJournal {
  const raw = parseJson(bytes, maintenanceInvalid);
  if (
  ...
  const checkpointValueDigest = assertDigest(
    raw.checkpointDigest,
    maintenanceInvalid,
  );
  const tombstone =
    raw.tombstone === null ? null : assertDestructionTombstone(raw.tombstone);
  const tombstoneDigest = assertNullableDigest(
    raw.tombstoneDigest,
    maintenanceInvalid,
  );
  const journal: AuthorityAuditMaintenanceJournal = deepFreeze({
    schemaVersion: 1,
    kind: 'authority-audit-maintenance-journal.v1',
    repositoryId: assertDigest(raw.repositoryId, maintenanceInvalid),
    checkpoint,
  ...
  if (
    journal.repositoryId !== checkpoint.repositoryId ||
...
function canonicalMaintenanceJournal(
  journal: AuthorityAuditMaintenanceJournal,
): string {
...
function assertAnchorBackend(
  raw: AuthorityAuditAnchorBackend,
): AuthorityAuditAnchorBackend {
  if (
    !isPlainRecord(raw) ||
...
function canonicalAnchorReceipt(receipt: AuthorityAuditAnchorReceipt): string {
  return `${canonicalJson(assertAnchorReceipt(receipt))}\n`;
}

function assertAnchorReceipt(raw: unknown): AuthorityAuditAnchorReceipt {
  if (
    !isPlainRecord(raw) ||
...
function parseAnchorReceipt(raw: string): AuthorityAuditAnchorReceipt {
  const receipt = assertAnchorReceipt(parseJson(raw, anchorInvalid));
  if (raw !== canonicalAnchorReceipt(receipt)) throw anchorInvalid();
  return receipt;
}

function readAnchorPublications(
  paths: AuthorityAuditLedgerPaths,
): AuthorityAuditAnchorPublication[] {
  const result: AuthorityAuditAnchorPublication[] = [];
  for (const name of fs.readdirSync(paths.anchors).sort()) {
    const match = /^([0-9a-f]{64})\.json$/.exec(name);
    if (match === null || match[1] === undefined) throw anchorInvalid();
    const receiptPath = path.join(paths.anchors, name);
    const bytes = readExactPrivateFile(
      receiptPath,
      MAX_METADATA_BYTES,
      anchorInvalid,
    );
...
function assertAnchorDirectoryLocal(directory: string): void {
...
function assertAuditDestructionGrantPayload(
  raw: unknown,
): AuditDestructionGrantPayload {
  if (
    !isPlainRecord(raw) ||
  ...
  return deepFreeze({
    ...
    expectedHeadRecordDigest: assertDigest(
      raw.expectedHeadRecordDigest,
      destructionGrantInvalid,
    ),
    throughSequence: raw.throughSequence,
    throughRecordDigest: assertDigest(
      raw.throughRecordDigest,
      destructionGrantInvalid,
    ),
...
function assertAuditDestructionGrantEnvelope(
  raw: unknown,
...
function assertDestructionGrantTime(
  payload: AuditDestructionGrantPayload,
  now: string,
): void {
  const observed = Date.parse(now);
  if (
    observed < Date.parse(payload.issuedAt) ||
...
function assertDestructionTombstone(
  raw: unknown,
): AuthorityAuditDestructionTombstone {
  if (
    !isPlainRecord(raw) ||
  ...
  return deepFreeze({
    ...
    destroyedThroughRecordDigest: assertDigest(
      raw.destroyedThroughRecordDigest,
      destructionTombstoneInvalid,
    ),
...
function canonicalTombstone(
  tombstone: AuthorityAuditDestructionTombstone,
): string {
...
function parseDestructionTombstone(
  raw: string,
): AuthorityAuditDestructionTombstone {
  const tombstone = assertDestructionTombstone(
    parseJson(raw, destructionTombstoneInvalid),
  );
...
function assertTombstoneDirectoryLocal(directory: string): void {
  for (const name of fs.readdirSync(directory)) {
    const match = /^([0-9a-f]{64})\.json$/.exec(name);
...
function readDestructionTombstones(
  paths: AuthorityAuditLedgerPaths,
): AuthorityAuditDestructionTombstone[] {
  return fs.readdirSync(paths.tombstones).map((name) => {
    ...
    const tombstone = parseDestructionTombstone(
      readExactPrivateFile(
        path.join(paths.tombstones, name),
        MAX_METADATA_BYTES,
        destructionTombstoneInvalid,
      ),
    );
...
function recoverRecordPublications(
  ...
  for (const publication of publications) {
    ...
    const head = records.at(-1);
    const expectedPreviousSequence =
      head?.record.sequence ?? checkpoint?.prunedThroughSequence ?? 0;
...
function scanRecords(
  ...
  checkpoint: AuthorityAuditRetentionCheckpoint | null = null,
): AuthorityAuditLedgerEntry[] {
  ...
  let expectedPrevious: Sha256Digest | null =
    checkpoint?.prunedThroughRecordDigest ?? null;
...
function readExactPrivateFile(
  ...
  return readPrivateFileWithAllowedLinks(
    filePath,
    maximumBytes,
    [1],
    makeError,
...
function readPrivateFileWithAllowedLinks(
  filePath: string,
  maximumBytes: number,
  allowedLinks: readonly number[],
  makeError: () => Error,
...
function writePreparedPrivateFile(
  filePath: string,
  content: string,
  makeError: () => Error,
): void {
...
function writeExclusivePrivateFile(
  ...
  const preparationPath = path.join(
    directory,
    `.${baseName}.${contentDigest}.${crypto.randomUUID()}.publish.tmp`,
  );
...
function publishImmutableObject(
  filePath: string,
  content: string,
  makeError: () => Error,
  testAfterPreparation?: () => void,
...
function recoverPrivateFilePublications(
  directory: string,
  isAllowedFinalName: (name: string) => boolean,
  makeError: () => Error,
): void {
  const recoveredFinalPaths = new Set<string>();
  for (const name of fs.readdirSync(directory)) {
    if (!name.endsWith('.publish.tmp')) continue;
    ...
    const content = readPrivateFileWithAllowedLinks(
      preparationPath,
      MAX_METADATA_BYTES,
      [1, 2],
      makeError,
    ...
    if (finalStats === undefined) {
      ...
      if (currentPreparationStats.nlink !== 1) throw makeError();
      try {
    ...
    if (
      ...
      readPrivateFileWithAllowedLinks(
        finalPath,
        MAX_METADATA_BYTES,
        [1, 2],
        makeError,
    ...
    const currentPreparationStats = fs.lstatSync(preparationPath);
    if (
      currentPreparationStats.nlink === 2 &&
...
function parseJson(raw: string, makeError: () => Error): unknown {
  try {
...
function assertMaintenanceTimestamp(raw: unknown): string {
  if (!isCanonicalTimestamp(raw)) throw maintenanceInvalid();
  return raw;
}
...
function invalidProfile() {
  return workflowError(
    'AUTHORITY_AUDIT_PROFILE_INVALID',
    'Persisted authority audit profile or code-owned policy binding is invalid.',
    ExitCode.staleState,
  );
}
...
function profileMismatch() {
  return workflowError(
    'AUTHORITY_AUDIT_PROFILE_MISMATCH',
    'Authority audit profile is already persisted and cannot be downgraded or mixed.',
    ExitCode.guard,
  );
}
...
function protectedProfile() {
  return workflowError(
    'AUTHORITY_AUDIT_PROFILE_PROTECTED',
    'Protected audit records cannot be pruned by development retention maintenance.',
    ExitCode.guard,
  );
}
...
function developmentProfile() {
  return workflowError(
    'AUTHORITY_AUDIT_PROFILE_DEVELOPMENT',
    'This authority audit operation requires the persisted protected profile.',
    ExitCode.guard,
  );
}
...
function protectedConfigurationInvalid() {
  return workflowError(
    ...
    'Protected authority audit configuration is missing, malformed, unsafe, or no longer bound to its pinned backend and destruction key.',
    ExitCode.staleState,
  );
}
...
function protectedConfigurationMismatch() {
  return workflowError(
    'AUTHORITY_AUDIT_PROTECTED_CONFIGURATION_MISMATCH',
    'Protected authority audit backend and destruction public key are already pinned and cannot be replaced.',
    ExitCode.guard,
  );
}
...
function maintenanceInvalid() {
  return workflowError(
    ...
    'Authority audit retention checkpoint or maintenance journal is missing, malformed, or tampered.',
    ExitCode.staleState,
  );
}
...
function anchorInvalid() {
  return workflowError(
    'AUTHORITY_AUDIT_ANCHOR_INVALID',
    'Protected authority audit anchor receipt is malformed, tampered, or not bound to ledger history.',
    ExitCode.staleState,
  );
}
...
function anchorBackendInvalid() {
  return workflowError(
    'AUTHORITY_AUDIT_ANCHOR_BACKEND_INVALID',
    'Protected authority audit anchor backend did not produce a verifiable bounded receipt.',
...
function anchorBackendRequired() {
  return workflowError(
    'AUTHORITY_AUDIT_ANCHOR_BACKEND_REQUIRED',
    'Protected authority audit verification requires the exact backend verifier named by every receipt.',
    ExitCode.guard,
  );
}
...
function destructionGrantInvalid() {
  return workflowError(
    ...
    'Audit destruction requires an exact domain-separated, human-verified, single-use grant.',
    ExitCode.verification,
  );
}
...
function destructionGrantExpired() {
  return workflowError(
    'AUTHORITY_AUDIT_DESTRUCTION_GRANT_EXPIRED',
    'Audit-Destruction Grant is not currently valid.',
    ExitCode.staleState,
  );
}
...
function destructionGrantBindingInvalid() {
  return workflowError(
    ...
    'Audit-Destruction Grant does not bind the exact current ledger head and prefix.',
    ExitCode.staleState,
  );
}
...
function destructionGrantConsumed() {
  return workflowError(
    'AUTHORITY_AUDIT_DESTRUCTION_GRANT_CONSUMED',
    'Audit-Destruction Grant already has a durable tombstone and cannot replay.',
    ExitCode.conflict,
  );
}
...
function destructionTombstoneInvalid() {
  return workflowError(
    ...
    'Protected authority audit destruction tombstone is missing, malformed, or tampered.',
    ExitCode.staleState,
  );
}
