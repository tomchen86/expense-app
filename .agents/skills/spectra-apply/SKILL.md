...
1a. **Verify the implementation baseline**

Run `git status --porcelain --untracked-files=all` before marking the change in progress or editing code.

Spectra 2.3.1 records every dirty non-OpenSpec file when `spectra task done` runs; it does not distinguish files that were already dirty before this change. Compare the dirty source-file list with `.spectra/touched/<name>.json` when that file exists.

- If the worktree has no dirty source files, continue.
- If every dirty source file is already tracked for this change, show the list and continue.
- If untracked-by-this-change source files exist, show them and STOP. Ask the user to commit/separate them, use an isolated worktree, or explicitly confirm that every listed file belongs to this change before continuing.

Never silently attribute a pre-existing dirty file to the selected change.
...
5. **Check project preferences**
   ...
   **Database-test safety:** Before running any API test command, require `TEST_DATABASE_URL` to be explicitly set to a disposable PostgreSQL database whose data the user has confirmed may be truncated or dropped. Never use the development-database fallback. Prefer the narrowest relevant spec first; if no disposable database is available, skip database-writing tests and report that limitation.
