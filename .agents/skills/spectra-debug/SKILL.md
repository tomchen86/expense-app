...
- **Consider git bisect** when the bug is a regression. Because it changes checked-out state, use it only in a clean isolated worktree and after explicit user approval
...
4. **Run the broadest safe relevant test suite** — ensure no regressions
5. **Check related code** — if this pattern exists elsewhere, fix those too

For the API, never run `pnpm --filter api test` unless `TEST_DATABASE_URL` is explicitly set to a disposable PostgreSQL database and the user has confirmed that its data may be truncated or dropped. Prefer a targeted API spec first. If no disposable test database is available, skip database-writing suites and report the limitation.
...
| "Let me try reverting this change"        | In a clean worktree, ask before using bisect     |
...
- **Protect data** — Never point API tests at a development or shared PostgreSQL database
