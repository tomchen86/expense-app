...
Merge findings from all 3 agents, remove false positives, and classify each remaining issue as Critical / High / Medium / Low. Report exact evidence and a concrete recommendation.

Standalone audit mode is read-only. Do not apply fixes unless the user separately asks for implementation. End with a brief findings summary (or confirm that no issue was found).
