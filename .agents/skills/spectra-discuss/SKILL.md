...
**IMPORTANT: Discuss mode is read-only thinking, not implementation.** You may read files, search code, and investigate the codebase, but you must NEVER write code, edit artifacts, or implement features. If the user asks you to create artifacts or implement something, conclude the discussion and direct them to `$spectra-propose` or `$spectra-ingest`.
...
Announce which mode you picked and why: "Found `expense.controller.ts`, `expense.service.ts`, and `expenseStore.ts` — I have enough context to list my assumptions." or "Didn't find much related code — I'll ask questions instead."
...
1. **New expense rules belong in the API service**
   Evidence: existing expense behavior is in `apps/api/src/services/expense.service.ts`
   If wrong: the rule may belong in the mobile store or a new domain service

2. **Use the existing feature store for mobile state**
   Evidence: `apps/mobile/src/store/features/expenseStore.ts` already manages expense state
   If wrong: parallel state would cause sync bugs

3. **Persistence remains behind the API boundary**
   Evidence: TypeORM entities and services live under `apps/api/src/`
   If wrong: an offline-first write path may require explicit reconciliation in the mobile store
```
...
- A **new module** (for example, a NestJS module/service or a new React Native feature store).
- A **new API contract** (a new endpoint, DTO, authentication boundary, or mobile-to-API message shape).
- A **cross-layer mobile ↔ API ↔ PostgreSQL flow** that did not exist before.
- A **new storage abstraction** (for example, a TypeORM entity/migration, Zustand persistence layer, or sync adapter).
...
Present the summary and recommend the exact capture destination. Do not write it in discuss mode; direct the user to `$spectra-propose` for a new change or `$spectra-ingest` for an existing one.
...
- Or recommend `$spectra-ingest <change-name>` to capture the decision in existing artifacts
