...
1. **Determine the requirement source**
   ...
   - If found, resolve it relative to the project root; when only a filename is provided, also check `docs/planning/<filename>`
...
4. **Create the change directory**
   ...
   Before creation, check active changes, parked changes, `openspec/changes/archive/*-<name>/`, and `.spectra/touched/<name>.json`. If the name was used before or stale tracking exists, do not reuse it silently: propose a fresh descriptive name or ask the user to resolve the old tracking/archive explicitly.
...
5. **Write the proposal**
   ...
   **IMPORTANT — file path rules for the `## Impact` section:**
   - All file paths SHALL be written relative to the project root (e.g., `apps/api/src/main.ts`, `apps/mobile/app/index.tsx`, `openspec/specs/auth/spec.md`).
...
6. **Get the artifact build order**
   ...
   - `artifacts`: list of all artifacts with their status, in workflow order

   The status output does not include dependency details for every ready/done artifact. Read `dependencies` and `unlocks` from `spectra instructions <artifact-id> --change "<name>" --json`.
...
7. **Create remaining artifacts in sequence**
   ...
   - The instructions JSON includes:
     ...
     - `locale`: The language to write the artifact in (e.g., "Japanese (日本語)"). If present, you MUST write the artifact content in this language. Exception: spec files (`specs/*/spec.md`) MUST always be written in English regardless of locale, because they use normative language (SHALL/MUST).
...
11. **Leave the change active and end the workflow**
    ...
    Leave the proposal under `openspec/changes/<name>/` so it remains visible to Git and can be compared with the roadmap and current-state document. Inform the user that `$spectra-apply <change-name>` starts implementation when ready.

    Only run `spectra park "<name>"` when the user explicitly asks to park the change after being told that parking moves it outside the normal worktree and Git status.
    ...
    The propose workflow ENDS here. Do NOT invoke `$spectra-apply` and do not park automatically. The user decides when to start implementation or explicitly park the change.
