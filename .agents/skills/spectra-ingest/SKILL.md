...
Use an explicit plan file when one is supplied. Otherwise, look for a relevant plan under `docs/planning/` or use conversation context.

**Prerequisites**: This skill requires the `spectra` CLI. If any `spectra` command fails with "command not found" or similar, report the error and STOP.

**Input**: Optionally specify an existing change name, and optionally a plan file.

- `$spectra-ingest add-receipts`
- `$spectra-ingest add-receipts docs/planning/PLAN-RECEIPTS.md`
- `$spectra-ingest docs/planning/PLAN-RECEIPTS.md` (select the change afterward)
- `$spectra-ingest` (select the change and use conversation context or a detected plan)

**Steps**

1. **Resolve the change and requirement source**

   First run `spectra list --json` and `spectra list --parked --json` so an argument can be distinguished from a plan filename.

   a. **First argument matches an active or parked change** → preselect that change.
   - If a second argument is present, resolve it as a path relative to the project root; when it is only a filename, also try `docs/planning/<filename>` and append `.md` when needed. If it does not resolve to a file, report the error and STOP.
   - If there is no second argument, use a plan detected from conversation/docs or fall back to conversation context.

   b. **Argument resolves to a file** → use it as the plan source and select the existing change in Step 3. For a bare filename, also try `docs/planning/<filename>` and append `.md` when needed.

   c. **Argument matches neither a change nor a file** → report both checks and STOP. Do not silently treat a change name as a missing plan path.

   d. **No argument, plan file detectable**:
   ...
   e. **No argument, no plan file detectable**:
   - Check `docs/planning/` for recent Markdown files
   ...
   f. **Conversation context fallback** (no plan files found at all):
...
2. **Parse the plan structure** (skip if using conversation context)
   ...
   Project plan files typically contain:
...
3. **Check for active changes** (REQUIRED — ingest only updates existing changes)
   ...
   - If a change was preselected in Step 1 → use it after verifying that it is still in this combined list
...
5. **Update artifacts**
   ...
   The instructions JSON includes `locale` — the language to write artifacts in. If present, you MUST write the artifact content in that language. Exception: spec files (`specs/*/spec.md`) MUST always be written in English regardless of locale, because they use normative language (SHALL/MUST).
   ...
   **When updating an existing change:**
   - Reconcile new context with the existing proposal; preserve accurate history while replacing obsolete pending requirements when necessary
   ...
   - Pending content that is no longer valid may be revised or removed, but summarize the reconciliation in the change artifacts
...
- **NEVER** modify the source plan file under `docs/planning/` unless the user explicitly asks for that separate update
